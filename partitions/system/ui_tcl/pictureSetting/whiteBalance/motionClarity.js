...
function toRight()
{
	...
	else if(focusType=="motionMode")
	{
		...
		if(top.g_motionClarity == "Off")
		{
			...
			if(presetPositionH==2)
			{
				setting.previewMemcConfig(4);
			}
			else
			{
				setting.previewMemcConfig(presetPositionH);
			}
		}
		else
		{
...
function toOK()
{
	...
	{
		if(top.g_motionClarity == "Off" && presetPositionH == 2)
		{
			picMotionModeVal = 4;
		}
		else
...
function toBack()
{
	...
	{
		setting.previewMemcConfig(picMotionModeVal);
		updateMotionData(picMotionConName[0]);
		$("preSetBar").style.display="none";
		parent.showPage(3);
		focusType="picMotionList";
...
function initVarValue()
{
	try
	{
	picMotionModeVal       = setting.motionEnhancementSwitch; 
	picMotionInterVal      = setting.motionInterpolation;
	picLEDMotionVal        = setting.lEDMotionClarity;
...
function showMotionModeMenu()
{
	...
	if(picMotionModeVal!=4&&setting.checkMotionModeIsUser(picMotionModeVal)==1)
	{
...
{
	var div=lis[3].getElementsByTagName("div");
	div[0].innerHTML=picLEDMotion[picLEDMotionVal];
}
...
function showMotionMode()//显示该选项的选择控制界面和准备选择选项
{
	...
	console.log("picMotionModeVal="+picMotionModeVal+"top.g_motionClarity=="+top.g_motionClarity);
	if(top.g_motionClarity == "Off" )
	{
		showPersetSelect(3);
		...
		spans[3].innerHTML = picMotionMode[4];
		if(picMotionModeVal>2)
		{
			presetPositionH = 2;
		}
		else
...
{
	showPersetSelect(3);
	$("preSetOption").innerHTML=picMotionConName[2];
	var spans=$("preSetBar").getElementsByTagName("span");//picPreset[pictureMode];
	for(var i=0;i<3;i++)
	{
		spans[i+1].innerHTML=picLEDMotion[i];
	}
	presetPositionH=picLEDMotionVal;
	top.doElementScroll(presetSpans,presetPositionH);
	$("preSetSelectFocus").style.left=305+presetPositionH*presetFocusWidth+"px";
	$("currentOption").style.left = 320+presetPositionH*presetFocusWidth+"px";
	$("currentOption").style.display = "block";
...
function checkShowMenu()
{
	try
	{
		console.log("picMotionModeVal=="+picMotionModeVal+"getPanelFrequency=="+setting.getPanelFrequency+"picMotionInterVal=="+picMotionInterVal);
		if(picMotionModeVal!=4)
		{
		...
		{
			if(setting.getPanelFrequency==120)
			{
				menuList[1].showFlag=0;
			}else
			{
		...
		var pic4k1k = 0;//setting.checkPQis4K1k;
		console.log("I get 4k1k value="+pic4k1k);
		if(pic4k1k==1 || setting.getPanelFrequency==120)
		{
