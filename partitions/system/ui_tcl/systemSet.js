...
var systemSetTitleWords 	= [top.sysOptions[0], top.chScanOptions[0], top.sysOptions[15],top.sysOptions[1],  top.sysOptions[1], top.sysOptions[2],
	                           ...
	                           top.footballModeName,top.ClosedcaptionName, top.sysOptions[12], top.sysOptions[13],top.eWarrantyCardWords[0],top.sysOptions[11]];
...
if((top.isAUFlag && 26 == channel.systemCountry) ||
    ("IFFALCON" == top.g_setting.getProperty("ro.sita.model.MODEL_MANUFACTURER_NAME_CFG.MANUFACTURER_NAME", "TCL") && 58 == channel.systemCountry)) //AU选到新西兰国家后隐藏shop with demo,越南雷鸟也隐藏
    var homeShopWords 			= [top.initLocations[0],top.initLocations[1]];
else
...
{name:"E_warranty_Card",param:function(){doE_warranty_Card();},showFlag:1},
...
function toDown()
{
	...
	{
		form_position_h = form_position_h == homeShopWords.length - 1 ? 0 : form_position_h + 1;
		$("listFormFocus").style.top = form_position_h * FORM_STEP + "px";
		top.doEleScrForNoOption(listFormLiList, form_position_h, 1);
	}
...
//		refreshFocusOfMenuList();
	...
	else if(focusOn == "location")
	{
		if(form_position_h == 0)
		{
			form_position_h = homeShopWords.length - 1;
		}
		else
		{
			form_position_h--;
...
function doEnter()
{
	...
	else if(focusOn == "country")
	{
        ...
        var url = setting.getProperty("ro.sita.model.Apps.launcher_branch", "null");
        if ( url.indexOf("launcher_ui3") != -1 || channel.systemCountry == 52)
        {
        ...
        {
            console.log("[ldc] Close GloboPlay service in next boot.");
            setting.setAppPersistent("GloboPlay",0);
        }
	...
	else if(focusOn == "location")
	{
		try
		{
			...
			$("listFocus").src=mainFocus[0];
			$("listForm").style.display = "none";
            var prelocationValue = setting.location;
			...
			{
				menuList[footballModePosIndex].showFlag = 0;
				setting.footballMode = 0;
				refreshAllMenuListShow();
			}
			
			if(locationValue > 0 && demo_videos_name != "")
			{
                menuList[demoModePosIndex].showFlag = 1;
                refreshAllMenuListShow();
			}
            ...
            */
			...
			// 	return;
			// }
			//播放eSticker时，在setting菜单切换location需要停止eSticker
			if (prelocationValue != locationValue && top.iseStickerPlaying == 1)
			{
                top.stopeSticker();
			}
			top.showEsticker();
...
function initCountryList()
{
	if(top.isCAFlag){
		...
		if((top.supportATVSource != 1) || (top.supportDTVSource != 1)){
			if (top.isIRNFlag)
			{
				countryIndexList = top.mlmCountryIndexIRN;
				countryListWords = new Array(countryIndexList.length);
			}
			else if(top.isAPFlag)
			{
				countryIndexList = top.mlmCountryIndexAP;
				countryListWords = new Array(countryIndexList.length);
			}
		}
	}
...
function doLocation()
{
	...
	var topOffset = SUB_MENU_START + SUB_MENU_STEP*list_position_h - 10;
	var leftOffset = 774;
	top.initListForm($("listForm"),topOffset,leftOffset,listFormLiList,homeShopWords,homeShopWords.length,form_position_h,0);
	focusOn = "location";
}
...
function doE_warranty_Card()
{
	...
	{
        parent.popWindow(2,"eWarrantyCard.html");
	}
	else
	{
        top.$("operatePage").src = "";
        top.$("otherPage").src = "activationTips.html";
...
function refreshUIWhenLanguageChange()
{
	...
	systemSetTitleWords 	= [top.sysOptions[0], top.chScanOptions[0],top.sysOptions[15],top.sysOptions[1], top.sysOptions[1], top.sysOptions[2], top.sysOptions[3], 
							   ...
							   top.ClosedcaptionName,top.sysOptions[12], top.sysOptions[13], top.eWarrantyCardWords[0], top.sysOptions[11]];
	for(var i = 0; i < menuList.length; i++)
	{
...
function doCheck(which)
{
	...
	if(passwordLength == 4)
	{
		...
		else if (hotelEnable && ((list_position_h + focus_off_set == findPosition("Lock")) ||
                	(list_position_h + focus_off_set == findPosition("Reset_shop"))))
		{
            superPword = "2013";
            pword = factory.hotelPassword;//"1968";
            console.log("--hzm get hotel password = " + pword);
		...
		else
		{
			...
			if(list_position_h + focus_off_set == findPosition("Country"))//Country
			{
				doCountryLA();
				parent.showPage(1);
				ttsRead();
...
function initResetShop()
{
	...
	resetFormSpanList[0].innerHTML = systemSetTitleWords[resetShopPosIndex];
...
function showPromptCont()//显示提示语内容函数
{
	if(focusOn == "menu" && parent.currentShowMenu == 1)
	{
        ...
        else if(name == "footballmode")
        {
            ...
            $("promptWin2").style.top = -340 - 35 + list_position_h * LIST_BOX_HEIGHT + "px";
...
function initAllMenuList()
{
	...
	else
	{
       ...
       // console.log("======================= LEDMode : " + LEDMode + "======================");
		...
		console.log("setting.getCardState() : " + setting.getCardState());
		if(top.isEUFlag == 1){
			if(inputSource != 1 || (top.supportCI == 0 && top.supportCII == 0))
				menuList[commonInterfacePosIndex].showFlag = 0;
		}
		else{
			if(inputSource != 1 || setting.getCardState() == 0)
				menuList[commonInterfacePosIndex].showFlag = 0;
		}
		 ...
		 * 越南电子保单
         */
        if (top.isAPFlag !=1 || setting.location != 0 || (index != 58 && index != 82 ))
            menuList[eWarrantyCardPosIndex].showFlag = 0;
        //越南电子保单end
		...
		//factory request by wujy
		if(hotelEnable)
		{
			menuList[instantPowerOnPosIndex].showFlag = 0;
			//menuList[resetShopPosIndex].showFlag = 0;
		}
	}
...
function refreshAllMenuListShow()
{
	...
	{
		menuList[footballModePosIndex].showFlag = 1;
	}
	for(var i = 0; i < menuList.length; i++)
	{
...
function init()
{
    ...
    if(demoVideoPath != "NULL")//有内置视频目录
	{
        ...
        if (playList.length > 0)//有目录有视频
		{
            ...
            {
            	var y = i + 1;
                demo_videos_name[i] = "Demo"+y;//playList[i].name;
