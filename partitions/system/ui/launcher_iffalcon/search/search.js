...
function init()
{
    ...
    $("popularSearches").style.direction = top.textDirection;

	$("searchText").innerHTML = searchPageWords[2];
    $("searchText").style.direction = top.textDirection;
...
function keyProcess(evt)
{
	...
	if (keyCode > 352 && keyCode < 379) {
		if (currentFocusId == "Frame") {
			writeString += character[keyCode - 353];
			realKey += character[keyCode - 353];
...
function setWord()
{
	...
	//1、储存上次输入后的clientwidth
	lastWidth = searchText.clientWidth;

	searchText.innerHTML = "<font class='inputString'>" + writeString + "</font>";
	
	var currentwidth = searchText.clientWidth;
	
	
	var styleInfo = window.getComputedStyle ? window.getComputedStyle(searchText) : searchText.currentStyle;

	var left = parseInt(styleInfo.left.substr(0,styleInfo.left.length-2));
	if(currentwidth>1010||((currentwidth-lastWidth)<0&&left<0))//此时再输入的话，就要超出边界了。或删除时。要移动里面的div的left，使之移动
	{
		searchText.style.left = (left-(currentwidth-lastWidth));
	}
}
