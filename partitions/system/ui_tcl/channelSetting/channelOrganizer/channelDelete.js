...
function init()
{
    ...
    atvcount = channel.getChListCountByServiceType(0);
    dtvcount = channel.getChListCountByServiceType(1) + channel.getChListCountByServiceType(2)+channel.getChListCountByServiceType(3);
	// channelList =channel.getChannelList(0,currentChannelInfo.index,1);
    console.log("--hzm atvcount = " + atvcount + "  dtvcount = " + dtvcount);
...
function toOK()//按OK键
{
	...
	else
	{
		if(isDelete==1)
		{
            console.log("--hzm channelList["+positionV+"].index = " + channelList[positionV].index + "  serviceType = " + channelList[positionV].serviceType);
            console.log("--hzm lastindex = " + channel.endIndex);
            ...
            if (atvcount > 0 && dtvcount > 0)
            {
                ...
                {
                    endchannelindex = channel.endIndex;
                }
                else
                {
                    endchannelindex = channel.endIndex - atvcount;
                }
            }
            else
            {
            ...
            console.log("--hzm endchannelindex = " + endchannelindex);
            channel.insertChannel(channelList[positionV].index,endchannelindex);
            //channelList[positionV].isDelete = 1;
            channel.deleteChannel(endchannelindex);
			var length=channelList.length;
...
function ttsRead()
{
	...
	else if (focusType == "deleteRemind")
	{
		ttsName = ChDelContentName + chDeletDes + " ";
		if (isDelete == 1)
		{
			currentStatus = yesNo[0];
		}
		else
		{
			currentStatus = yesNo[1];
		}
	}
	else
	{
