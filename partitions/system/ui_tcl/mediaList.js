...
function initMediaListPage()
{
	...
	{
		focusIndex = 1;
		currentMediaType = PIC_TYPE;//PIC files
		previousMediaType = PIC_TYPE;
	}
	else
	{
		focusIndex = 0;
...
function refreshFileInfo(filetmp)
{
	...
	else
	{
		...
		console.log("=====refreshFileInfo=====   filetmp.size = " + filetmp.size);
        if (filetmp.size == 0)
            refreshZeroSizeFileInfoDetail(filetmp.name);
        else
