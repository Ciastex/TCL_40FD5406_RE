...
var channel_source = ["ATV","DTV","AV","CMP","HDMI1","HDMI2","HDMI3","HDMI4", "AV2"];
var SOURCE_ID_MAX = 8;
var picture_curve_setting = ["Backlight","Brightness","Contrast","Saturation","Sharpness","Tint"];
	...
	else if(form_flag == 1)
	{
		switch(form_position_h)
		{
			case 0:
				...
				if(source_input < 0)
				{
					source_input = SOURCE_ID_MAX;
				}
				span_contain_list[form_position_h].innerHTML = channel_source[source_input];
				setInputSourceUiRemapMw();
				if(source_input_mw == 1 || source_input_mw == 0)
				{
					channel.setServiceType(source_input_mw);
				}
                channel.inputSource = source_input_mw;
	...
	else if(form_flag == 3)
	{
		...
		//lvds		
			...
			case 7:
				...
				changeSSCToLeftButton(form_position_h,200);
				factory.lvdsSpan = tempValue;
				break;
			case 8:
				tempValue = factory.lvdsStep;
				changeSSCToLeftButton(form_position_h,60);
...
function toRight()
{
	...
	else if(form_flag == 3)
	{
		...
		//ddr
			...
			case 2:
				tempValue = factory.miu0Step;
				changeSSCToRightButton(form_position_h,20);
				factory.miu0Step = tempValue;
				break;
		//frc		
			case 3:
		...
		//lvds
			...
			case 7:
				tempValue = factory.lvdsSpan;
				changeSSCToRightButton(form_position_h,200);
				factory.lvdsSpan = tempValue;
				break;
			case 8:
				tempValue = factory.lvdsStep;
				changeSSCToRightButton(form_position_h,60);
				factory.lvdsStep = tempValue;
				break;
		//osd
			case 9:
...
function changeToUpButton(position, maxPostion)
{
	...
	if(p == TAP_POSITION && (locale == "EU" || support_pvr == 0) && form_flag == 0)
	{
		div_contain_list[p + 1].style.background = "#CCC";
		div_contain_list[p].style.background = "#FF0";
		p = TAP_POSITION-1;
	}
...
function changeToDownButton(position, maxPostion)
{
	...
	{
		if(p==TAP_POSITION-1 && (locale=="EU" || support_pvr == 0) && form_flag == 0)
		{
			div_contain_list[p + 1].style.background = "#FFF";
			if(TAP_POSITION>=maxPostion)
			{
				div_contain_list[1].style.background = "#FF0";
				p=0;
				return p;
			...
			{
				div_contain_list[p + 3].style.background = "#FF0";
				p = p+2;
				return p;
			}
		}
...
function init()
{
	...
	config_V2 = setting.getProperty("ro.sita.cfg.ver","");
	if(config_V2 == "2")
	{	var defaulSourceList = "ATV:ATV,DTV:DTV,AV:AV,YPBPR:YPBPR,HDMI1:HDMI1,HDMI2:HDMI2,HDMI3:HDMI3,HDMI4:HDMI4";
...
function showMain()
{
	...
	if(arrList.length>=2)
	{
		...
		{
			span_contain_list[8].innerHTML = "N/A";//start_stop[top.start_stop_flag];
...
function showOverscanAdjust()
{
	...
	sourceMwRemapToUi(channel.inputSource);
	span_contain_list[0].innerHTML = channel_source[source_input];
	span_contain_list[1].innerHTML = factory.getOSACrop(0)
	span_contain_list[2].innerHTML = factory.getOSACrop(1);
	span_contain_list[3].innerHTML = factory.getOSACrop(2);
...
/*
*position???1μ?????￡?maxValue??ê?D??éè?μ?×?′ó?μ
*/
