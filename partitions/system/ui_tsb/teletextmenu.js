...
function notifyProcess(evt)
{
	...
	switch(msg)
	{
		...
		{
			//调用closettx()会发送此消息，但该函数本身会把图文关闭。没必要再跳
			//document.location.href = "channelPlay.html";
		}
		break;
		...
		{
			//console.log("-------MSG E_STR_SCREEN_OFF--");
			setting.closettx();
			document.location.href = "channelPlay.html";
			top.systemEventProc(evt);	
		}
...
function keyproc(evt)
{
	...
	{
		if((keycode<top.VK_0||keycode>top.VK_9)&&keycode!=top.VK_RED&&keycode!=top.VK_GREEN&&keycode!=top.VK_YELLOW&&keycode!=top.VK_BLUE)//按虚拟遥控器会产生的按键,如果不是数字键，或者颜色键的话
		{
	...
	switch(keycode)
	{
		case top.VK_VIRTUAL_REMOTE:
			...
			{
				top.document.getElementById("virtualRemote").contentWindow.showVirtualRemote("Teletext");
			}
			else
			{
				//隐藏虚拟遥控器
