...
function toLeft()
{
	if(top.isEUFlag && country == 6 && scanMode == 2)//YouSee
	{
        if(list_position_h == 0)
        {
			if(scanMode <= 0)
	...
	else if(scanMode == 4/*ziggo*/ || scanMode == 3/*UPC*/)
	{
		if(list_position_h == 0)
		{
			doLeftFreqInNumberInput(autoFreqContentSpanList);
	...
	else if(scanMode == 1)//advanced
	{
		if(list_position_h == 0)
		{
			if(channel.getResetQuickStatus(country) || (top.isEUFlag && country == 6)|| (top.isEUFlag && country == 7 ))//判断是否有快速搜台选项,有快速搜台,当前只有德国,芬兰)
			{
				if(scanMode <= 0)
				{
					scanMode = 2;//quick
	...
	else if(scanMode == 0  || scanMode == 2)
	{
		if(list_position_h == 0)
		{
			if(channel.getResetQuickStatus(country) || (top.isEUFlag && country == 6)|| (top.isEUFlag && country == 7))//判断是否有快速搜台选项,有快速搜台,当前只有德国)
			{
				if(scanMode <= 0)
				{
					scanMode = 2;//quick
...
function toRight()
{
    if((top.isEUFlag && country == 6 && scanMode == 2) ||(top.isEUFlag && country == 7))//YouSee
	{
		if(list_position_h == 0)
		{
			if(scanMode >= 2)
...
function toUp()
{
	...
	else if(top.isEUFlag && country == 6 && scanMode == 2)//YouSee
	{
		if(list_position_h <= 0)
		{
			list_position_h = 5;
...
function doEnter()
{
	if(((scanMode == 4/*Ziggo*/ || scanMode == 3/*UPC*/) && list_position_h == 4) || (scanMode == 1/*Advanced*/ && list_position_h == 5) || (top.isEUFlag && country == 6 && scanMode == 2 && list_position_h == 5))
	{
		...
		else
		{
			...
			symbolRate = s;
			networkId = n;
			if((top.isEUFlag) && (country == 6) && (scanMode == 2)){
				scanMode =1 ; //Yousee like Advance mode
			}
...
function initAutoScan(scanModeValue,countryValue)
{
	...
	if(top.isEUFlag && countryValue == 6 && scanModeValue == 2)//EU YouSee
	{
        scanOptionsLiList[0].style.display = "block";
        $("autoModeContent").innerHTML = scanModeWords[2];
...
function init()
{
	...
	if(top.isEUFlag)//EU dvbc scan default full
	{
        scanMode = 0;//full
		if(country == 6)//EU francis requirement
            scanModeWords = ["Full","Advanced","YouSee"];
		if(country == 7)//EU Finland require Qicukly
		{
			scanMode = 2;//Quickly
