...
var soundSetTitleWords 		= [top.picOptions[15], top.souOptions[1], top.souOptions[8], top.souOptions[9], top.souOptions[10], top.isAPFlag?top.audioDescriptionName[1]:top.audioDescriptionName[9],
...
var progressDivList;
var maxVolume = 0;
var bmanualSetSpeaker=2;           //是否手动设置speaker 开关 0： off  1： on  2： 默认
...
function notifyProcess(evt)
{
	...
	switch(msg)
	{
		case top.E_HP_PLUGIN:
			...
			if(top.isEUFlag) {
				bmanualSetSpeaker = setting.getProperty("app.ui.bmanualSetSpeaker", "2");
				if( bmanualSetSpeaker != 2 ){

					speakerStatus = bmanualSetSpeaker;
					setting.speakerStatus = bmanualSetSpeaker;
				//	console.log("hhf  E_HP_PLUGIN speakerStatus = " + speakerStatus + "setting.speakerStatus" + setting.speakerStatus);
				}
			}
				prepareMenuShow();
	                refreshMenuShow();
		...
		case top.E_HP_PLUGOUT:
			...
			if(top.isEUFlag) {
				...
				if (bmanualSetSpeaker == 1) {
					speakerStatus = bmanualSetSpeaker;
					setting.speakerStatus = bmanualSetSpeaker;
					//	console.log("hhf  E_HP_PLUOUT speakerStatus = " + speakerStatus + "setting.speakerStatus" + setting.speakerStatus);
				}
...
function doEnter()
{
	...
	else if(focusOn == "speakerStatus")
	{
		...
		setting.setProperty("app.ui.bmanualSetSpeaker",speakerStatus);
	//	console.log("hhf  focusOn speakerStatus = " + speakerStatus + "setting.speakerStatus" + setting.speakerStatus);
		prepareMenuShow();
		refreshMenuShow();
...
function toLeft()
{
	...
	else if(focusOn == "btDeviceVolume")
	{
        ...
        setting.bluetoothVolume = btDeviceVolume;
...
function init()
{
   ...
   //showPromptCont();
	...
	btDeviceVolume = setting.bluetoothVolume;//bluetooth.getBluetoothVolume();
	if(top.isEUFlag){
		 bmanualSetSpeaker = setting.getProperty("app.ui.bmanualSetSpeaker","2");
		if( bmanualSetSpeaker != 2 ) {
			var devCurNum = setting.getProperty("app.ui.audioDeviceStatus", "000");//2 ARC; 1 bt; 0:hp
			if (devCurNum == "000" && bmanualSetSpeaker == 0) {
				speakerStatus = setting.speakerStatus;
			}
			else{
				speakerStatus = bmanualSetSpeaker;
			}
			//	console.log("hhf  init speakerStatus = " + speakerStatus + "setting.speakerStatus" + setting.speakerStatus);
		}else{
			speakerStatus = setting.speakerStatus;
		}
	}else{
	...
	$("adjustProgMain").onmousedown = function(evt){
		...
		if(evt.button == 0){//left key
			...
			else if(focusOn == "btDeviceVolume")
			{
				...
				$("adjustProgValue").innerHTML = btDeviceVolume;
                setting.bluetoothVolume = btDeviceVolume;//bluetooth.setBluetoothVolume(btDeviceVolume);
				progressDivList[0].style.width = parseInt(btDeviceVolume*EXTRA_PROGRESS_WIDTH/100,10) + "px";
				progressDivList[2].style.width = parseInt((100 - btDeviceVolume)*EXTRA_PROGRESS_WIDTH/100,10) + "px";
			}
			/*
