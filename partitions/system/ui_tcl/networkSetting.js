var netConName          = [top.netOptions[0],top.netOptions[1],top.netOptions[2],top.netOptions[3],top.netOptions[4],
                            top.netOptions[5],top.netOptions[6],top.homePageBackUp[1],top.netOptions[7],top.netOptions[8],
                            top.netOptions[9],top.netOptions[10],top.netOptions[11],top.netOptions[12],top.netOptions[13],
                            top.netOptions[14],top.netOptions[15],top.netOptions[16],top.netOptions[17],top.netOptions[19],
                            top.bt_title,top.proxySetting[0]];
...
var netInterfaceval        = 0;                 //保存Interface值
...
{name:netConName[7],func:function(){showGuideOnSelMenu();},showFlag:0},
{name:netConName[8],func:function(){parent.popWindow(2,"nScreenStart.html");},showFlag:1},
{name:netConName[9],func:function(){parent.jumpPage(2,"networkSetting/netflixSetting.html")},showFlag:1},
{name:netConName[10],func:function(){parent.popWindow(2,"networkSetting/miracast.html");},showFlag:1},
{name:netConName[20],func:function(){parent.popWindow(2,"blueTooth.html");},showFlag:0},
{name:netConName[21],func:function(){parent.popWindow(2,"networkSetting/proxySetting.html");},showFlag:1}
];
	...
	switch(msg)
	{
		case top.E_NETWORK_CONNECT_TEST_SUCCESS:
		{
			if(focusType=="connectTest")
			{
				...
				top.setTTSSay(top.SPEECH_TYPE_FLUSH,netConTest[1]);
				top.ttsLog("ttsStr="+netConTest[1]);
				setTimeout(function(){
            ...
            {
                if (top.$("main").contentWindow.showNetworkStatus)
                    top.$("main").contentWindow.showNetworkStatus();
                else if (top.$("main").contentWindow.updateNetwork)
                    top.$("main").contentWindow.updateNetwork();
            }
        ...
        case top.E_NETWORK_CONNECT_TEST_FAIL:
        {
			...
			top.setTTSSay(top.SPEECH_TYPE_FLUSH,netConTest[2]);
			top.ttsLog("ttsStr="+netConTest[2]);
            setTimeout(function(){
                $("connectTest").style.display="none";
                parent.showPage(1);
                top.restartTimeout();
...
function toOK()
{
	...
	else if(focusType=="InterfaceSel")
	{
        ...
        {
            setting.setInterfaceType(2);//wireless
        }
        ...
        if (netInterface != presetPositionV)
        {
            ...
            if (presetPositionV == 1)
            {
                ...
                //console.log("zhangljun networkSetting.js toOK() lastAp.ssid:" + lastAp.ssid+" -- lastAp.cipher:"+lastAp.cipher+" -- lastAp.encrypt:"+lastAp.encrypt+"-- lastAp.networkname:"+lastAp.networkname);
...
function init()
{
	...
	initSetRegion();
    console.log("cfg_BluetoothSoundbox_support:" + top.g_setting.getProperty("ro.sita.model.FUNCTION_ENABLE_CFG.cfg_BluetoothSoundbox_support", "0"));
    if (top.g_setting.getProperty("ro.sita.model.FUNCTION_ENABLE_CFG.cfg_BluetoothSoundbox_support", "0")=="1")
    {
       //menuList[11].showFlag = 1;
    }
    var bluetooth = tcl.bluetooth;
    if(bluetooth.hasBtPlugIn() == 1)
    {
        menuList[11].showFlag = 1;
	}
	else
	...
	else
	{
        ...
        spans[12].innerHTML=netConName[21];
		initVarValue();
		showInterConMenu();
		showInterfaceMenu();
		showDlnaMenu();
...
function initVarValue()
{
	 netIntConnection = setting.networkEnable;//后续添加
	  ...
	  netInterfaceval = setting.networkConnectType;
 	  if(netInterfaceval == 1 ||netInterfaceval == 0)//0 ,有线。1 无线
      {
          netInterface = 0;
      }
	  else
...
function showNetInfo()//显示网络的主要信息
{
	...
	var divs=$("smallPopWinDiv").getElementsByTagName("div");
	divs[0].innerHTML=netInterfaceSel[netInterface];
...
function checkShowMenu()
{
	try
	{
        ...
        if(!top.isAUFlag || 26 == top.g_channel.systemCountry)
        {
...
function ttsRead()
{
	...
	if("networkList" == focusType && 1 == parent.currentShowMenu)
	{
		switch(currentName)
		{
			case netConName[0]:
				ttsName = currentName + ",";
				currentStatus = netInterConSel[netIntConnection];
				break;
			case netConName[1]:
				ttsName = currentName + ",";
				currentStatus = netInterfaceSel[netInterface];
				break;
			case netConName[2]:
				ttsName = currentName;
				currentStatus = "";
			...
			case netConName[6]:
				ttsName = currentName + ",";
				currentStatus = netDLNASel[netDLNA];
				break;
			case netConName[7]:
				ttsName = currentName + ",";
				currentStatus = netDLNASel[netGuideOn];
				break;
			case netConName[8]:
				ttsName = currentName;
				currentStatus = "";
	...
	else if("InterConSel" == focusType)  //soundPreset选项菜单
	{
		ttsName = "";
		currentStatus = netInterConSel[presetPositionV];
		isSelected(netInterConSel[netIntConnection] == currentStatus);
	}
	else if("InterfaceSel" == focusType)
	{
		ttsName = "";
		currentStatus = netInterfaceSel[presetPositionV];
		isSelected(netInterfaceSel[netInterface] == currentStatus);
	}
	else if("DLNASel" == focusType)
	{
		ttsName = "";
		currentStatus = netDLNASel[presetPositionV];
		isSelected(netDLNASel[netDLNA] == currentStatus);
	}
	else if("GuideOnSel" == focusType)
	{
		ttsName = "";
		currentStatus = netDLNASel[presetPositionV];
		isSelected(netDLNASel[netGuideOn] == currentStatus);
	}
	else if("password" == focusType)
	{
		ttsName = "";
		currentStatus = "";
	}
	else if("information" == focusType)
	{
		ttsName = getNetInformation();
		currentStatus = "";
	...
	else if("connectTest" == focusType)
	{
		ttsName = "";
		currentStatus = $("conTestContent").innerText;
	}
	lastFocusType = focusType;
	ttsStr = ttsName + currentStatus;
	// ttsStr = ttsStr.replace(/ /g,',');
	if("" != ttsStr)
	{
		top.ttsLog("ttsStr=" + ttsStr);
		//top.ttsLog("focusType="+focusType+"   currentName="+findName(positionV));
		//top.ttsLog("form_position_h="+presetPositionV);
		top.setTTSSay(top.SPEECH_TYPE_FLUSH, ttsStr);
	}
}

//格式化information栏中信息，用于TTS
function getNetInformation()
{
	var inforStr = "";
	inforStr = netConName[1] + "  " + netInterfaceSel[netInterface] + ",";					//interface
	inforStr = inforStr + netConName[11] + "," + netAddressType[netAddressTypeVal] + ",";	//address type
	inforStr = inforStr + netConName[12] + "," + setting.networkIP + ",";					//IP address
	inforStr = inforStr + netConName[13] + "," + setting.networkSubnetMask + ",";			//subnet mask
	inforStr = inforStr + netConName[14] + "," + setting.networkGateway + ",";				//default gateway
	inforStr = inforStr + netConName[15] + "," + setting.networkPrimaryDNS + ",";			//primary DNS
	inforStr = inforStr + netConName[16] + "," + setting.networkPrimaryDNS + ",";			//Secondary DNS
	if(netInterface == 1)
	{
		inforStr = inforStr + netConName[17] + "," + setting.netSSID + ",";					//SSID
	}
	inforStr = inforStr + netConName[18] + "," + setting.networkMAC + ",";					//BSSID
	inforStr = inforStr + netConName[19] + "," + setting.getUserId;							//UserID
	return inforStr;
}