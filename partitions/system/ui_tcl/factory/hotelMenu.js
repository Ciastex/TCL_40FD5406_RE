...
var off_on = ["Off","On"];
var picture_mode = ["Standard","Dynamic","Natural","Movie","Personal","HDR enhance","Stadium","Digital cinema"];
var sound_mode = ["Music","Movie","Voice","Standard","Personal","Subwofofer","Stadium","Game","Sport"];
var power_signal = ["ATV","DTV","AV1","AV2","CMP",/*"VGA",*/"HDMI1","HDMI2","HDMI3","HDMI4"];
...
var config_V2 = "";
var SupportSource = [0,0,0,0,0,0,0,0,0];
var SupportPictureMode = [0,0,0,0,0,0,0,0];
var SupportSoundMode = [0,0,0,0,0,0,0,0,0];
...
function toLeft()
{
	switch(position_h)
	{
		...
		case 7:
			var counter=0;
			while(counter<picture_mode.length)
			{
			pictureMode--;
			if(pictureMode < 0)
			{
				pictureMode = picture_mode.length - 1;
				}
				if(SupportPictureMode[pictureMode]==1)
				{
					break;
				}
				counter++;
			}
			factory.hotelPic = pictureModeTransform(pictureMode, 1);
			span_contain_list[position_h-2+88].innerHTML = picture_mode[pictureMode];
			break;
		case 8:
			var counter=0;
			while(counter<sound_mode.length)
			{
			soundMode--;
			if(soundMode < 0)
			{
				soundMode = sound_mode.length - 1;
				}
				if(SupportSoundMode[soundMode]==1)
				{
					break;
				}
				counter++;
			}
			factory.hotelSound = soundModeTransform(soundMode, 1);
		...
		case 10:
			...
			if(powerSignalIndex < 0)
			{
				...
				if(SupportSource[powerSignalIndex])
				{
					break;
				}
				counter++;
			...
			getPowerChannelList();
			if((powerSignalIndex != 0 && powerSignalIndex != 1) || ((powerSignalIndex == 0 || powerSignalIndex == 1)&& channelList.length<=0))
			{
				div_contain_list[11].style.background = "#FF0";
				div_contain_list[12].style.background = "#CCC";
				span_contain_list[position_h-1+88].innerHTML =1;
			}
			else if(powerSignalIndex == 0 || powerSignalIndex == 1)
			{
...
function toRight()
{
	switch(position_h)
	{
		...
		case 7:
			...
			{
			pictureMode++;
			if(pictureMode > picture_mode.length - 1)
			{
				pictureMode = 0;
				}
				if(SupportPictureMode[pictureMode] == 1)
		...
		case 8:
			...
			{
			soundMode++;
			if(soundMode > sound_mode.length - 1)
			{
				soundMode = 0;
				}
				if(SupportSoundMode[soundMode] == 1)
...
function init()
{
	...
	config_V2 = top.g_setting.getProperty("ro.sita.cfg.ver","");
	
	hotelEnable = factory.hotelEnable;
	powerLogo = factory.hotelPowerLogo;
	maxVol = factory.hotelMaxVolume;
	autoSet = factory.hotelAutoSet;
	pictureMode = pictureModeTransform(factory.hotelPic, 0);
    soundMode = soundModeTransform(factory.hotelSound, 0);
	...
	{	
		var i=0;
		var defaulSourceList = "ATV:ATV,DTV:DTV,AV:AV,YPBPR:YPBPR,HDMI1:HDMI1,HDMI2:HDMI2,HDMI3:HDMI3";
		var SourceList = setting.getProperty("ro.sita.cfg.model.Apps.SOURCE_SUPPORT",defaulSourceList);
		SourceList = SourceList.trim();
		SourceList = SourceList.replace("{","");
		var SourceListSplit = SourceList.trim().split(",");

		if(SourceListSplit.length > 0)
		{
			for(i = 0; i<SourceListSplit.length; i++)
			{
				var SourceItem = SourceListSplit[i].split(":");
				if(SourceItem[0] == "ATV")
				{
					SupportSource[0]= 1;
				}
				else if(SourceItem[0] == "DTV")
				{
					SupportSource[1]= 1;
				}
        else if(SourceItem[0] == "AV")
				{
					SupportSource[2]= 1;
				}
				else if(SourceItem[0] == "AV1")
				{
					SupportSource[2]= 1;
				}
				else if(SourceItem[0] == "AV2")
				{
					SupportSource[3]= 1;
				}
				else if(SourceItem[0] == "YPBPR")
				{
					SupportSource[4]= 1;
				}
				else if(SourceItem[0] == "HDMI1")
				{
					SupportSource[5]= 1;
				}
				else if(SourceItem[0] == "HDMI2")
				{
					SupportSource[6]= 1;
				}
				else if(SourceItem[0] == "HDMI3")
				{
					SupportSource[7]= 1;
				}	
				else if(SourceItem[0] == "HDMI4")
				{
					SupportSource[8]= 1;
				}				
			}
		...
		var defaulPictureList = "STANDARD:Standard,VIVID:Dynamic,MILD:Natural,STUDIO:Movie,USER:User,DYNAMIC:HDR enhance,STADIUM:Stadium,CINEMA:Digital cinema";
		var PictureListSplit = defaulPictureList.trim().split(",");
		for(i=0; i<PictureListSplit.length; i++)
		{
			var PictureItem = PictureListSplit[i].split(":");
			var KeyString = "ro.sita.cfg.hotel.PictureModePreset"+"."+PictureItem[0];
			var mode = setting.getProperty(KeyString,"null");
			...
			{
				SupportPictureMode[i]=1;
				picture_mode[i]=mode;
			}
		}
		
		var defaulSoundList = "MUSIC:Music,MOVIE:Movie,NEWS:News,STANDARD:Standard,USER:Personal,SUBWOFOFER:Subwofofer,STADIUM:Stadium,GAME:Game,SPORT:Sport";
		var SoundListSplit = defaulSoundList.trim().split(",");	
		for(i=0; i<SoundListSplit.length; i++)
		{
			var SoundItem = SoundListSplit[i].split(":");
			var KeyString = "ro.sita.cfg.hotel.SoundModePreset"+"."+SoundItem[0];			
			var mode = setting.getProperty(KeyString,"null");
	...
	else
	{
		...
		SupportPictureMode[0]= 1;
		SupportPictureMode[1]= 1;
		SupportPictureMode[2]= 1;
		SupportPictureMode[3]= 1;
		SupportPictureMode[4]= 1;
		SupportPictureMode[6]= 1;

		SupportSoundMode[0]= 1;
		SupportSoundMode[1]= 1;		
		SupportSoundMode[2]= 1;
		SupportSoundMode[3]= 1;
		SupportSoundMode[4]= 1;
		SupportSoundMode[6]= 1;	
	}
...
function showOther()
{
	...
	if(hotelEnable == 1)
	{
		...
		else if(powerSignal == 1 || powerSignal == 0)
		{
			if(channelList.length<=0)
			{
				span_contain_list[97].innerHTML =1;
...
function getPowerChannelList()
{
	channelList="";
	powerChannelIndex=0;
	if((powerSignalIndex == 0)||(powerSignalIndex == 1))
	{
		if(powerSignalIndex == 0)
			channelList = top.g_channel.getChannelList(2,0,0);//Atv
		else if(powerSignalIndex == 1)
			channelList = top.g_channel.getChannelList(1,0,0);//Dtv
...
function pictureModeTransform(prepictureMode, mode)   //if mode is 0, get data from mw; if mode is 1, set data to mw
{
    var aftpictureMode = prepictureMode;
    //if(config_V2 == "2")
    //{
    //}
    //else
	{
		...
		else
		{
			...
			else if (5 == prepictureMode)
			{
				aftpictureMode = 6;
			}
		}
	}
    return aftpictureMode;
}

function soundModeTransform(preSoundMode, mode)   //if mode is 0, get data from mw; if mode is 1, set data to mw
{
    var aftSoundMode = preSoundMode;
    /*
	if(config_V2 == "2")
	{
	}
