...
top.keySetForUI(1);
var init_title = ["Main Menu\\","Design mode hotkey","1--Factory Menu","2--Other Setting","3--Service Menu","4--Param Setting","5--Hotel Menu","6--Reset All","SW NO:","Project Name:","SIACP Version:","Date:"];

var off_on = ["Off","On"];
var actionWord = ["Do","WAIT","OK","Fail"];
var moth_string = ["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"];

var DESIGN_ITEM_LENGHT = 11;
...
var factoryInfo;
var IDX_HOTEL_MENU = 5;
var IDX_RESET_ALL = 6;
var IDX_LAST = IDX_HOTEL_MENU;
var HIDE_MENU_RESET_ALL = 1;
...
function toRight()
{
    ...
    else if(position_h == IDX_RESET_ALL)
    {
        doResetAll();
    }
	else
...
function toUp()
{
	if(position_h == 0)
	{
		...
		div_contain_list[IDX_LAST + 1].style.background = "#FF0";
		position_h = IDX_LAST;
	}
	else
	{
...
function toDown()
{
	if(position_h == IDX_LAST)
	{
		div_contain_list[IDX_LAST + 1].style.background = "#FFF";
		div_contain_list[1].style.background = "#FF0";
		position_h = 0;
	}
	else
...
function init()
{
	...
	for(var i = 0; i <= DESIGN_ITEM_LENGHT; i++)
	{
		...
		if (((IDX_RESET_ALL + 1) == i) && (HIDE_MENU_RESET_ALL == 1))
		{
			div_contain_list[i].innerHTML = "";
		}
	}
	for(var i = DESIGN_ITEM_LENGHT; i < 12; i++)
	{
		span_contain_list[i].style.display = "none";
		div_contain_list[i + 1].style.display = "none";
	}
	span_contain_list[0].innerHTML = off_on[designHotKey];
	if (HIDE_MENU_RESET_ALL == 0)
	{
    		span_contain_list[IDX_RESET_ALL].innerHTML = actionWord[0];
	}
    if(top.configPackVersion == "")
	{
        span_contain_list[7].innerHTML = factoryInfo.swName;
	}
	else
	{
        span_contain_list[7].innerHTML = factoryInfo.swName+"."+top.configPackVersion;
	}
	span_contain_list[8].innerHTML = factoryInfo.projectName;
	span_contain_list[9].innerHTML = factoryInfo.siacpVer;
	span_contain_list[10].innerHTML = factoryInfo.compileDate;
...
function doResetAll()
{
	if (HIDE_MENU_RESET_ALL == 1)
	{
		return ;
	}
	
    span_contain_list[6].innerHTML = actionWord[1];
    setTimeout(function(){
    	top.sendEvent("vod.netflix", 0xf00c);
        if(factory.nvmResetAll(0)==0)
        {
            span_contain_list[6].innerHTML = actionWord[2];
            span_contain_list[0].innerHTML = off_on[top.g_factory.designModeHotkey];
            factoryHotKey=1;
        ...
        {
            span_contain_list[6].innerHTML = actionWord[3];
        }
    },500)
}