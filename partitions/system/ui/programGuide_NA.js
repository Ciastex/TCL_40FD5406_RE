...
var epgHotKey					= [top.epgHotKey[0], top.epgHotKey[1], top.epgHotKey[3], top.epgHotKey[4]];
...
var SHOW_WIDTH = 1216;//显示program list的元素的宽度
				...
				if(form_flag == 0)
				{
					...
					if(curIndex != channel.currentIndex)
					{
						...
						if(top.isGingaOn && top.GingaObj.isInit == top.GingaStates.True)
						{
							...
							var next_channel = channel.getChannelInfoByIndex(curIndex);
							var next_freq = next_channel.frequency;
							console.log("[zhangljun][programGuide.html] channelChange curr_freq="+curr_freq+" -- next_freq="+next_freq);
							...
							console.log("[zhangljun][programGuide.html] channelChange channel.inputSource="+channel.inputSource);

						}
                        // if(channel.inputSource == 1 && top.GingaObj.isEnable == 1)//DTV and Ginga enable  //EPG中切频道暂停Ginga已移置中间件做
                        // {
                        // 	console.log("[zhangljun][programGuide.html] channelChange to call ginga ap");
...
function notifyProcess(evt)
{
	...
	switch(msg)
	{
		...
		case top.E_CHANNEL_PLAY_END://channel change end
			...
			channelInfos_list[list_position_h] = tchannel;//可以更改锁的状态
		...
		case top.E_DTV_EPG_SCH_FINISH://epg refresh
			...
			if(value == channelInfos_list[i].number)
			{
				...
				if(list_position_h == i)//如果刷新的是当前焦点所在的节目则要注意焦点的刷新
				{
					...
					if(fucous_program[positionOfPiece].style.color == "rgb(176, 176, 176)")
						fucous_program[positionOfPiece].style.color = "#fff";
					fucous_program[positionOfPiece].style.border = "thin "+top.uiColourStyle+" solid";
		...
		case top.E_CHANNEL_LIST_UPDATE:
			//调用init刷新EPG页面先要清除top的页面，避免焦点跑到main页面上
			top.$("otherPage").src = "";
...
function doChannelUpDownRefresh(channelIndex)
{
	...
	for(i=0;i<1;i++)
	{
		...
		piece.setAttribute("class","program_name");//firefox
...
function toLeft()
{
	...
	else if(form_flag == 0)//在主页面上
	{
		if(positionOfPiece > 0)//当前节目焦点不在时间片的最左端
		{
			...
			if(fucous_program[positionOfPiece].style.color == "rgb(255, 255, 255)")
				fucous_program[positionOfPiece].style.color = "#b0b0b0";
			...
			if(fucous_program[positionOfPiece].style.color == "rgb(176, 176, 176)")
				fucous_program[positionOfPiece - 1].style.color = "#fff";
			fucous_program[positionOfPiece - 1].style.border = "thin "+top.uiColourStyle+" solid";
...
function toUp()
{
	if(form_flag == 0)
	{
		...
		playTimer = setTimeout("doEnter()",2000);
	}
}
function toDown()
{
	...
	{		
		if(channelLength > 1)
		{
			var tIndex = channel.getNextIndexByIndex(channelInfos_list[0].index);
			refreshChannelShow(tIndex,1);
			refreshPieceTime(nowDate,distanceDate,startTimePiece);//更新时间片时间
			//setTimeout(function (){getMoreProgramList(distanceDate,1/*direction,右移获取event列表*/,startTimePiece);showProgramToRight()},1);
			getMoreProgramList(distanceDate,1/*direction,右移获取event列表*/,startTimePiece);
...
function doEnter()
{
	if(form_flag == 0)
	{
		if(channelInfos_list[0].index == curIndex)//当前频道
		{
		...
		{
			if(channel.currentIndex!=channelInfos_list[0].index)
			{
				top.g_openMheg5State=0;
				 if(top.currentPanel==1)
					{
				...
				if(top.isGingaOn && top.GingaObj.isInit == top.GingaStates.True)
				{
					var cur_channel = channel.getCurrentChannelInfo();
					var curr_freq = cur_channel.frequency;
					var next_channel = channel.getChannelInfoByIndex(channelInfos_list[list_position_h].index);
					var next_freq = next_channel.frequency;
					console.log("[zhangljun][programGuide.html] channelChange curr_freq="+curr_freq+" -- next_freq="+next_freq);
					if(curr_freq == next_freq)
					{
...
function getMoreProgramList(distance,direction,startTime)
{
	...
	for( i = 0; i < 1; i++)
	{
		...
		/*
		console.log("programInfos_list[" + i + "].length = " + programInfos_list[i].length );
		for(var j = 0; j < programInfos_list[i].length; j++)
		{
			console.log("j = " + j + ",event name = " + programInfos_list[i][j].name);
		}*/
...
function refreshChannelShow(firstChannelIndex,flag)
{
	...
	if(flag == 1)
	{
		...
		var tLists = channel.getEpgNextChannelList(chListType,firstChannelIndex,0/*0 list*/,6);
		len = tLists.length;

		var currentPos = 0;
		for(var i = 0; i < 6 && i < len; i++)
		{			
			if(tLists[i].index == firstChannelIndex)
			{
				currentPos = i;
			}
		}
		...
		{
			for(var i = currentPos; i < 6 && i < len; i++)
			{	
				channelInfos_list[i-currentPos] = tLists[i];
			}
			for(var i = 0; i < currentPos; i++)
			{
				channelInfos_list[len-currentPos+i] = tLists[i];
			}
		}
	...
	else
	{
		...
		var tLists = channel.getEpgPrevChannelList(chListType,firstChannelIndex,0/*0 list*/,6);
		len = tLists.length;
		for(var i = 0; i < 6 && i < len; i++)
		{			
			channelInfos_list[i] = tLists[i];
...
//				channelListDivList[list_position_h].style.backgroundColor = "#500000";
			...
			console.log("channelInfos_list["+i+"] number is "+channelInfos_list[i].number+"name is "+channelInfos_list[i].name+" serviceType is "+channelInfos_list[i].serviceType);
			infoTag[0].innerText = flagImg[channelInfos_list[i].serviceType];
			//var infoTag = channelListDivList[i].getElementsByTagName('span');
			//infoTag[0].innerText = channelInfos_list[i].number;
			infoTag[1].innerText = top.parseChNumber(channelInfos_list[i]);
			infoTag[2].innerText = channelInfos_list[i].name;	
		}
		else
		{
			channelLength = len;
...
{
	//console.log("in showContent isHaveProgram = " + isHaveProgramOnPiece_list[channelPosition] + ",channelPosition = " + channelPosition + ",eventPosition = " + eventPosition);
	...
	// unlockFlag 0:locked 1:lock released
	//console.log("unlockFlag = " + channelInfos_list[channelPosition].unlockFlag+"isLock = " + channelInfos_list[channelPosition].isLock);
	//console.log(" top.parentalLock is "+top.parentalLock+" top.channelLock is "+top.channelLock+" top.inputLock is "+top.inputLock);
	if(channelInfos_list[channelPosition].unlockFlag == 1 && (top.parentalLock == 0 && top.channelLock == 0 && top.inputLock == 0) && country != franceCountryNum/*france 当为法国的时候unlockFlag标志不适用*/)//已经手动解锁
	{
		isLocked = 0;
	}
	else
	{
		if(channelInfos_list[channelPosition].isLock && (top.parentalLock == 1 || top.channelLock == 1 || top.inputLock == 1))//优先级最高
		{
			isLocked = 1;			
		}
	...
	if(isHaveProgramOnPiece_list[channelPosition] == 0)//没有节目
	{
		...
		$("program_content_rating").innerText = "";
		$("program_content_cc").style.display = "none";
	...
	{
		//console.log("firstProgramNumOnPiece_list[channelPosition] = " + firstProgramNumOnPiece_list[channelPosition] + ",eventPosition = " + eventPosition);
		...
		if(top.isNAFlag)
		{
			$("program_content_rating").style.display = "block";
			$("program_content_cc").style.display = "none";
			$("program_content_cc").innerText = "CC";
		}
		else
		{
			$("program_content_rating").style.display = "none";
			$("program_content_cc").style.display = "none";
		}
		
		if(top.isNAFlag)
		{
			var rating = setting.getEpgEventRating(channelInfos_list[channelPosition].index,nowEvent.id);
			console.log(" getEpgEventRating rating is "+rating+" index is "+channelInfos_list[channelPosition].index+" id is "+nowEvent.id);
			if(rating.length > 0)
			{
				$("program_content_rating").innerText = rating;
			}
			else
			{
				$("program_content_rating").innerText = "No Rating";
			}
		}
		//showSubtitle	
		//第一个事件的subtitle状态和channelPlay相同
		console.log("nowEvent.id is "+nowEvent.id+"  channel.getCurrentChannelInfo().getProgramPlayList()[0].id is "+channel.getCurrentChannelInfo().getProgramPlayList()[0].id+" nowEvent.isSttlExist is "+nowEvent.isSttlExist);
		//change by liukl bugID:43441
		if(channelInfos_list[channelPosition].index == curIndex && nowEvent.id == channel.getCurrentChannelInfo().getProgramPlayList()[0].id)//(firstProgramNumOnPiece_list[channelPosition] + eventPosition) == 0)//当前频道的第一个事件
		{
			//console.log("-------------->CCCCCCC="+setting.isCCAvailable());
            if(nowEvent.isSttlExist)
            {
                $("program_content_cc").style.display = "none";
				$("program_content_cc").innerText = "CC";
            }
            ...
            {
				$("program_content_cc").style.display = "none";
				//$("program_content_cc").innerText = "No CC";
				$("program_content_cc").innerText = "";
            }
		...
		else
		{
			...
			else//没有锁或有锁但是已经解锁
			{
				...
				console.log("leng is "+leng+" detail is "+detail);
				...
				if(leng > 0)
				{
					...
					for(var i = 0; i < detail.length; i++){
						if(i % 75 == 0){
							var temp = detail.substr(i, 75) + " ";
							i == 0 ? newdetail = temp : newdetail += temp;
						}
					}
					$('program_content').innerText = newdetail;
				}
				else
				{
...
function doDetialAnimation()
{
	...
	//console.log("in doDetialAnimation detailHeight = " + detailHeight);
	if(detailHeight <= 210)
	{
		return;
	}
...
{
	//console.log("in refreshAnimation ");
	$("program_content").style.webkitTransition = "none";
	$("program_content").style.top = "0px";
...
*/
var eventTriangleWidth = 20;//用来表示当前时间片没有将事件显示完整的三角图标所占用的宽度
function sliceTimeLeftToRight(position,distance)
{
	...
	var widthPerMinute = 1216/(2*60); //算法：SHOW_WIDTH/(2*60);//每分钟所占的像素宽度
	console.log("zenglm =======>widthPerMinute is "+widthPerMinute);
	isHaveProgramOnPiece_list[position] = 0;//时间上是否有节目 1 是，0 否
	while(rightPosition < SHOW_WIDTH)
	{//console.log("length = " + length + " firstProgramNumOnPiece_list[position] + i = " + (firstProgramNumOnPiece_list[position] + i))
		...
		else
		{
			...
			else //endHour > reallyStartTimePiece
			{
				if(startHour < reallyStartTimePiece)
				{
					...
					{
						piece.innerText = programInfos_list[position][firstProgramNumOnPiece_list[position] + i].startTime+"\n"+titleName;
					}
					else
					{
						piece.innerText = programInfos_list[position][firstProgramNumOnPiece_list[position] + i].startTime+"\n"+program_guide[4];
					...
					if(endHour < (reallyStartTimePiece + 2*60))//节目没有占据整个时间片
					{
						...
						if(pieceBarWidth > eventTriangleWidth)
						{
							...
							eImg.setAttribute("src","images/EPG/Event-Default-LA.png");
					...
					{						
						...
						eImg.setAttribute("src","images/EPG/Event-Default-RA.png");
						programListDivList[position].appendChild(eImg);
						break;
					}
...
function showProgramToLeft()
{
	...
	console.log("zenlmg ==========>positionOfPiece  "+positionOfPiece);
	fucous_program[positionOfPiece].style.backgroundColor = "#000";
	if(fucous_program[positionOfPiece].style.color == "rgb(176, 176, 176)")
		fucous_program[positionOfPiece].style.color = "#fff";
	fucous_program[positionOfPiece].style.border = "thin "+top.uiColourStyle+" solid";
	fucous_program[positionOfPiece].style.zIndex = 100;
	if(top.isNAFlag && fucous_program.length < programInfos_list[list_position_h].length)//in case LA that get data not in current time piece.
	{
		var adjPositionOfPiece = positionOfPiece + (programInfos_list[list_position_h].length - fucous_program.length)
		showContent(list_position_h,adjPositionOfPiece); 
	}
	else
	{
	showContent(list_position_h,positionOfPiece);
...
function sliceTimeLeftToLeft(position,distance)
{
	...
	{//console.log("in sliceTimeLeftToLeft  length = " + length + " firstProgramNumOnPiece_list[position] = " + firstProgramNumOnPiece_list[position] + " i = " + i)
		...
		{
			//console.log("in sliceTimeLeftToLeft 0 firstProgramNumOnPiece_list[position] = " + firstProgramNumOnPiece_list[position]);
		...
		else
		{
			...
			//console.log("startHour = " + startHour + " endHour = " + endHour + " reallyStartTimePiece = " + reallyStartTimePiece);
			...
			else //startHour < (reallyStartTimePiece + 2*60)
			{
				if(endHour > (reallyStartTimePiece + 2*60))//当前节目是该时间片上最后一个节目
				{
					...
					else //节目占据整个时间片，是时间片上的最后一个也是第一个节目
					{
						...
						maxCount += 1;
						firstProgramNumOnPiece_list[position] = firstProgramNumOnPiece_list[position] - i;
						//console.log("in sliceTimeLeftToLeft 1 firstProgramNumOnPiece_list[position] = " + firstProgramNumOnPiece_list[position]);
				...
				else
				{
					...
					{//console.log("in sliceTimeLeftToLeft a");
						//新添加一个时间片
						var piece = document.createElement('span');
						piece.setAttribute("class","program_name");//firefox
						var titleName = programInfos_list[position][firstProgramNumOnPiece_list[position] - i].name;
						...
						leftPosition =  parseInt(widthPerMinute*(startHour - reallyStartTimePiece),10)
						console.log("zenglm======> startHour "+startHour+" reallyStartTimePiece "+reallyStartTimePiece+"  leftPosition "+leftPosition);
						piece.style.left = leftPosition + "px";
...
//add chenhua 2014.11.4 修正当此时leftPosition == 0时，循环停止但是firstProgramNumOnPiece_list[position]却没有赋予正确的值。
						...
						rightPosition = parseInt(widthPerMinute*(endHour - reallyStartTimePiece),10);
						console.log("zenglm======> endHour "+endHour+" reallyStartTimePiece "+reallyStartTimePiece+"  rightPosition "+rightPosition);
						if(isHaveProgramOnPiece_list[position] == 0)
						{//console.log("in sliceTimeLeftToLeft a.a");
							if(rightPosition <= (leftPosition + 5))
							{
								pieceBarWidth =   2;
							}
						...
						{//console.log("in sliceTimeLeftToLeft a.b");
							pieceBarWidth = rightPosition -  leftPosition - 5;
						...
						maxCount += 1;
						i++;
						//console.log("in sliceTimeLeftToLeft a.b end leftPosition = " + leftPosition);
					}
					else//节目开始时间比时间片开始时间小
					{//console.log("in sliceTimeLeftToLeft b");
						if(endHour > reallyStartTimePiece)//节目部分落在时间片上,该节目时是时间片上的第一个节目
						{//console.log("in sliceTimeLeftToLeft b.a");
							//新添加一个时间片
							var piece = document.createElement('span');
							piece.setAttribute("class","program_name");//firefox
							var titleName = programInfos_list[position][firstProgramNumOnPiece_list[position] - i].name;
							...
							maxCount += 1;
							firstProgramNumOnPiece_list[position] = firstProgramNumOnPiece_list[position] - i;
							//console.log("in sliceTimeLeftToLeft 2 firstProgramNumOnPiece_list[position] = " + firstProgramNumOnPiece_list[position]);
						...
						{//console.log("in sliceTimeLeftToLeft b.b");
							firstProgramNumOnPiece_list[position] = firstProgramNumOnPiece_list[position] - i + 1;
							//console.log("in sliceTimeLeftToLeft 3 firstProgramNumOnPiece_list[position] = " + firstProgramNumOnPiece_list[position]);
							if(isHaveProgramOnPiece_list[position] == 0)
							{
								//新添加一个时间片
								var piece = document.createElement('span');
...
function init()
{
	...
	top.RemoteConntrolType = "EPG";//lqt- 设置虚拟遥控器的场景类型
	setting.sendMsgToDBC(1);
	//console.log("in programGuide init top.g_temp is"+top.g_temp);
	//在launcher下面进入全局菜单中的EPG需要取消静音 add liukl
	...
	refreshTime();
	$('time_bar1').style.display = "block";
	$('time_bar2').style.display = "block";
	channelStatus = channel.channelProgramStatus();
	if(channelStatus!=0 && listType == 5)//favorite channel list
	{
		var num = channel.getEpgNextChannelList(listType,tChannel.isSkip?channel.nextIndex:curIndex = tChannel.index,0/*0 list*/,6).length;
		if(num == 0)
		{
			channelStatus = 0;
		}
	}
	//存在dtv节目但所有节目都被skip掉. 此刻说明无节目
	var allDigital = channel.getEpgNextChannelList(1/*digital*/,tChannel.index,0/*0 list*/,6).length;
	console.log("in init allDigital is "+allDigital);
	if(allDigital== 0)
	{
		channelStatus = 0;
	}
	if(channelStatus)//有频道
	{
		...
		for(var i = 0; i < hotkeySpanList.length; i++)
		{
			hotkeySpanList[i].innerText = epgHotKey[i];
			if( i == hotkeySpanList.length - 1)//最后一个
			{	
				hotkeySpanList[i].style.width = "225px";		
				top.doEleScrForNoOption(hotkeySpanList,i,0);
			}
		}	
		...
		//setTimeout(function(){
			...
			var fucous_program = programListDivList[list_position_h].getElementsByTagName('span');
			fucous_program[positionOfPiece].style.backgroundColor = "transparent";
			console.log("zenglm color is "+fucous_program[0].style.color);
			if(fucous_program[positionOfPiece].style.color == "rgb(255, 255, 255)")
				fucous_program[positionOfPiece].style.color = "#b0b0b0";
			fucous_program[positionOfPiece].style.border = "none";
			fucous_program[positionOfPiece].style.zIndex = 0;
...
function uninit()
{
	...
	console.log("currentChannelListType"+currentChannelListType);
	//top.restartSubtitle();
