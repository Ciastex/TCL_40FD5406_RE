...
function uninit()
{
    ...
    if (regionCount >=2  && top.g_channel.inputSource == 1 && (country == 98 || country == 134))
	{
	...
	if(focusOn == "autoScan")
	{
		...
		{/*
			...
			else if(scanMode == 1)//advanced
			{
				if(list_position_h == 0)
				{
					if(channel.getResetQuickStatus(country)||(country == 7 ))//判断是否有快速搜台选项,有快速搜台,当前只有德国)
					{
						if(scanMode <= 0)
						{
							scanMode = 2;//quick
			...
			else if(scanMode == 0  || scanMode == 2 ||  scanMode == 5 )
			{
				if(channel.getResetQuickStatus(country)||(country == 7 ))//判断是否有快速搜台选项,有快速搜台,当前只有德国)
				{
					if(scanMode <= 0)
				...
				else if(country == 6 ) //Denmark have Yousee
				{
					if(scanMode == 0)
					{
						scanMode = 5;//Yousee
	...
	else if(focusOn == "autoScan")
	{
		...
		/*	if(country == 7)//finland only have quick search，when get into auto search , it's run at once ,so isSearch == 1
			...
			*/
			...
			else if(scanMode == 1||scanMode == 5)//advanced && Yousee
			{
				if(list_position_h == 5)
				{
					var f = freqWords[5] + freqWords[4]*10 + freqWords[3]*100 + freqWords[2]*1000 + freqWords[1]*10000 + freqWords[0]*100000;
					...
					else
					{
						...
						$("scanStatus").innerText = scanStatusWords[0];
						$("search").innerText = searchStop[1];
						channel.startScanDtvAuto(1,frequency,-1,modulation,symbolRate,networkId); //Add Yousee like Advanced
					}
				}
			}
			else if(scanMode == 2 || scanMode == 0)//full or quick
		...
		if(chType == 0 || chType == 1)//digital + analogue或digital
		{
			...
			else
			{
				...
				$("search").innerText = searchStop[0];
					...
					else if (scanModeValue == 5 )//Yousee
					{
						initScanParameter(scanModeValue);
						scanHeight = SCAN_FORM_HEIGHT_START + 5*SCAN_FORM_AND_FOCUS_STEP +30;

						scanOptionsLiList[0].style.display = "block";
						$("autoModeContent").innerText = "Yousee";
						scanOptionsLiList[1].style.display = "block";
						scanOptionsLiList[2].style.display = "block";
						scanOptionsLiList[3].style.display = "block";
						scanOptionsLiList[4].style.display = "block";
...
function initChType()
{
	...
	{
		top.g_nextHtmlPage = "";
		console.log("--hzm initChType top.cmd_vrc_auto_search_type = " + top.cmd_vrc_auto_search_type);
		if (top.cmd_vrc_auto_search_type == 0)
		{
            list_position_h = 1;
		}
		else if (top.cmd_vrc_auto_search_type == 1)
		{
            top.cmd_vrc_auto_search_type = 0;
            list_position_h = 2;
		}
		doEnter();
	}
	...
	for(var i = 0; i < 5; i++)
	{
		...
		scanOptionsLiList[i].onmousedown = function(evt){
			...
			{//left key
				...
				{
					//取消先前焦点
					if(tunerMode == 0/*cable*/ && (channelType == 0 || channelType == 1))
					{
						if(scanMode == 4/*ziggo*/ || scanMode == 3/*UPC*/)
						{
