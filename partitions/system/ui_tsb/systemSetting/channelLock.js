...
function doEnter()
{
	if(programStatus)
	{
		...
		//操作频道锁界面后，原本的lastindex会改变，导致按返回键无法返回上一个频道。以下接口重新设置正确的上一个节目序号
	...
	if(programStatus)
	{
		...
		for (var i = 0; i < ONE_PAGE_CHANNEL_SUM; i++)
		{
			if (i < listSum)
			{
				...
				fourthContentSpanList[i].innerHTML = (top.isLAFlag || top.isNAFlag) ? top.parseChNumber(channelList[i]) : channelList[i].number;
...
function showList(page,cursor)
{
	if(page >= 0 && page < (pageSum - 1))
	{
		for(var i = 0; i < ONE_PAGE_CHANNEL_SUM; i++)
		{
			...
			fourthContentSpanList[i].innerHTML = (top.isLAFlag || top.isNAFlag) 
				? top.parseChNumber(channelList[page*ONE_PAGE_CHANNEL_SUM + i]) 
				: channelList[page*ONE_PAGE_CHANNEL_SUM + i].number;
...
function ttsRead()
{
	...
	top.ttsLog("programStatus=" + programStatus + "   parent.currentShowMenu=" + parent.currentShowMenu);
	if(3 == parent.currentShowMenu)
	{
		if(programStatus)
		{
			top.ttsLog("top.channelLock=" + top.channelLock);
			top.ttsLog("fourthContentImgList[list_position_h].src=" + fourthContentImgList[list_position_h].src);
			top.ttsLog("channelList[list_position_h].isLock=" + channelList[list_position_h].isLock);
			ttsName = fourthContentSpanList[list_position_h].innerHTML + "," + fourthContentLabelList[list_position_h].innerHTML + "," + fourthContentDivList[list_position_h].innerHTML + ",";
			currentStatus = "";
			isSelected(1 == channelList[list_position_h].isLock);
		}
		else
		{
			ttsName = noPrgram;
