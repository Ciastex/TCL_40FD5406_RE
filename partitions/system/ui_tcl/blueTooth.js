...
var curBlueToothDevice;
var curPairBtDevice;
var curChgStateBtDevice;
var blueToothDeviceList = []; //蓝牙列表对象
var newBtDeviceList = []; //蓝牙列表对象
var pairBtDeviceList = []; //蓝牙列表对象
var deviceListContentLi; //html文件中显示蓝牙设备信息的li标签
var currentBluetoothDevice;//表示当前设备，包括断开，连接的当前设备等。
var connectedBtAudioDevice;
...
var deviceListTipsArray = ["",top.bt_deviceList_State_Connecting,top.bt_deviceList_State_Connected,top.bt_deviceList_State_NoConnect,top.bt_deviceList_State_Paired];//["","Connecting...","Connected","Not connected"];
...
var AUDIO_DEVICE_TYPE = 2;//1;//音箱
var EARPHONE_DEVICE_TYPE = 1;//2;//耳机

var HID_UNKOWN_DEVICE_TYPE = 100;
var KEYBOARD_DEVICE_TYPE = 3;//101;//键盘
var MOUSE_DEVICE_TYPE = 4;//102;//鼠标
...
var progressTimer;
var connectTimer;
var isConnectTimerOn = 0;



function emptyBlueToothDeviceList()
{
	while(blueToothDeviceList.length != 0)
	{
        blueToothDeviceList.pop();
	}
}

function findCurrentBtDevice()
{
    var btdev_name = deviceListContentLi[settingFocusV].getElementsByTagName("span")[0].innerHTML;
    for(var i=0;i< blueToothDeviceList.length;i++)
    {
        if(btdev_name == blueToothDeviceList[i].name)
        {
            return blueToothDeviceList[i];
        }
    }
}
...
function startConnectTimer()
{
    connectTimer = setTimeout(function(){
        console.log("zk bt test -------------------------- connectTimer timeout -----------------------------");
        ...
        /*deviceListContentLi[settingFocusV].getElementsByTagName("span")[1].innerHTML = deviceListTipsArray[3];//Not connected
        deviceListContentLi[settingFocusV].getElementsByTagName("img")[1].style.display = "block";
        deviceListContentLi[settingFocusV].getElementsByTagName("img")[1].src = "bluetooth-images/Tip_40.png";
        deviceListContentLi[settingFocusV].getElementsByTagName("img")[2].style.display = "block";*/
        refreshAvaDeviceList();
    }, 20000);
}

function refreshAvaDeviceList()
{
    console.log("-------------------------- refreshAvaDeviceList -----------------------------");
...
function setOutPutByValue(value)
{
	switch(value)
	{
		case 0:
			...
			setting.bluetoothStatus = btDeviceStatus;
...
function initText()
{
	...
	deviceNameSpan = $("deviceList").getElementsByClassName("deviceName");
}
...
function notifyProcess(evt)  //蓝牙消息处理函数
{
	/**
	***消息种类
	var UI_BLUETOOTH_EVENT_UNKNOWN                   =  0x8110;   //33040
	var UI_BLUETOOTH_EVENT_CONNECT_SUCCESS           =  0x8111;
	var UI_BLUETOOTH_EVENT_CONNECT_FAIL              =  0x8112;
	var UI_BLUETOOTH_EVENT_DISCOVER_START            =  0x8113;
	var UI_BLUETOOTH_EVENT_DISCOVER_END              =  0x8114;  //33044
	var UI_BLUETOOTH_EVENT_DEV_LIST_STATE_CHANGE     =  0x8115;  //33045
	var UI_BLUETOOTH_EVENT_ERROR                     =  0x8116;
	var UI_BLUETOOTH_EVENT_DEVICES_CHANGE            =  0x8117;
	var UI_BLUETOOTH_EVENT_PIN_CODE_PAIR             =  0x8118;
	var UI_BLUETOOTH_EVENT_HH_BACK_CONNECT           =  0x8119;
	var UI_BLUETOOTH_EVENT_HH_REMOVE_SUCCESS              =  0x811a;
	var UI_BLUETOOTH_EVENT_HH_REMOVE_FAIL                 =  0x811b;
	**/
	var msg = evt.which;
	...
	switch(msg)
	{
        ...
        case top.E_NOTIFY_BTSPEAKER_CONNECT:
            {
                console.log("zk test ------------------------------------------------------- Con MSG:"+msg);
                var devCurNum = setting.getProperty("app.ui.audioDeviceStatus","000");//bit 2:ARC ;bit 1:bluetooth, bit 0:headphone
                ...
                if(top.mute_status == 0)//判断当前是否为静音状态
                {
                    if(top.bt_soundOutPutValue == 1)
                    {
                        ...
                        setting.bluetoothStatus = 1;
                    }
                    else
                    {
                        //bluetooth.setBluetoothMute(1);//mute
                        setting.bluetoothStatus = 0;
                    }
                }
                else
                {
        ...
        case top.E_NOTIFY_BTSPEAKER_DISCONNECT:
            console.log("zk test ------------------------------------------------------- Dis MSG:"+msg);
            {
                var devCurNum = setting.getProperty("app.ui.audioDeviceStatus","000");
            ...
            break;
        case top.UI_BLUETOOTH_EVENT_BOND_OK:
            console.log("zk test --------------------------------------- bond ok ----------------");
            break;
        case top.UI_BLUETOOTH_EVENT_BOND_ERROR:
            console.log("zk test --------------------------------------- bond error ----------------");
            if(isConnectTimerOn == 1)
            {
                isConnectTimerOn = 0;
                clearConnectTimer();
            }
            ...
            refreshAvaDeviceList();
            break;
		case top.UI_BLUETOOTH_EVENT_CONNECT_SUCCESS:    //MSG_BLUEZ_CONNECT_SUCCESS
			console.log("zk test --------------------------------------- connect success----------------devName:"+currentBluetoothDevice.name);
			if(isConnectTimerOn == 1)
			{
				isConnectTimerOn = 0;
				clearConnectTimer();
			}
			/*currentBluetoothDevice.status = 1;
			currentBluetoothDevice.isConnected = 1;*/
			var blueName = currentBluetoothDevice.name;
			var deviceType = currentBluetoothDevice.deviceType;
			//setAudioMute(MUTE);  //声音mute
			//判断连接设备的deviceType
			if(deviceType == AUDIO_DEVICE_TYPE || deviceType == EARPHONE_DEVICE_TYPE)
				return;

			if(deviceType == KEYBOARD_DEVICE_TYPE)
			{
				if(focusTab == TIPS_PAIR_AREA)
				{
					showTipsWindow(0,-1);
			...
			//{
				...
				//deviceListContentLi[settingFocusV].getElementsByTagName("span")[1].innerHTML = deviceListTipsArray[2];//connected
				...
				/*if(deviceType == AUDIO_DEVICE_TYPE || deviceType == EARPHONE_DEVICE_TYPE || deviceType == UNKOWN_DEVICE_TYPE)
				{
					var devCurNum = setting.getProperty("app.ui.audioDeviceStatus","000");//bit 2:ARC ;bit 1:bluetooth, bit 0:headphone
					var devNewNum = devCurNum.charAt(0)+"1"+devCurNum.charAt(2);
					setting.setProperty("app.ui.audioDeviceStatus",devNewNum);
		...
		case top.UI_BLUETOOTH_EVENT_ERROR://配对失败
			...
			break;
		case top.UI_BLUETOOTH_EVENT_CONNECT_FAIL:      //MSG_BLUEZ_CONNECT_FAIL
			console.log("zk test --------------------------------------- connect fail----------------");
			if(isConnectTimerOn == 1)
			{
				isConnectTimerOn = 0;
				clearConnectTimer();
        ...
        case top.UI_BLUETOOTH_EVENT_DISCOVER_NEW:       //0x8400+201://MSG_BLUEZ_DISCOVER_NEW
        case top.UI_BLUETOOTH_EVENT_GET_ALL_AVA_DEV:    //0x8400+214://MSG_BLUEZ_GET_ALL_AVAILABLE_DEV
        case top.UI_BLUETOOTH_EVENT_GET_PAIRED_DEV:     //0x8400+215://MSG_BLUEZ_GET_PAIRED_DEV
        {
            console.log("zk test --------------------------------------- discover new----------------");
            if(!isBlueOpen)//防止扫描的过程中突然关闭bluetooth，导致扫描完成的消息发上来引发错误
            {
                return;
            }
            curBlueToothDevice = getDevicesList(0);
            var newBtDev = objBtDevice(
                curBlueToothDevice[0].addr,
                curBlueToothDevice[0].status,
                curBlueToothDevice[0].name,
                curBlueToothDevice[0].deviceType,
                curBlueToothDevice[0].isConnected,
                curBlueToothDevice[0].pinCode,
                curBlueToothDevice[0].avaDevCount,
                curBlueToothDevice[0].pairedDevCount
            );
            if(newBtDev.addr == null || newBtDev.addr == "")
            {
            	return;
            }
            ...
            for(var i = 0; i < blueToothDeviceList.length; i++)
            {
            	if(newBtDev.addr == blueToothDeviceList[i].addr)
            	{
            		return;
            	}
            }
            /*while(newBtDeviceList.length != 0)
            {
                newBtDeviceList.pop();
            }
            newBtDeviceList.push(newBtDev);

            blueToothDeviceLength = blueToothDeviceList.length;
            for(var i = 0; i< blueToothDeviceLength; i++)
            {
                var tmpBtDev = objBtDevice(
                    blueToothDeviceList[i].addr,
                    blueToothDeviceList[i].status,
                    blueToothDeviceList[i].name,
                    blueToothDeviceList[i].deviceType,
                    blueToothDeviceList[i].isConnected,
                    blueToothDeviceList[i].pinCode,
                    blueToothDeviceList[i].avaDevCount,
                    blueToothDeviceList[i].pairedDevCount
                );
                newBtDeviceList.push(tmpBtDev);
            }*/
            if(newBtDev.isConnected == 1 || newBtDev.status == 1)
            {
            	blueToothDeviceList.unshift(newBtDev);
            }
            else {
                blueToothDeviceList.push(newBtDev);
            }

            if(newBtDev.deviceType == AUDIO_DEVICE_TYPE || newBtDev.deviceType == EARPHONE_DEVICE_TYPE)//记录当前连接的音箱类设备
            {
                if(newBtDev.status == 1 && newBtDev.isConnected == 1)
                {
                    currentBluetoothDevice = newBtDev;
                    console.log("   autoConDevName:"+currentBluetoothDevice.name);
                }
            }
            refreshDeviceList();
            break;
		...
		case top.UI_BLUETOOTH_EVENT_DISCOVER_END:  //扫描完成//停止扫描AP未发消息   //MSG_BLUEZ_DISCOVER_END
            console.log("zk test --------------------------------------- discover end----------------");
			if(!isBlueOpen)//防止扫描的过程中突然关闭bluetooth，导致扫描完成的消息发上来引发错误
			{
				return;
			}
        ...
        case top.UI_BLUETOOTH_EVENT_DEV_LIST_STATE_CHANGE:  //MSG_BLUEZ_CONNTECT_CHANGE_STATE
            console.log("zk test --------------------------------------- change state ----------------");
            if(!isBlueOpen)//防止扫描的过程中突然关闭bluetooth，导致扫描完成的消息发上来引发错误
            {
                return;
            }
		...
		case top.UI_BLUETOOTH_EVENT_DEVICES_CHANGE: //断开连接才会有此消息//MSG_BLUEZ_DISCONNECT_SUCCESS
			...
			//if(blueToothDeviceList[startDeviceNum + settingFocusV].deviceType == AUDIO_DEVICE_TYPE)
				...
				//}
			//testLog("connectDevicePosition:" + connectDevicePosition);
			console.log("zk bt test -------------------------- disconnect success -----------------------");
			//currentBluetoothDevice = findCurrentBtDevice();
			/*currentBluetoothDevice.isConnected = 0;*/
			/*deviceListContentLi[settingFocusV].getElementsByTagName("span")[1].innerHTML = deviceListTipsArray[3];//Not connected
			deviceListContentLi[settingFocusV].getElementsByTagName("img")[1].style.display = "block";
			deviceListContentLi[settingFocusV].getElementsByTagName("img")[1].src = "bluetooth-images/Tip_40.png";
			deviceListContentLi[settingFocusV].getElementsByTagName("img")[2].style.display = "block";*/
			...
			/*if(currentBluetoothDevice.deviceType == AUDIO_DEVICE_TYPE || currentBluetoothDevice.deviceType == EARPHONE_DEVICE_TYPE || currentBluetoothDevice.deviceType == UNKOWN_DEVICE_TYPE)
			{
				var devCurNum = setting.getProperty("app.ui.audioDeviceStatus","000");
				var devNewNum = devCurNum.charAt(0)+"0"+devCurNum.charAt(2);
				setting.setProperty("app.ui.audioDeviceStatus",devNewNum);
		...
		case top.UI_BLUETOOTH_EVENT_PIN_CODE_PAIR:  //配对提示  //MSG_BLUEZ_PIN_CODE_REPLY
            console.log("zk bt test -------------------------- pin code pair -----------------------");
            if(!isBlueOpen)//防止扫描的过程中突然关闭bluetooth，导致扫描完成的消息发上来引发错误
            {
                return;
            }
            ...
            curPairBtDevice = getDevicesList(1);
            var tmpBtDev = objBtDevice(
                curPairBtDevice[0].addr,
                curPairBtDevice[0].status,
                curPairBtDevice[0].name,
                curPairBtDevice[0].deviceType,
                curPairBtDevice[0].isConnected,
                curPairBtDevice[0].pinCode,
                curPairBtDevice[0].avaDevCount,
                curPairBtDevice[0].pairedDevCount
            );

			var pinCode = curPairBtDevice[0].pinCode;
			console.log("zk bt test -------------------------- pincode :"+pinCode);
			...
			showTipsWindow(3,pinCode);
			break;
		case top.UI_BLUETOOTH_EVENT_HH_BACK_CONNECT:
			break;
		case top.UI_BLUETOOTH_EVENT_HH_REMOVE_SUCCESS:  //MSG_BLUEZ_REMOVE_DEV_SUCCESS
			console.log("zk bt test -------------------------- remove success -----------------------");
			/*currentBluetoothDevice.status = 0;
			currentBluetoothDevice.isConnected = 0;*/
			/*
...
function toRight()
{
	...
	else if(focusTab == DEVICE_LIST_AREA)//焦点在列表中
	{
		...
		currentBluetoothDevice = findCurrentBtDevice();
		currentBluetoothDevice.avaDevCount = settingFocusV;
		currentBluetoothDevice.pairedDevCount = startDeviceNum;
		//currentBluetoothDevice = bluetooth.getBluetoothDeviceById(deviceId);
		//testLog("currentBluetoothDevice.status:" + currentBluetoothDevice[i].status );
		if(currentBluetoothDevice.status == 1 && currentBluetoothDevice.isConnected == 0)//先断开再移除配对
		{
			$("settingWindow").style.display = "none";
			showTipsWindow(5,-1);
		}
...
function toOk()
{
	...
	else if(focusTab == DEVICE_LIST_AREA)
	{
		...
		//currentBluetoothDevice.avaDevCount = settingFocusV;
		//currentBluetoothDevice.pairedDevCount = startDeviceNum;
		console.log("zk bt test -------------------------- OK OK OK -----------------------------devName:"+currentBluetoothDevice.name);
		blueName = currentBluetoothDevice.name;
		blueAddr = currentBluetoothDevice.addr;
		isConnected = currentBluetoothDevice.isConnected;
		deviceType = currentBluetoothDevice.deviceType;
		isPaired = currentBluetoothDevice.status;

		//isAutoConnect = false;
		currentDeviceStatus = isConnected;  //获取当前选中设备的连接状态
		testLog("YM currentDeviceStatus:" + currentDeviceStatus);
		if(currentDeviceStatus == 0 || isPaired == 0)
		{
			isConnectTimerOn = 1;
            startConnectTimer();
			/*
...
/*
			for(var i=0;i<blueToothDeviceList.length;i++)
			{
				if(blueToothDeviceList[i].deviceType == 1 || blueToothDeviceList[i].deviceType == 2)
				{
					if(blueToothDeviceList[i].status == 1 && blueToothDeviceList[i].isConnected == 1)
					{
						connectedBtAudioDevice = blueToothDeviceList[i];
						isAnyDeviceConnect = true;
					}
				}
			}
*/
			...
			//{
				...
				if(isPaired == 0)//未配对
				{
                    //myPause(3000);//for timeout test
					pairFlag = pair(blueAddr);
                    testLog("YM pairFlag:" + pairFlag);
                    if(pairFlag == 1)
                    {
                        connectFlag = connect(blueAddr);
                        testLog("YM conncetFlag:" + connectFlag);
                        deviceListContentLi[settingFocusV].getElementsByTagName("img")[1].style.display = "none";
                        deviceListContentLi[settingFocusV].getElementsByTagName("span")[1].innerHTML = deviceListTipsArray[1];//连接中...
                    ...
                    {
                        console.log("zk test ----------------------------------- pair fail -------------------");
                        deviceListContentLi[settingFocusV].getElementsByTagName("span")[1].innerHTML = deviceListTipsArray[3];//连接失败
                        deviceListContentLi[settingFocusV].getElementsByTagName("img")[1].style.display = "block";
                        deviceListContentLi[settingFocusV].getElementsByTagName("img")[1].src = "bluetooth-images/Tip_40.png";
                        deviceListContentLi[settingFocusV].getElementsByTagName("img")[2].style.display = "block";
                    }
				...
				else//已配对
				{
                    ...
                    if(connectFlag == 1)
                    {
                        ...
                        console.log("zk test ----------------------------------- connect timeout -------------------");
                        deviceListContentLi[settingFocusV].getElementsByTagName("span")[1].innerHTML = deviceListTipsArray[3];//连接失败
                        deviceListContentLi[settingFocusV].getElementsByTagName("img")[1].style.display = "block";
                        deviceListContentLi[settingFocusV].getElementsByTagName("img")[1].src = "bluetooth-images/Tip_40.png";
                        deviceListContentLi[settingFocusV].getElementsByTagName("img")[2].style.display = "block";
	...
	else if(focusTab == TIPS_FAILED_AREA)//连接失败提示框
	{
		...
		showTipsWindow(0,-1);
		focusTab = DEVICE_LIST_AREA;//preFocusTab;
		if(focusTab == DEVICE_LIST_AREA)
		{
			startElementScroll(deviceNameSpan[settingFocusV]);
		}
	...
	else if(focusTab == TIPS_DISCONNECT_AREA)//断开连接提示框
	{
		...
		for(var i=0;i< blueToothDeviceList.length;i++)
		{
			...
			{
				blueAddr = blueToothDeviceList[i].addr;
				console.log("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~selected blueAddr:"+blueAddr);
				break;
			}
		}
		if(tipsFocusH == 1)
	...
	else if(focusTab == TIPS_PAIR_REMOVE_AREA)//移除配对提示框
	{
		...
		{
			//移除配对
			removeBluetoothFromPairList(blueAddr);
			showTipsWindow(0,-1);//add yums 2015-3-12
			$("settingWindow").style.display = "block";//add yums 2015-3-12
			focusTab = DEVICE_LIST_AREA;//add yums 2015-3-12
		}
	...
	if(area == SETTING_AREA)
	{
		...
		*/
			...
			else if(focusV == 2)  //focus on refresh
			{
				...
				if(1)//(soundOutputShowFlag == false)
				{
					settingFocusImg.style.top = 180 + "px";
				}
				else
...
function objBtDevice(addr,status,name,deviceType,isConnected,pinCode,avaDevCount,pairedDevCount)
{
    var tmpDev = new Object;
    tmpDev.addr = addr;
    tmpDev.status = status;
    tmpDev.name = name;
    tmpDev.deviceType = deviceType;
    tmpDev.isConnected = isConnected;
    tmpDev.pinCode = pinCode;
    tmpDev.avaDevCount = avaDevCount;
    tmpDev.pairedDevCount = pairedDevCount;
    return tmpDev;
}

function myPause(numberMillis)
{
    var now = new Date();
...
function refreshDeviceList()
{
	...
	var listnote;
	var newlen = 0;
	newlen = newBtDeviceList.length;

	for(var i = 0; i < newlen; i++)
	{
		listnote = newBtDeviceList[i];
		if(listnote.isConnected == 1 || listnote.status == 1)
		{
			blueToothDeviceList.push(listnote);
		}
	}
	for(var i = 0; i < newlen; i++)
	{
	...
	else
	{
		...
		for(var i = 0;i < length;i++)
		{
			...
			if(blueToothDeviceList[startDeviceNum + i].isConnected == 1 && blueToothDeviceList[startDeviceNum + i].status == 1)
			{
...
function showTipsWindow(type,num)
{
	...
	else if(type == 3)//显示配对提示
	{
		...
		$("tipsContent2").innerHTML = text[0] + num + text[1];//num:pinCode
		$("tipsWindow").style.display = "block";
		$("okButton").style.display = "block";
		$("yesButton").style.display = "none";
		$("noButton").style.display = "none";
...
{
	emptyBlueToothDeviceList();
    return bluetooth.getAvailableDevice();
}

function getDevicesList()
{
	//testLog("YM getDevicesList");
	return bluetooth.getDeviceList(arguments[0]);
}

function pair(btdev_addr)
{
    //testLog("YM pair");
    return bluetooth.pair(btdev_addr);
}

function connect(btdev_addr)
{
	//testLog("YM connect");
	return bluetooth.connect(btdev_addr);
}

function disconnect(btdev_addr)
{
	//testLog("YM disconnect");
	return bluetooth.disconnect(btdev_addr);
}
...
function getBluetoothVolume()
{
    ...
    return setting.bluetoothVolume;
}
...
function removeBluetoothFromPairList(btdev_addr)
{
    testLog("removeBluetoothFromPairList id:" + btdev_addr);
    bluetooth.removeBluetoothFromPairList(btdev_addr);
}
