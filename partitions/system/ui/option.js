...
var listItemName = [top.optionOptions[0], top.optionOptions[1], top.optionOptions[2],
				    ...
				    top.bt_title,top.sysOptions[6],top.sysOptions[14],top.sysOptions[16],top.optionOptions[7]];
...
if((top.isAUFlag && 26 == channel.systemCountry) ||
    ...
    var LocationWords = [top.initLocations[0],top.initLocations[1]];
else
...
{name:"autoShopMode",param:function(){doAutoShopMode();},showFlag:1},
...
function toLeft()
{
    ...
    else if(focusType =="autoShopMode")
    {
        presetPositionH = presetPositionH == 1 ? 0 : 1;
        changeFoucusColor();
        $("preSetSelectFocus").style.left = 305 + presetPositionH * presetFocusWidth + "px";
    }
    else if(focusType =="demoMode")
    {
        presetPositionH--;
        if(presetPositionH<0)
        {
            presetPositionH = demo_videos_name.length-1;
        }
        changeFoucusColor();
        $("preSetSelectFocus").style.left = 305 + presetPositionH * presetFocusWidth + "px";
    }
...
function toRight()
{
    ...
    {
        presetPositionH++;
        if(presetPositionH > demo_videos_name.length-1)
        {
            presetPositionH = 0;
        }
        changeFoucusColor();
...
function toOK()
{
    ...
    {
        autoShopmodeValue = presetPositionH;
    ...
    {
        demoVideoValue = presetPositionH;
...
function doAutoShopMode()
{
    showPersetSelect(offOnWords.length);
    $("preSetOption").innerHTML = listItemName[11];
    for(var i = 0; i < offOnWords.length; i++)
    {
        spans_preset[i + 1].innerHTML = offOnWords[i];
        spans_preset[i + 1].style.color = "#fff";
    }
    presetPositionH = autoShopmodeValue;
    changeFoucusColor();
    $("preSetSelectFocus").style.left = 305 + presetPositionH * presetFocusWidth + "px";
    $("currentOption").style.left = 320 + presetPositionH * presetFocusWidth + "px";
    $("currentOption").style.display = "block";
    ...
    $("smallPopWin").style.display = "none";
    $("preSetBar").style.display = "block";
    focusType ="autoShopMode";
}

function showAutoShopModeItem()
{
    var divs = lis[autoShopModePosIndex].getElementsByTagName("div");
    divs[0].innerHTML = offOnWords[autoShopmodeValue];
}

function doDemo_mode()
{
    showPersetSelect(demo_videos_name.length);
    $("preSetOption").innerHTML = listItemName[12];
    for(var i = 0; i < demo_videos_name.length; i++)
    {
        spans_preset[i + 1].innerHTML = demo_videos_name[i];
        spans_preset[i + 1].style.color = "#fff";
    }
    presetPositionH = demoVideoValue;
    changeFoucusColor();
    $("preSetSelectFocus").style.left = 305 + presetPositionH * presetFocusWidth + "px";
    $("currentOption").style.left = 320 + presetPositionH * presetFocusWidth + "px";
    $("currentOption").style.display = "block";
...
function showDemoModeItem()
{
    var divs = lis[demoModePosIndex].getElementsByTagName("div");
    divs[0].innerHTML = demo_videos_name[demoVideoValue];
}
...
function refreshAllMenuListShow()
{
	...
	if(top.appOpenFlag){//APP下只显示图像和声音模式选择
		for(var i = 0; i < menuList.length; i++)
		{
		...
		{
			menuList[picPresetPosIndex].showFlag = 1;
			menuList[soundPresetPosIndex].showFlag = 1;
			showPicPresetItem();
			showSoundPresetItem();
		}
		else
...
function ttsRead()
{
	...
	if("opitonList" == focusType)
	{
		switch(currentName)
		{
            ...
            case "autoShopMode":
                ...
                currentStatus = offOnWords[autoShopmodeValue];
                break;
            case "demoMode":
                ttsName = listItemName[12] + ",";
                currentStatus = demo_videos_name[demoVideoValue];
                break;
			case "settings":
				ttsName = listItemName[13];
				currentStatus = "";
				break;
			default:
				ttsName = "";
    ...
    else if("autoShopMode" == focusType)  //soundPreset选项菜单
    {
        ttsName = "";
        currentStatus = offOnWords[presetPositionH];
        isSelected(offOnWords[locationValue] == currentStatus);
    }
    else if("demoMode" == focusType)  //soundPreset选项菜单
    {
        ttsName = "";
        currentStatus = demo_videos_name[presetPositionH];
        isSelected(demo_videos_name[demoVideoValue] == currentStatus);
    }
