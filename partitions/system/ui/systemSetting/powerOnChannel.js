...
var SUB_MENU_START 				= 160;					//五级级菜单焦点移动起始位置
var SUB_MENU_STEP 				= 70;					//五级级主菜单焦点移动的step
var FORM_STEP					= 80;					//弹出框焦点移动的step
var ONE_PAGE_CHANNEL_SUM		= 11; 					//每一页最多含有的channel数量
var CHANNEL_POSITION_START 		= 60;					//power on chanenl list起始坐标
var FOCUS_POSITION_START 		= 230;					//power on chanenl list起始坐标
...
function toUp()
{
	...
	{			
		...
		else
		{
			...
			list_position_h--;
			if(list_position_h == 0 && nowPage != 0)
			{
				nowPage--;
				list_position_h = ONE_PAGE_CHANNEL_SUM;
				showList(nowPage,list_position_h - 1);
...
function doEnter()
{
	if(focusOn == "menu")
	{
		...
		{
			$("statusFocus").style.top = FOCUS_POSITION_START + SUB_MENU_STEP*(list_position_h - 1) + "px";
	...
	else
	{
		...
		for (var i = 0; i < ONE_PAGE_CHANNEL_SUM; i++)
		{
			if (i < listSum)
			{
				...
				fifthMenuRightSpanList[i + 1].innerHTML = channelList[i].number;
				...
				{
					$("statusFocus").style.top = FOCUS_POSITION_START + SUB_MENU_STEP*i + "px";
					$("statusFocus").style.display = "block";
				}
			}
			else
...
function showList(page,cursor)
{
	...
	if(page >= 0 && page < (pageSum - 1))
	{
		for(var i = 0; i < ONE_PAGE_CHANNEL_SUM; i++)
		{
			fifthMenuRightSpanList[i + 1].innerHTML = channelList[page*ONE_PAGE_CHANNEL_SUM + i].number;
...
function init()
{
    ...
    $("titleNameSpan").innerHTML = powerOnChTitleWords[0];
	for(var i = 0; i < 1; i++)
	{
		fifthMenuRightSpanList[i].innerHTML = powerOnChTitleWords[i+1];
	}
...
function ttsRead()
{
	...
	top.ttsLog("focusOn=" + focusOn + " parent.currentShowMenu=" + parent.currentShowMenu);
	if(("menu" == focusOn) && (4 == parent.currentShowMenu))
	{
		top.ttsLog("list_position_h=" + list_position_h);
		if(0 == list_position_h)
		{
			top.ttsLog("fifthMenuRightSpanList[list_position_h].innerHTML=" + fifthMenuRightSpanList[list_position_h].innerHTML);
			ttsName = fifthMenuRightSpanList[list_position_h].innerHTML;
			top.ttsLog("selectModeWords[powerOnChannelMode]=" + selectModeWords[powerOnChannelMode]);
			currentStatus = selectModeWords[powerOnChannelMode];
		}
		else
		{
			top.ttsLog("fifthMenuRightLabelList[list_position_h].innerHTML=" + fifthMenuRightLabelList[list_position_h].innerHTML);
			top.ttsLog("fifthMenuRightLabelList[list_position_h+1].innerHTML=" + fifthMenuRightLabelList[list_position_h + 1].innerHTML);
			ttsName = fifthMenuRightSpanList[list_position_h].innerHTML + "," + fifthMenuRightLabelList[list_position_h - 1].innerHTML + "," + fifthMenuRightDivList[list_position_h].innerHTML + ",";
			currentStatus = "";
			top.ttsLog("defaultPowerOnCh.number=" + defaultPowerOnCh.number);
			top.ttsLog("channelList[list_position_h].number=" + channelList[list_position_h].number);
			top.ttsLog("defaultPowerOnCh.serviceType=" + defaultPowerOnCh.serviceType);
			top.ttsLog("channelList[list_position_h].serviceType=" + channelList[list_position_h].serviceType);
			isSelected((defaultPowerOnCh.number == fifthMenuRightSpanList[list_position_h].innerHTML) && (defaultPowerOnCh.serviceType == channelList[list_position_h].serviceType))
		}
	}
	else if(("powerOnChannelMode" == focusOn) && (4 == parent.currentShowMenu))
	{
		top.ttsLog("powerOnChannelMode=" + powerOnChannelMode + " form_position_h=" + form_position_h);
		ttsName = "";
		currentStatus = selectModeWords[form_position_h];
		isSelected(selectModeWords[powerOnChannelMode] == currentStatus);
	}
	else
	{
		top.ttsLog("focusOn=" + focusOn);
