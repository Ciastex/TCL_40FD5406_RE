...
var phyIni=parseInt(top.g_setting.getProperty("ro.sita.model.HDMI_ARC_CONFIG.HDMISupportARCPort","1"))*16;
	...
	{	
		...
		case top.E_CEC_HDMI_DEVICE_INSERTED:
			...
			if(top.getArcStatus() == true)
			{
				...
				//if(setting.sPDIFDDP != 1 && setting.sPDIFType == 0)
				//{
				//	setting.sPDIFDDP = 1;
				//}
			...
			else
			{
				...
				//if(devCurNum.charAt(0) == "1"&& setting.sPDIFDDP != 0)
				//{
				//	setting.sPDIFDDP = 0;
				//}
			}
...
function toBack()
{
	...
	else if(focusType == "amplifierstatus")
	{
		...
		setting.speakerStatus=amplifierstatusIndex?0:1;
		positionV=2;
		showDeviceName();
	}
	else if(focusType=="guideMenu")
...
function changeFoucusColor()
{
	...
	if(focusType=="tLinkList")
	{
		...
		else if(oldpositionV  == 1)
			divs[1].innerHTML="<img src='images/arrow/icon_Submenu.png' height='32px' width='32px'/>";
		else if(oldpositionV  == 2)
			divs[1].innerHTML="";	
	}
	else if(focusType=="setuppage")
	{
	...
	if(focusType=="tLinkList") 
	{
		if(positionV==0)
			...
			divs_pop[positionV*2+1].innerHTML="<img src='images/arrow/icon_Submenu_focus.png' height='37px' width='37px'/>";
	}
	else if(focusType=="setuppage")
	{
		if(positionV <= 1)
...
function deviceInit()
{
    ...
    for(var i=0;i<devicenum;i++)
	{
		...
		if(devTypeArr[i] == 5)
		{
			 ...
			 setting.speakerStatus=(amplifierstatusIndex && setting.getArcStatus(devNumArr[i]) == 1)?0:1;
			 arcloga=tloga;
			 arcphya=tphya;
		}
...
function devicefunction(index)
{
	...
	else if(index == 1)
	{
		...
		console.log("CEC device standby,devType = " + devTypeArr[devicePV-2]);
		if(devTypeArr[devicePV-2] == 5)
		{
			console.log("ARC device standby, Unmute speaker !");
			if(setting.speakerStatus == 0)
			{
				setting.speakerStatus = 1;
			}
		}
...
function ttsRead()
{
	...
	if("tLinkList" == focusType)
	{
		switch(positionV)
		{
			case 0:
				...
				currentStatus = tLinkOption[tLinkIndex];
				break;
			case 1:
			...
			case 4:
			case 5:
				ttsName = spans_pop[positionV].innerHTML
				break;
			default:
				ttsName = "";
				currentStatus = "";
	...
	else if("TlinkSel" == focusType)  //soundPreset选项菜单
	{
		ttsName = "";
		currentStatus = tLinkOption[presetPositionH];
		isSelected(tLinkOption[tLinkIndex] == currentStatus);
	}
	else if("setuppage" == focusType)
	{
		switch(positionV)
		{
			case 0:
				ttsName = setupListName[0] + ",";
				currentStatus = tLinkOption[autoPowerIndex];	//auto fowerr on
				break;
			case 1:
				ttsName = setupListName[1] + ",";
				currentStatus = tLinkOption[autoStandbyIndex];	//auto standby
				break;
			case 2:
				ttsName = setupListName[2];		//Guide menu
				break;
			default:
				break;
		}
	}
	else if("autopoweron" == focusType)  //soundPreset选项菜单
	{
		ttsName = "";
		currentStatus = tLinkOption[presetPositionH];
		isSelected(tLinkOption[autoPowerIndex] == currentStatus);
	}
	else if("autostandby" == focusType)  //soundPreset选项菜单
	{
		ttsName = "";
		currentStatus = tLinkOption[presetPositionH];
		isSelected(tLinkOption[autoStandbyIndex] == currentStatus);
	}
	else if("amplifiername" == focusType)
	{
		switch(positionV)
		{
			case 0:
				ttsName = deviceOptionName[0];	//power on
				break;
			case 1:
				ttsName = deviceOptionName[1];	//standby
				break;
			case 2:
				ttsName = deviceOptionName[2];		//amplifier volume
				currentStatus = tLinkOption[amplifierstatusIndex];
				break;
			default:
				break;
	...
	else if("amplifierstatus" == focusType)  //soundPreset选项菜单
	{
		ttsName = "";
		currentStatus = tLinkOption[presetPositionH];
		isSelected(tLinkOption[amplifierstatusIndex] == currentStatus);
	}
