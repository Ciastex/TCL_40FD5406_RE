//常量-----------tclLauncherUpdateDate 20160801------------
...
function addKeyDownListener() {
	...
	console.log("--hzm index addKeyDownListener currentElemInBottomTitleIndex = " + currentElemInBottomTitleIndex);
    console.log("--hzm index addKeyDownListener sessionStorage.subpageFlag = " + sessionStorage.subpageFlag);
    if (sessionStorage.subpageFlag == 1)
	{
        console.log("--hzm index addKeyDownListener test sessionStorage.subpageFlag!!!!");
        currentElemInBottomTitleIndex = -1;
        //sessionStorage.subpageFlag = 0;
        delete sessionStorage.subpageFlag;
    }
	if (currentElemInBottomTitleIndex != -1 ) {// 由二级目录返回
        ...
        console.log("--hzm index addKeyDownListener sessionStorage.blockId = " + sessionStorage.blockId);
...
function keyProcess(evt) 
{
	...
	case VK_RIGHT: // 按right var VK_RIGHT = 39;
		...
		case FocusFiled_BottomTitle:// 下标题
            console.log("--zhm VK_RIGHT FocusFiled_BottomTitle currentElemInBottomTitleIndex = "+currentElemInBottomTitleIndex);
			if (currentElemInBottomTitleIndex == 3)
			{
				return;
			}
			// 在换页前，把原来的页恢复原样
		...
		case FocusFiled_page:// page(home,videos,tv,apps)
            ...
            console.log("--zhm VK_RIGHT FocusFiled_page rightNodeIdValue = "+rightNodeIdValue);
            console.log("--zhm VK_RIGHT FocusFiled_page currentElemInBottomTitleIndex = "+currentElemInBottomTitleIndex);
	...
	case VK_LEFT: // Left var VK_LEFT = 37;
		...
		case FocusFiled_topTitle:// 上标题
			...
			break;
		case FocusFiled_BottomTitle:// 下标题
            console.log("--zhm VK_LEFT currentElemInBottomTitleIndex0 = "+currentElemInBottomTitleIndex);
            if (currentElemInBottomTitleIndex == 0) {
                //gotoRecent();
                //currentElemInBottomTitleIndex = -1;//从more页面退出到recent页面再退出到主页会导致焦点丢失  没作用
                swithcWindow("recentContent.html");
                //window.parent.location = "recentContent.html";//不能用parent参数，否则recent页面无法调用top函数
                return;
            }
			// 在换页前，把原来的页恢复原样
			iframeList[currentElemInBottomTitleIndex].classList.remove('active');
		...
		case FocusFiled_page:// page(home,videos,tv,apps)
			...
			console.log("--hzm VK_LEFT FocusFiled_page leftNodeIdValue = " + leftNodeIdValue);
            console.log("--hzm VK_LEFT FocusFiled_page currentElemInBottomTitleIndex = " + currentElemInBottomTitleIndex);
	...
	case VK_ENTER:
    {
		...
		case FocusFiled_page:// 主页			
			...
			sessionStorage.blockId = top.pressOkObject.getAttribute('id');
