...
var focusTopDistance = 0;
var focusLeftDistance = 418;
var focusMenuHeight = 150;
var focusMenuTopDistance = 30;
...
var VC_I = 4;//Vertical C/ount Icon: [4]picture/video: the number of one page on Vertical direction
var VC_F = 7;//Vertical Count Filelist: [8]all file/music: the number of one page on Vertical direction
...
var isLoadingDisplayFlag = false;
var fileListDepth = 0;
...
//var divxActiveStatus = multimedia.divxActiveStatus();
var firstEnterInitingFlag = false;
...
function init()
{
	...
	*/
	...
	{
		initMediaListPage();
		if(top.g_deviceInfo.type==ENTER_USB_TYPE)
		{
		...
		{
			testLog("usb first entering init tree finished");
			currentList = getFileList(top.g_mediaFolderPath, listStart, -1);
			setPageListVCount();
			refreshFileList(currentList, "");
			displayLoadingBox(0);
...
function initMediaListPage()
{
	...
	console.log("----------------------------------g_mediaFolderPath=="+top.g_mediaFolderPath);
	document.onkeydown = keyProc;
	ttsRead();
}
...
function setMediaListPage()
{
	...
	if(focusTab == LIST_AREA 
		&& (currentMediaType == ALL_TYPE || currentMediaType == MUSIC_TYPE || currentMediaType == PIC_TYPE || currentMediaType == VIDEO_TYPE))
	{
		...
		else
		{
			...
			$(fName + getF(curListF)).style.left = cRightDis + "px";//current list become bg
			//$(fName + getF(curListF + 1)).style.zIndex = 22;
			$(fName + getF(curListF + 1)).style.left = cLeftDis + "px";//info list become current list
		...
		&& (currentMediaType == PIC_TYPE || currentMediaType == VIDEO_TYPE))
	{
		...
		menuListDiv[currentMediaType].style.color = "#ffffff";
		if("true" == mediaPictureOnly)
		{
			$('menuFocus').style.top = focusMenuTopDistance + (0 * focusMenuHeight) + "px";
		}
		else
		{
			$('menuFocus').style.top = focusMenuTopDistance + (currentMediaType * focusMenuHeight) + "px";
...
function keyProc(evt)
{
	...
	var keycode = evt.which;
		console.log("-------------------------------------keycode=="+keycode+"g_mediaFolderPath=="+top.g_mediaFolderPath+"focusTab=="+focusTab);
	switch (keycode)
	{
	...
	if(!operateFlag && (keycode != top.VK_TV && keycode != top.VK_HOME && keycode != top.VK_EXIT && keycode != top.VK_BACK)
	...
	{
		case top.VK_ENTER:
			if(!emptyDeviceFlag && !isLoadingDisplayFlag)
			{
		...
		case top.VK_RIGHT:
            if(!emptyDeviceFlag && !isLoadingDisplayFlag)
			{
				...
				toRight();
				if(focusTab == LIST_AREA && (currentMediaType == ALL_TYPE || currentMediaType == MUSIC_TYPE)){
					setTimeout(function(){ 
						ttsRead();
					}, 200)
				} 
		...
		case top.VK_LEFT:
            if(!emptyDeviceFlag && !isLoadingDisplayFlag)
			{
				...
				toLeft();
				if(focusTab == LIST_AREA){
					if((currentMediaType == PIC_TYPE || currentMediaType == VIDEO_TYPE) && (focusIndex % 4 == 0)){
							setTimeout(function(){ 
							ttsRead();
						}, 200)
					} 
...
function notifyProcess(evt)
{
    ...
    var value = evt.modifiers;
		console.log("-------------------------------------msg=="+msg+"g_mediaFolderPath=="+top.g_mediaFolderPath);
	...
	switch(msg)
	{
		...
		case top.E_DLNA_DMR_PUSH_YOUTUBE:
			...
			break;
		case top.E_MEDIA_THUMBNAIL_SUCCESS:
			//if(currentList[listStart + focusIndex].type != ALL_TYPE)
				//refreshFileInfo(currentList[listStart + focusIndex]);
			break;
		case top.E_MEDIA_FILE_SCAN_STARTED:
			testLog("usb first entering init tree started");
			operateFlag = false;
			break
		case top.E_MEDIA_FILE_SCAN_FINISHED:
			testLog("usb first entering init tree finished");
			currentList = getFileList(top.g_mediaFolderPath, listStart, -1);
			setPageListVCount();
			refreshFileList(currentList, "");
		...
		*/
		...
		case top.E_APP_PROCESS_INITIALIZATION:
			...
			top.systemEventProc(evt);
			break;
		case top.E_MEDIA_PLAYER_STATUS_PREPARED://add for file details acquirement
			//refreshFileInfoDetail();
			displayLoadingBox(0);
			operateFlag = true;
			break;
...
function toLeft()
{
	if(focusTab == LIST_AREA)
	{
		...
		if(currentMediaType == ALL_TYPE || currentMediaType == MUSIC_TYPE || currentMediaType == PIC_TYPE || currentMediaType == VIDEO_TYPE)//allfile & music
...
function toRight()
{
		...
		else*///all file & music
		{
			...
			else if(currentList[listStart + focusIndex].type == PIC_TYPE)
			{
				...
				top.g_currentMedia = currentList[listStart + focusIndex];
					console.log("----------------------------------g_currentMedia=="+top.g_currentMedia.path);
	...
	else if(focusTab == OPTION_AREA)
	{
		...
		else
		{
			...
			{
				if(multimedia.divxActiveStatus() == 1)
				{
				...
				else if((multimedia.divxActiveStatus() == 0)||(multimedia.divxActiveStatus() ==2))
				{
			...
			else	
			{
				...
				else if(multimedia.divxActiveStatus() ==2)   //when divxActiveStatus==0,deregistration is not displayed.
				{
...
function toUp()
{
	if(focusTab == MAIN_AREA && focusIndex >= 0)
	{
		if(focusIndex == 0)
	    {
			focusIndex = 3;
		    menuListDiv[focusIndex - 3].style.color = "#aaaaaa";
		    menuListDiv[focusIndex].style.color = "#ffffff";
		    setHTMLNotScroll(menuListDiv[focusIndex - 3]);
	...
	else if(focusTab == LIST_AREA)
	{
		...
		else*///all file & music
		{
		    ...
		    else if(focusIndex == 0	&& listStart == 0)
			{	
				listStart = listLength - lCount;
				focusIndex = pageVCount-1;
				$('focus').style.top = (focusTopDistance + focusIndex * focusHeight) + "px";
				refreshFileList(currentList, (fName + "Ul" + curListF));
				//$(sfName + getF(curListF)).style.top = SCROLL_F_TOP + 
				//	parseInt(SCROLL_MAX_HEIGHT * listStart / listLength) + "px";
	...
	else if(focusTab == OPTION_AREA)
	{
		...
		if(0)//pingbi dvix
		{
			optionFirstListLength = ((multimedia.divxActiveStatus() == 2) || (multimedia.divxActiveStatus() == 0)) ? 3 : 4;
		}
		else
		{
			optionFirstListLength =2;
		}
		changeOptionFocus(-1);
...
function toDown()
{
	...
	else if(focusTab == LIST_AREA)
	{
		...
		else*/ //all file & music
		{
			...
			else if( (listStart + lCount == listLength))//the bottom of current page
				{	
					listStart=0;
					refreshFileList(currentList, (fName + "Ul" + curListF));
					//$(sfName + getF(curListF)).style.top = SCROLL_F_TOP + 
...
function toOK()
{
	...
	{
		if(currentMediaType == ALL_TYPE || currentMediaType == MUSIC_TYPE || currentMediaType == PIC_TYPE || currentMediaType == VIDEO_TYPE)//all files & music
		{
			toRight();
		}
	...
	else if(focusTab==DIVX_DEREG_ARER)
	{
		if(form_position_h == 1)
		{
			...
			registContentPList[1].innerText = registWords[1] + multimedia.registDivX();
...
function toBack()
{
	...
	else if((currentMediaType == ALL_TYPE || currentMediaType == MUSIC_TYPE || currentMediaType == PIC_TYPE || currentMediaType == VIDEO_TYPE)
		&& focusTab == LIST_AREA)
	{
		toLeft();
	}
...
function changeOptionFocus(flag)
{
	...
	if(flag == 1)//down
	{
		if(0)//pingbi dvix
		{
			optionFocusIndex_1 = ((multimedia.divxActiveStatus() == 2) || (multimedia.divxActiveStatus() == 0))
				? parseInt((optionFocusIndex_1 + 1) % 3, 10)
				: parseInt((optionFocusIndex_1 + 1) % HC_I, 10);
		}
		else
		{
			optionFocusIndex_1 =parseInt((optionFocusIndex_1 + 1) % optionFirstListLength, 10);
		}
...
function resetScrollHtmlInner(htmlTagList, displayStyle, overFlowStyle)
{
	...
	setTimeout(function()
	{
		htmlTagList.innerHTML = htmlTagList.getAttribute("contentText");
	},0);
...
function toOption()
{
	...
	else//show option menu
	{
		...
		$('optionFocus_1').style.display = 'block';
		setOptionMenuFocus(optionFocusIndex_1, -1);
		top.g_channel.testLog("@@@@@@@@@@@multimedia.divxActiveStatus()="+multimedia.divxActiveStatus());
		if(0)//((multimedia.divxActiveStatus() == 2) || (multimedia.divxActiveStatus() == 0))//pingbi dvix
		{
			optionFirstListLength = 3;
		...
		{
			optionFirstListLength = 2;
			var tmpLi1 = $('optionFirstList').getElementsByTagName('li');
			tmpLi1[2].style.display = 'none';
			tmpLi1[3].style.display = 'none';
		}
		hideMenu(DELAY_TIME_15);
	}
}
...
function leaveRefreshFileColumn()
{
	...
	else
	{
		...
		$(fName + getF(curListF)).style.left = cMiddleDis  + "px";
		$(fName + getF(curListF + 1)).style.left = cRightDis + "px";
		//$(fName + getF(curListF + 2)).style.zIndex = 1;
		$(fName + getF(curListF + 2)).style.left = cRightDis + "px";
...
function showIconEmptyTips(time)
{
	...
	var ttsStr = $("iconListEmptyTipsInfo").innerText;
...
function updateFileIconImg(divName, list, start)
{
	...
	for(var i = 0, countTmp = HC_I * VC_I; i < countTmp; i++)
	{
		...
		{
			tmpPicPath = multimedia.getVideoScreenShot(list[start + i].path,list[start + i].name, listImg[i].width, listImg[i].height);
			if(tmpPicPath != null && tmpPicPath != "")
			{
				listImg[i].src = tmpPicPath;
			}
...
function displayLoadingBox(flag)
{
	switch(flag)
	{
		case 0://hide loading box
			operateFlag = true;
            isLoadingDisplayFlag = false;
			$('loadingBox').style.display = 'none';
			break;
		case 1://display loading box
			operateFlag = false;
            isLoadingDisplayFlag = true;
...
function refreshFileList(list, divName)//type: [0]all files [1]pics [2]video [3]music
{
	...
	if(divName == "")
	{
		$('fileInfo').style.left = cRightDis + "px";
		$('emptyFolderTips').style.left = cRightDis + "px";
		if(currentMediaType == ALL_TYPE	|| currentMediaType == MUSIC_TYPE || currentMediaType == PIC_TYPE
				|| currentMediaType == VIDEO_TYPE)//all file
		{
...
function refreshFileInfo(filetmp)
{
	...
	else
	{
		...
		displayLoadingBox(1);
		operateFlag = false;
		fileDetailType = filetmp.type;
		//multimedia.mediaOpen(filetmp.path);
		//refreshFileInfoDetail(filetmp.path,filetmp.name);
		displayLoadingBox(0);
		operateFlag = true;
	}
}

function refreshZeroSizeFileInfoDetail(name)
{
    console.log("=====call refreshZeroSizeFileInfoDetail =====");
    var img = $('fileInfo').getElementsByTagName('img');
    var listSpan = $('fileInfo').getElementsByTagName('span');
    if(fileDetailType == PIC_TYPE)
	{
        img[0].src = "images/USB-picture-Default.png";
        listSpan[1].innerText = fileNameText + ": " + name;
        listSpan[2].innerText = fileSizeText + ": ";
        listSpan[3].innerText = fileDateText + ": ";
        listSpan[4].innerText = "";
        listSpan[5].innerText = "";
        listSpan[6].innerText = "";
	}
    else if(fileDetailType == VIDEO_TYPE)
	{
        img[0].src = "images/USB-video-Default.png";
        listSpan[1].innerText = fileNameText + ": " + name;
        listSpan[2].innerText = fileDirectorText + ": ";
        listSpan[3].innerText = fileCopyrightText + ": ";
        listSpan[4].innerText = fileDateText + ": ";
        listSpan[5].innerText = "";
        listSpan[6].innerText = "";
	}
    else if(fileDetailType == MUSIC_TYPE)
	{
        img[0].src = "images/USB-Music-Default.png";
        listSpan[1].innerText = fileNameText + ": " + name;
        listSpan[2].innerText = fileArtistText + ":  ";
        listSpan[3].innerText = fileAlbumText + ":  ";
        listSpan[4].innerText = fileYearText + ":  ";
        listSpan[5].innerText = "";
        listSpan[6].innerText = "";
	}
}
function refreshFileInfoDetail(path,name)
{
	var img = $('fileInfo').getElementsByTagName('img');
	var listSpan = $('fileInfo').getElementsByTagName('span');
	if(fileDetailType == PIC_TYPE)
	{
		var info = getFileInfo(path,name);
	...
	{	
		var tmp = multimedia.getVideoScreenShot(path,name,img[0].width, img[0].height);
		var info = getFileInfo(path,name);
		if(tmp != null && tmp != "")
		{
			img[0].src = tmp;
	...
	else if(fileDetailType == ICON_TYPE)
	{	
...
function resetFocusShape()
{
	if(focusTab == MAIN_AREA)
	{
		$(fName + getF(curListF)).style.backgroundImage = "url(images/Background2_Tab_disable_Config_Media_MENU.png)";
	    $('menu').style.backgroundImage = "url(images/Background_Tab_enable_Config_Media_MENU.png)";
		$('focus').style.display = 'none';
		focusMenuHeight = 150;
		focusWidth = 0;
		focusMenuTopDistance = 30;
		menuListDiv[currentMediaType].style.color = "#ffffff";
	...
	{
		$(fName + getF(curListF)).style.backgroundImage = "url(images/Background2_Tab_enable_Config_Media_MENU.png)";
		//$(fName + getF(curListF+1)).style.backgroundImage = "url(images/Background2_Tab_enable_Config_Media_MENU.png)";
		//$(fName + getF(curListF+2)).style.backgroundImage = "url(images/Background2_Tab_enable_Config_Media_MENU.png)";
		//$(fName + getF(curListF+3)).style.backgroundImage = "url(images/Background2_Tab_enable_Config_Media_MENU.png)";
	    $('menu').style.backgroundImage = "url(images/Background_Tab_disable_Config_Media_MENU.png)";
		setHTMLNotScroll(menuListDiv[currentMediaType]);
		$('menuFocus').src = "images/focus_Activation.png";
		$('focus').style.display = 'block';
		focusTopDistance = 35;
		if(currentMediaType == ALL_TYPE || currentMediaType == MUSIC_TYPE || currentMediaType == PIC_TYPE || currentMediaType == VIDEO_TYPE)
		{
			focusHeight = 138;
			focusWidth = 0;
			focusLeftDistance = 418;
			$('focus').style.top = focusTopDistance + focusIndex * focusHeight + "px";
			$('focus').style.left = focusLeftDistance + "px";
			//$('focus').style.left = (fileListDepth == 0) ? 
				//focusLeftDistance + "px" : (focusLeftDistance + cSmallDis) + "px";
			$('focus').style.width = 972 + "px";
			$('focus').style.height = 138 + "px";
...
function setPageListVCount()
{
	...
	if(currentMediaType == ALL_TYPE || currentMediaType == MUSIC_TYPE || currentMediaType == PIC_TYPE || currentMediaType == VIDEO_TYPE)//all file & music
	{
...
function getFileInfo(path,name)
{
	...
	if(fileDetailType == PIC_TYPE)
	{
		info = multimedia.getMediaPictureFileInfo(path,name,1);
	}
	else if(fileDetailType == VIDEO_TYPE)
	{
		info = multimedia.getMediaVideoFileInfo(path,name);
	}
	else if(fileDetailType == MUSIC_TYPE)
	{
		info = multimedia.getMediaMusicFileInfo(path,name);
	}
	return info;
}
...
function initDeregistration()
{
	...
	$("deregistContent1").innerHTML = deregistWords[0]+multimedia.deregistDivX();
...
function ttsRead()
{
	...
	switch(focusTab)
	{
		case MAIN_AREA:
			try
			{
				ttsStr = mainListArray[focusIndex];
			} catch(e)
			{
			...
			break;
		case LIST_AREA:
			if(lastFocusIndex == focusIndex && focusTab == lastFocusTabTts)
			{
				return;
			}
			...
			try
			{
				ttsStr = currentList[listStart + focusIndex].name;
			}
			catch(e)
			{
				console.log("---------ttstest---------error=" + e);
		...
		case OPTION_AREA:
			...
			{
				ttsStr = sortName + "," + sortArray[sortOrder];
			}
			else
			{
				ttsStr = parserName + "," + parserArray[parser];
			}
			break;
		case OPTION_DETAIL_AREA:
			if(0 == optionFocusIndex_1)
			{
				ttsStr = sortArray[optionFocusIndex_2] + "," + isSelected(sortOrder == optionFocusIndex_2);
			}
			else
			{
				ttsStr = parserArray[optionFocusIndex_2] + "," + isSelected(parser == optionFocusIndex_2);
			}
			break;
		default:
			break;
	}
	lastFocusTabTts = focusTab;
	if("" != ttsStr)
	{
		console.log("---------ttstest---------ttsStr=" + ttsStr);
		top.setTTSSay(top.SPEECH_TYPE_FLUSH, ttsStr);
