...
var skipFlagSubtitleEncode = false;
...
var subtitleEncodeInfoList = 0;
var subtitleCount = 0;
var subtitleEncodeCount = 0;
var subtitleStartIndex = 0;
var subtitleEncodeStartIndex = 0;
...
var tmpInfoTimer = 0;
var breakpointTimer = 0;//用于断点续播提示计时
var BREAKPOINT_TIME = 30*1000;
...
var optionIndexSubtitleEncode = 15;
var optionIndexSoundTrack = 16;
var optionIndexTitle = 17;
var optionIndexChapter = 18;
var optionIndexInfo = 19;
...
var optionListNameArray = [top.optionVideoListArray_1[0],
    ...
    top.optionVideoListArray_1[6], top.optionVideoListArray_1[7], "Subtitle Encode", top.optionVideoListArray_1[8],
...
var EncodeTextList = ["AUTO","UCS-2LE", "Big-5", "GB18030", "CP1250", "CP1251", "CP1252", "CP1253", "CP1254", "CP1255", "CP1256", "CP1257", "CP1258"];
...
var soundPresetArray = [top.souPreset[0], top.souPreset[1], top.souPreset[2], top.souPreset[3], 
						top.resetStadium, top.picturePresetArray[1],top.souPreset[4]];
...
var screenModeArray = [top.picSize[0],top.picSize[1],top.picSize[12],top.picSize[13]];
...
var bPrepared =0;
var dataAccess = new DataAccess("/data/tbrowser/videoRecord.ini");
...
function init()
{
    ...
    operationTimer = setTimeout(function () {
        multimedia.getMediaFileList(top.g_deviceInfo.type, 2, top.g_mediaFolderPath, 0, -1);
    ...
    if (top.g_lastEnterType == ENTER_USB_TYPE) {
        setting.setProperty("app.ui.mediaPlay", "1");
	}
	testLog("init done");
}
...
function unInit()
{
	...
	setMEMC(0);
    setting.setProperty("app.ui.mediaPlay", "0");
...
function keyProc(evt)
{
	...
	if((exitFlag || !operatableFlag) && 
		(keycode != top.VK_BACK && keycode != top.VK_HOME && keycode != top.VK_EXIT && keycode != top.VK_TV)) 
	{
		return;
	}
	...
	else
	{
		...
		switch (keycode)
		{
			...
			case top.VK_OPTION:
				if(dmrFlag) break;
				if(focusTab == PICTURE_PRESET_AREA||focusTab == PIC_ZOOM_AREA||focusTab == SOUND_PRESET_AREA) 
					toBack();
				else
				{
			...
			case top.VK_0:
				...
				if(seekableFlag && focusTab != TIPS_AREA && !skipFlagChapter)
				{
					...
					if(chapterSetIndex * 10 > chapterCount)
					{
						setChapterIndex(0);
						break;
					}
			...
			case top.VK_STOP:
					...
					|| playStatus == S_FF_FB || playStatus == S_SF_SB)
				{
                    ...
                    console.log("---------ttstest---------ttsStr=" + STOP_RESUME_INFO);
                    top.setTTSSay(top.SPEECH_TYPE_FLUSH, STOP_RESUME_INFO);
				}
			...
			case top.VK_PLAY_PAUSE:
				...
				else if(playStatus == S_PLAY || playStatus == S_FF_FB || playStatus == S_SF_SB)
				{
					...
					else
					{
                        ...
                        console.log("---------ttstest---------ttsStr=" + cantOperateText);
                        top.setTTSSay(top.SPEECH_TYPE_FLUSH, cantOperateText);
					}
				}
				break;
			case top.VK_PAUSE:
			...
			case top.VK_REWIND:
				...
				hideInfoBox(0);
				hideTmpInfoBox(0);
				if(playStatus != S_STOP_RESUME && playStatus != S_ERROR&&playStatus!=S_SEEK&& seekableFlag && errorFlag != ERROR_VIDEO_NOT_SUPPORTED)
				{
					$('progressArea').style.display = 'block';
					refreshProgressBar();
					clearInterval(progressBarTimer);
			...
			case top.VK_PREV: //previous
				...
				hideMenu(0);
				hideInfoBox(0);
				if(0)//seekableFlag && !skipFlagChapter) 中间件徐昊说不做DIVX认证，所以上下快捷键功能变为切上一个和下一个视频
				{
					clearTimeout(metaInfoTimer);
					previousKeyCount++;
					displayTipsBox(0);
					$('statusIcon').style.display = 'none';
					optionInfoArray[OPTION_CHAPTER] = multimedia.getCurrentProgram();
					if(optionInfoArray[OPTION_CHAPTER] - 1 >= 0)//not the 1st chapter, play previous one //metadatainfo display later
					{
							if(ff_fb_flag||sf_sb_flag)
							{
								multimedia.mediaFastForward(1);
								ff_fb_flag=0;
								sf_sb_flag=0;
							}
							multimedia.changeChapter(--optionInfoArray[OPTION_CHAPTER]);
					}
					else
					{
						...
						|| ($("tmpInfoBox").style.display == 'none' && metadataTipsFlag != 1))
					{
						...
						chapterInfoContainer = multimedia.getCurrentChapter();
						var str = chapterInfoContainer.chapterName;
						showTmpInfoBox(FADE_TIME_4, str);
						break;
					}
			...
			case top.VK_NEXT: //next
				...
				if(0)//seekableFlag && !skipFlagChapter) 中间件徐昊说不做DIVX认证，所以上下快捷键功能变为切上一个和下一个视频
				{
					...
					if(optionInfoArray[OPTION_CHAPTER] + 1 < chapterCount)
					{
						...
						multimedia.changeChapter(++optionInfoArray[OPTION_CHAPTER]);
						/*$("metaDataInfoChapter").innerText = metaDataArray[1] + fillMetaChapterInfo();  //metadatainfo display later
						showMetaInfoBox(FADE_TIME_4);
						var str = chapterInfoContainer.chapterName;
						showTmpInfoBox(FADE_TIME_4, str);*/
					}
...
function notifyProcess(evt)
{
	...
	{/*media下不允许出现这些个消息
        ...
        case top.E_CEC_ARC_AUDIO_STATUS_CHANGE://TODO:2018/9/14
            top.systemEventProc(evt);
            break;
		default:
			break;
    ...
    switch(msg)
	{
		...
		case top.E_MEDIA_BACKWORD_EOS:
			...
			hideMetaInfoBox(0);
			multimedia.mediaStop(0);
			mediaOpen(top.g_currentMedia.path);
			break;
		case top.E_MEDIA_FORWORD_EOS:
			testLog("---- THIS IS E_MEDIA_FORWORD_EOS MSG ----");
			storeVideoTime();
			if(dmrFlag)
			{
				top.g_temp="quickMenu";
			...
			if(titleCount > 1)
			{
				...
				if(playStatus == S_FF_FB || playStatus == S_SF_SB)//ff to eos
				{
					setTipsBox(1);
					clearTimeout(operationTimer);
					operationTimer = setTimeout("multimedia.changeTitle(optionInfoArray[OPTION_TITLE]);", DELAY_TIME);
					break;
				}
				else if(optionInfoArray[OPTION_TITLE] + 1 < titleCount)
				{
					setTipsBox(1);
					clearTimeout(operationTimer);
					operationTimer = setTimeout("multimedia.changeTitle(++optionInfoArray[OPTION_TITLE]);", DELAY_TIME);
			...
			chapterStartIndex = 0;
			subtitleCount = 0;
            subtitleEncodeCount=0;
			subtitleStartIndex = 0;
            subtitleEncodeStartIndex = 0;
			soundTrackCount = 0;
			...
			operationTimer = setTimeout(function(){
		    	multimedia.mediaStop(0);
		    	mediaOpen(top.g_currentMedia.path);}, DELAY_TIME);
			break;
		...
		case top.E_MEDIA_CUSTOM_VIDEO_NO_SUPPORT:
			...
			operatableFlag = true;
			focusTab = TIPS_AREA;
			if(errorFlag==ERROR_AUDIO_NOT_SUPPORTED||errorFlag==ERROR_OTHER)
				{
				...
				{
					errorFlag = ERROR_VIDEO_NOT_SUPPORTED;
					$('eb_info').innerText = eb_info[3];//Audio format not supported.
				}			
			displayTipsBox(1);
			break;
		case top.E_MEDIA_CUSTOM_AUDIO_NO_SUPPORT:
			...
			operatableFlag = true;
			focusTab = TIPS_AREA;
			if(errorFlag==ERROR_VIDEO_NOT_SUPPORTED||errorFlag==ERROR_OTHER)
				{
					errorFlag = ERROR_OTHER;
					$('eb_info').innerText = eb_info[6];
				}
		...
		//case top.E_WIRELESS_CONNECT_FAILURE://net connect failed
			...
			if(dmrFlag)
			{
				...
				focusTab = TIPS_AREA;
				errorFlag = ERROR_OTHER;
				$('eb_info').innerText = netConnectFailText;//Play error, please try again.
				displayTipsBox(1);
			}
		...
		case top.E_MEDIA_PLAYER_STATUS_PLAYING: //playing
			if(focusTab==TIPS_AREA&&(errorFlag==ERROR_NET_DISCONNECTED||errorFlag==ERROR_AUDIO_ERROR||
				errorFlag==ERROR_VIDEO_ERROR||errorFlag==ERROR_OTHER||errorFlag==ERROR_CANT_OPEN))
				return;
			testLog("---- THIS IS PLAYING MSG ----");
			playStatus = S_EOS;
			operatableFlag = false;
			recordtime=playVideoRecord();
			if(recordtime)
				multimedia.mediaPause();
			$('loadingBox').style.display = 'none';
			...
			setTimeout(function(){
				...
				if(!dmrFlag)
				{
					...
					titleCount = multimedia.titleCount;
					chapterCount = multimedia.chapterCount;
					chapterInfoContainer = multimedia.getCurrentChapter();
					fillPGInfo(massInfo.titleLawRating);
					subtitleCount = multimedia.getVideoSubtitleCount(currentFileInfo.path);
                    subtitleEncodeCount = EncodeTextList.length;
					...
					if(subtitleCount <= 1)//subtitle
					{
                        ...
                        skipFlagSubtitleEncode = true;
					}
                    /*if (subtitleEncodeCount <= 1)//subtitle
                    {
                        skipFlagSubtitleEncode = true;
                    }*/
                    ...
                    encodeIndex=getSubtitleEncodeIndex();
                    ...
                    optionInfoArray[OPTION_SUBTITLE_ENCODE] = encodeIndex;
					optionInfoArray[OPTION_SOUND_TRACK] = multimedia.getVideoSoundTrackIndex();
					optionInfoArray[OPTION_TITLE] = multimedia.getCurrentTitle();
					optionInfoArray[OPTION_CHAPTER] = multimedia.getCurrentProgram();
					videoFileInfo = multimedia.getMediaVideoFileInfo(currentFileInfo.path,currentFileInfo.name);
				...
				if(recordtime)
				{
					hideMenu(0);
					focusTab = TIPS_AREA;
					var tmpContinueTime = doTime(recordtime);
					$('eb_info').innerText = eb_info[9] + "(" + tmpContinueTime + ")";
					displayTipsBox(2);
					setBreakpointTimer();
				}
			...
			break;
		case top.E_MEDIA_AV_SYNC_DONE:  //metadatainfo display later
			showMetaInfoBox(FADE_TIME_4);
			var str = chapterInfoContainer.chapterName;
			if(str != undefined)
				showTmpInfoBox(FADE_TIME_4, str);			
...
{
	divxActiveStatus = multimedia.divxActiveStatus();
	protectInfo = multimedia.getProtectInfo;
...
function toLeft()
{
	...
	*/
	...
	else if(focusTab == PICTURE_PRESET_AREA)
	{
		...
		//multimedia.previewPictureMode(optionFocusIndex_2);
		setting.previewPictureMode(pictureModeTransform(optionFocusIndex_2, 1));
	...
	else if(focusTab == PIC_ZOOM_AREA)
	{
		if(optionFocusIndex_2 - 1 >= tmpStartIndex)
		{
			...
			setting.previewScreenMode(screenModeTransform(optionFocusIndex_2,1));
			resetFocusShape();
		}
		...
		else
		{
			...
			switch(optionFocusIndex_1)
			{
				...
				case OPTION_SOUND_PRESET:
					...
					if(optionFocusIndex_2==OPTION_INFO_MAX){
                        soundPresetStartIndex=1;
					}
					else if(optionFocusIndex_2==6){
                        soundPresetStartIndex=2;
					}
					else{
                        soundPresetStartIndex=0;
					}
				...
				case OPTION_SUBTITLE:
					...
					break;
                case OPTION_SUBTITLE_ENCODE:
                    hideMetaInfoBox(0);
                    OPTION_INFO_COUNT = EncodeTextList.length;

                     if(encodeIndex<OPTION_INFO_MAX){
                         subtitleEncodeStartIndex=0;
                         optionFocusIndex_2=encodeIndex;
                         tmpStartIndex=subtitleEncodeStartIndex;
					 }
                       else if(encodeIndex >= EncodeTextList.length - OPTION_INFO_MAX) //
                        {
                            subtitleEncodeStartIndex = EncodeTextList.length - OPTION_INFO_MAX;
                            optionFocusIndex_2 = optionInfoArray[optionFocusIndex_1] - subtitleEncodeStartIndex;
                            tmpStartIndex = subtitleEncodeStartIndex;

                        }
                        else
                        {
                            subtitleEncodeStartIndex=encodeIndex+1-OPTION_INFO_MAX;
...
function toUp()
{
	...
	else if(focusTab == SOUND_PRESET_AREA)
	{
		if(optionFocusIndex_2 - 1 >= 0)
		{
			...
			optionFocusIndex_2 = parseInt((optionFocusIndex_2 - 1 + OPTION_INFO_COUNT) % OPTION_INFO_COUNT, 10);
			//setting.previewSoundEffect(optionFocusIndex_2);
			setOptionSecFocusScroll(optionFocusIndex_2, -1);
			$('optionFocus_2').style.top = focusOptionTopDistance + optionFocusIndex_2 * focusOptionHeight + "px";
		}
		else
		{
			moveUpRefreshSecList(soundPresetArray, "", soundPresetArray.length, 0);
		}
	...
	else if(focusTab == OPTION_DETAIL_AREA && optionFocusIndex_1 != OPTION_INFORMATION)
	{
		...
		else
		{
            ...
            else if (optionFocusIndex_1 == OPTION_SUBTITLE_ENCODE) {
                moveUpEncodeRefreshSecList( subMenuTitleText, subtitleEncodeCount, 0);
...
function toDown()
{
	...
	else if(focusTab == SOUND_PRESET_AREA)
	{
		...
		{
			moveDownRefreshSecList(soundPresetArray, "", soundPresetArray.length, 0);
		}
		setting.previewSoundEffect(soundModeTransform(tmpStartIndex + optionFocusIndex_2, 1));
	}
	else if(focusTab == OPTION_AREA)
	...
	else if(focusTab == OPTION_DETAIL_AREA && optionFocusIndex_1 != OPTION_INFORMATION)
	{
		...
		else
		{
			...
			else if(optionFocusIndex_1 == OPTION_SUBTITLE)
			{
                ...
                moveDownEncodeRefreshSecList( subtitleEncodeCount, 0);
            }
            else if (optionFocusIndex_1 == OPTION_SOUND_TRACK) {
...
function toOK()//DISPLAY_AREA, OPTION_AREA, OPTION_DETAIL_AREA, PLAY_LIST_AREA, PROGRESS_AREA
{
	...
	else if(focusTab == QUICK_MENU_AREA)
	{
		...
		else if(quickMenuType == Q_MENU_TITLE)
		{
			...
			hideMetaInfoBox(0);
			clearTimeout(operationTimer);
			operationTimer = setTimeout(function(){multimedia.changeTitle(quickMenuStartIndex + quickMenuIndex);}, DELAY_TIME);
		}
	...
	else if(focusTab == TIPS_AREA)
	{
		if(tipsBoxFlag == 2)
		{
			clearInterval(breakpointTimer);
			displayTipsBox(0);
			focusTab = DISPLAY_AREA;
			console.log("---------------------------------------------==="+tipsBoxIndex);
			if(tipsBoxIndex)
			{
				multimedia.setSeekPoint(recordtime*1000);
			}
			else
			{
				multimedia.mediaResume();
	...
	else if(focusTab == PLAY_LIST_AREA)
	{
		...
		top.g_availableStatus = mediaOpen(currentPlayList[playListStart + playListIndex].path);
		*/
		hideMenu(0);
		hideTipsBox(0);
		hideInfoBox(0);
		...
		operationTimer = setTimeout(function(){
			storeVideoTime();
			multimedia.mediaStop(1);
			mediaOpen(currentPlayList[playListStart + playListFocusIndex].path);
			}, playListAnimateTime);
	}
	else if(focusTab == OPTION_AREA)
	{
...
function toBack()
{
	...
	switch(focusTab)
	{
		...
		case SOUND_PRESET_AREA:
			setting.previewSoundEffect(optionInfoArray[OPTION_SOUND_PRESET]);
			setOptionSecFocusScroll(-1, -1);
			focusTab = OPTION_AREA;
			resetFocusShape();
			$('optionInfoDetails').style.display = 'none';
		...
		case PICTURE_PRESET_AREA:
			//hideMenu(0);
			setHTMLNotScroll(picPresetListName[optionFocusIndex_2]);
			//multimedia.previewPictureMode(optionInfoArray[OPTION_PIC_PRESET]);
			setting.previewPictureMode(pictureModeTransform(optionInfoArray[OPTION_PIC_PRESET], 1));
			toOption(0);
			break;
		case PIC_ZOOM_AREA:
			//hideMenu(0);
			setHTMLNotScroll(picZoomSpan[optionFocusIndex_2]);
			setting.previewScreenMode(screenModeTransform(optionInfoArray[OPTION_SCREEN_MODE],1));
...
function toYellow()
{
	...
	if($('quickMenu').style.display == 'none')
	{
		...
		if(!skipFlagTitle)
		{
			...
			quickIndex = multimedia.getCurrentTitle();
			fillQuickMenuInfo();
			refreshQuickCheckFlag();
			$('quickMenu').style.display = 'block';
			clearTimeout(operationTimer);
...
function setBreakpointTimer()
{
	clearInterval(breakpointTimer);
	breakpointTimer = setTimeout(function(){
		displayTipsBox(0);
		focusTab = DISPLAY_AREA;
		console.log("breakpointTimer playback!!!");
		multimedia.mediaResume();
	}, BREAKPOINT_TIME);
}
...
//获取encode编号
function getSubtitleEncodeIndex() {

    var index=0;
    var subtitle_encode=multimedia.getSubtitleEncode();
    if(typeof subtitle_encode == "undefined" || subtitle_encode == null || subtitle_encode == ""){
        subtitle_encode=EncodeTextList[0];
    }
    for(var i=0;i<EncodeTextList.length;i++){
        if(subtitle_encode==EncodeTextList[i]){
            index=i;
            break;
        }
    }
...
{
	/*if(string != null && string != "")  Divx不做认证，删除 --hhf
	{
		lawRatingFlag = 1;
...
{
	var CurrentProgram = multimedia.getCurrentProgram();
    var string = CurrentProgram < 0
		? (metaChapterInfo + ": " + (optionInfoArray[OPTION_CHAPTER] + 1))
		: metaChapterInfo + ": " + (CurrentProgram + 1);
    return string;
}
...
function fillOptionSecinfoEncode(totalCount) {
    var num = 5;
    var count = OPTION_INFO_MAX;
    if (totalCount < OPTION_INFO_MAX) {
        num = totalCount;
    ...
    for (var i = 0; i < count; i++) {
        optionSecListSpan[i].innerText = EncodeTextList[subtitleEncodeStartIndex+i];
    }
  /*  for (var i = count; i < OPTION_INFO_MAX; i++) {
        optionMenuList_2[i].style.display = 'none';
        optionSecListSpan[i].innerText = "";
...
function fillOptionSoundSecinfo(totalCount, name, startIndex)
{
	var num = 5;
	var count = OPTION_INFO_MAX;
	if(totalCount < OPTION_INFO_MAX)
...
{
	//optionInfoArray[OPTION_PIC_PRESET] = multimedia.getVideoPicturePreset();
	optionInfoArray[OPTION_FOOTBALL_MODE] = setting.footballMode;//add for football mode
	optionInfoArray[OPTION_PIC_PRESET] =  pictureModeTransform(setting.pictureMode, 0);//
	if(!optionInfoArray[OPTION_FOOTBALL_MODE])
	{
	...
	//optionInfoArray[OPTION_SOUND_PRESET] = multimedia.audioSoundEffect;
	...
	optionInfoArray[OPTION_SCREEN_MODE] =screenModeTransform(setting.pictureSize, 0);
...
function fillOptionFirstList()
{
	...
	if(!skipFlagSubtitle)
	{
        ...
        $('optionSubtitleEncode').getElementsByClassName('optionFirstListSpan_marquee')[0].innerText
            = EncodeTextList[encodeIndex];
    }
...
function fillOptionSecList(index)
{
	...
	switch(index)
	{
		case OPTION_SOUND_PRESET:
			...
			//fillOptionSecinfo(subtitleCount, subMenuTitleText, subtitleStartIndex, 0);
			num = fillOptionSoundSecinfo(soundPresetArray.length, soundPresetArray, soundPresetStartIndex);
			optionArrowDisplay(SEC_INDEX, soundPresetStartIndex, soundPresetArray.length);
			/*
			num = soundPresetArray.length;
        ...
        case OPTION_SUBTITLE_ENCODE:
            ...
            num = fillOptionSecinfoEncode(subtitleEncodeCount);
            if (subtitleEncodeStartIndex == 0) {
                optionSecListSpan[0].innerText = EncodeTextList[0];
            }

            optionArrowDisplay(SEC_INDEX, subtitleEncodeStartIndex, subtitleEncodeCount);
            break;
        case OPTION_SOUND_TRACK:
...
function setChapterIndex(time)
{
	chapterTimer = setTimeout(function(){
		...
		hideTipsBox(0);
		if((multimedia.getCurrentProgram()!=(chapterSetIndex - 1))&&(ff_fb_flag||sf_sb_flag))
		{
				multimedia.mediaFastForward(1);
				ff_fb_flag=0;
				sf_sb_flag=0;
		}
		multimedia.changeChapter(chapterSetIndex - 1);
		//$("metaDataInfoChapter").innerText = metaDataArray[1] + fillMetaChapterInfo();//metadatainfo display later
		chapterSetCount = 0;
		chapterSetIndex = 0;
		chapterSetIndexStr = "";
		...
		chapterInfoContainer = multimedia.getCurrentChapter();
		var str = chapterInfoContainer.chapterName;
		showTmpInfoBox(FADE_TIME_4, str);*/ //metadatainfo display later
		}, time);
}
...
function setSPOption()
{
	switch(optionFocusIndex_1)
	{
		case OPTION_PIC_PRESET://picture preset
			...
			setting.pictureMode=pictureModeTransform(optionFocusIndex_2, 1); 
			$('optionPicturePreset').getElementsByClassName('optionFirstListSpan_marquee')[0].innerText = 
		...
		case OPTION_SOUND_PRESET:
			...
			optionInfoArray[OPTION_SOUND_PRESET] = tmpStartIndex + optionFocusIndex_2;
			$('optionSoundPreset').getElementsByClassName('optionFirstListSpan_marquee')[0].innerText =
				soundPresetArray[optionInfoArray[OPTION_SOUND_PRESET]];
			break;
		case OPTION_FOOTBALL_MODE:
			...
			changeOptionListColor();
			arrowOptionDisplay();
			optionFocusIndex_1 = OPTION_FOOTBALL_MODE;
			//
			if(OPTION_FIRST_MAX_COUNT <= optionFocusIndex_1)
			{
				optionFirstStartIndex = (optionFocusIndex_1 + OPTION_FIRST_MAX_COUNT < optionFirstListLength)
					? optionFocusIndex_1 : optionFirstListLength - OPTION_FIRST_MAX_COUNT;
			}
			else
			{
				optionFirstStartIndex = 0;
		...
		case OPTION_SCREEN_MODE://screen mode
			...
			setting.pictureSize=screenModeTransform(optionFocusIndex_2,1);
        ...
        case OPTION_SUBTITLE_ENCODE:
             ...
             encodeIndex = tmpStartIndex + optionFocusIndex_2;
            optionInfoArray[optionFocusIndex_1] = encodeIndex;
            var encodeString=EncodeTextList[encodeIndex];

            if (OPTION_INFO_COUNT > OPTION_INFO_MAX) {
                subtitleEncodeStartIndex = (optionInfoArray[OPTION_SUBTITLE_ENCODE] > subtitleEncodeCount - OPTION_INFO_MAX)
                    ? subtitleEncodeCount - OPTION_INFO_MAX : optionInfoArray[OPTION_SUBTITLE_ENCODE];
            }
            else {
                subtitleEncodeStartIndex = 0;
            }
            ...
            $('optionSubtitleEncode').getElementsByClassName('optionFirstListSpan_marquee')[0].innerText =encodeString;

            multimedia.setSubtitleEncode(encodeString);
        ...
        case OPTION_TITLE://title
            ...
            clearTimeout(operationTimer);
            operationTimer = setTimeout(function () {
                multimedia.changeTitle(tmpStartIndex + optionFocusIndex_2);
            }, DELAY_TIME);
            return;
        case OPTION_CHAPTER://chapter
            if ((multimedia.getCurrentProgram() != (tmpStartIndex + optionFocusIndex_2)) && (ff_fb_flag || sf_sb_flag)) {
                multimedia.mediaFastForward(1);
                ff_fb_flag = 0;
                sf_sb_flag = 0;
            }
            multimedia.changeChapter(tmpStartIndex + optionFocusIndex_2);
            $('optionChapter').getElementsByClassName('optionFirstListSpan_marquee')[0].innerText =
                chapterInfo + " " + (tmpStartIndex + optionFocusIndex_2 + 1);
            optionInfoArray[OPTION_CHAPTER] = tmpStartIndex + optionFocusIndex_2;
            //metadatainfo display later
            //$("metaDataInfoChapter").innerText = metaDataArray[1] + fillMetaChapterInfo();
            //showMetaInfoBox(FADE_TIME_4);
            if (OPTION_INFO_COUNT > OPTION_INFO_MAX) {
...
function moveUpEncodeRefreshSecList(string, count, offset) {

    if (optionFocusIndex_2 == 0 && tmpStartIndex - 1 >= 0) {
        tmpStartIndex--;
        for (var i = 0; i < OPTION_INFO_MAX; i++) {
            optionSecListSpan[i].innerText = EncodeTextList[(i + tmpStartIndex + offset)];
        }
        if (tmpStartIndex == 0 && string == subMenuTitleText) {
            optionSecListSpan[0].innerText = EncodeTextList[0];
        }
...
function moveDownEncodeRefreshSecList( count, offset) {
...
function refreshProgressBar()
{
	...
	bottomTipsImg = $("bottomTips").getElementsByTagName('img');
	if(dmrFlag==true)
	{
		bottomTipsImg[3].style.display='none';
		bottomTipsImg[4].style.display='none';
		bottomTipsLi[3].style.display='none';
		bottomTipsLi[4].style.display='none';
	}
	if(fileDuration > 0)
	{
		//do nothing
...
function setOptionFocusPosition(type)//[-1]up [1]down
{
	...
	if(type == 1)//down
	{
		if(optionFirstListLength <= OPTION_FIRST_MAX_COUNT)
		{
			...
			optionFocusIndex_1 = optionFocusIndex_1 >= optionFirstListLength
				? 0 : optionFocusIndex_1;
		}
		else
		{
...
function initRegistration()
{
	...
	$("registContent2").innerHTML = registWords[1] + multimedia.registDivX();
...
function divxPlay()
{
	...
	currentFileInfo = multimedia.getCurrentPlayingMediaInfo();
	top.g_currentMedia = currentFileInfo;
	top.g_availableStatus = mediaOpen(currentFileInfo.path);
	...
	for(var i = 0; i < optionListLi.length; i++)
	{
		switch(i)
		{
			...
			case optionIndexSubtitle:
				...
				break;
            case optionIndexSubtitleEncode:
                if (skipFlagSubtitle) {
                    OPTION_SUBTITLE_ENCODE = -1;
                }
                else {
                    currentOptionFirstListArray[displayIndex] = $('optionSubtitleEncode');
                    OPTION_SUBTITLE_ENCODE = displayIndex++;
                }
                optionListLi[i].setAttribute(displayAttri, OPTION_SUBTITLE_ENCODE);
                break;
...
function clickPicZoomFunction(index)
{
	...
	else
	{
		setHTMLNotScroll(picZoomSpan[optionFocusIndex_2]);
		optionFocusIndex_2 = index;
		setting.previewScreenMode(screenModeTransform(index,1));
...
function clickSecondOptionDownIconFunction()
{
	switch(optionFocusIndex_1)
	{
		...
		case OPTION_SUBTITLE://subtitle
            ...
            break;
        case OPTION_SUBTITLE_ENCODE://subtitle_encode  //
            clickIconRefreshSecList(ICON_DOWN, "", "Encode", subtitleEncodeCount, 0);
            break;
        case OPTION_SOUND_TRACK://sound track
...
function storeVideoTime(type)
{	
	if(dmrFlag)
		return ;
	var currentTime=multimedia.getNowPlayTime();
	if(currentTime==fileDuration||type==1)
		currentTime=0;
	var name =currentFileInfo.path;
	var json =
	{
		"name":name,
		"value":currentTime
	...
	{
		top.removeData(dataAccess,"videoRecord",json);
		top.addData(dataAccess,"videoRecord",json);
	}		
}

function playVideoRecord()
{
	if(dmrFlag||bPrepared==0)
		return 0;
	bPrepared=0;
	var currentFileInfoRecord = multimedia.getCurrentPlayingMediaInfo();;
	var name =currentFileInfoRecord.path;
	var value = 0;
	var json =
	{
		"name":name,
	...
	var isCollected = top.getData(dataAccess,"videoRecord",json);
	return isCollected;
}
...
function screenModeTransform(preMode, type)//type[0]: mw to ui; [1]ui to mw
{
	var aftMode = preMode;
	if(type)//ui to mw
	{
...
function ttsRead()
{
    ...
    console.log("---------ttstest---------optionFocusIndex_1 = " + optionFocusIndex_1 + "   optionFocusIndex_2=" + optionFocusIndex_2);
	...
	switch(focusTab)
	{
		case PLAY_LIST_AREA:
			try
			{
                ttsName = currentPlayList[playListFocusIndex].name;
			}
			catch(e)
			{
		...
		case OPTION_AREA:
            ...
            {
                case OPTION_PIC_PRESET:
                    ttsName = optionListNameArray[0] + ",";
                    ttsStatus = picturePresetArray[optionInfoArray[OPTION_PIC_PRESET]];
                    break;
                case OPTION_BACK_LIGHT:
                    ttsName = optionListNameArray[1] + ",";
                    ttsStatus = optionInfoArray[OPTION_BACK_LIGHT];
                    break;
                case OPTION_BRIGHTNESS:
                    ttsName = optionListNameArray[2] + ",";
                    ttsStatus = optionInfoArray[OPTION_BRIGHTNESS];
                    break;
                case OPTION_CONTRAST:
                    ttsName = optionListNameArray[3] + ",";
                    ttsStatus = optionInfoArray[OPTION_CONTRAST];
                    break;
                case OPTION_SATURATION:
                    ttsName = optionListNameArray[4] + ",";
                    ttsStatus = optionInfoArray[OPTION_SATURATION];
                    break;
                case OPTION_TINT:
                    ttsName = optionListNameArray[5] + ",";
                    ttsStatus = optionInfoArray[OPTION_TINT];
                    break;
                case OPTION_SHARPNESS:
                    ttsName = optionListNameArray[6] + ",";
                    ttsStatus = optionInfoArray[OPTION_SHARPNESS];
                    break;
                case OPTION_SOUND_PRESET:
                    ttsName = optionListNameArray[7] + ",";
                    ttsStatus = soundPresetArray[optionInfoArray[OPTION_SOUND_PRESET]];
                    break;
                case OPTION_FOOTBALL_MODE:
                    ttsName = optionListNameArray[8] + ",";
                    ttsStatus = modeOptionOnOff[optionInfoArray[OPTION_FOOTBALL_MODE]];
                    break;
                case OPTION_PLAY_MODE:
                    ttsName = optionListNameArray[9] + ",";
                    ttsStatus = playModeArray[optionInfoArray[OPTION_PLAY_MODE]];
                    break;
                case OPTION_SCREEN_MODE:
                    ttsName = optionListNameArray[10] + ",";
                    ttsStatus = screenModeArray[optionInfoArray[OPTION_SCREEN_MODE]];
                    break;
                case OPTION_3D_MODE:
                    ttsName = optionListNameArray[11] + ",";
                    ttsStatus = Mode3DArray[optionInfoArray[OPTION_3D_MODE]];
                    break;
                case OPTION_3D_L_R:
                    ttsName = optionListNameArray[12] + ",";
                    ttsStatus = Mode3DLRArray[optionInfoArray[OPTION_3D_L_R]];
                    break;
                case OPTION_3D_FILED_DEPTH:
                    ttsName = optionListNameArray[13] + ",";
                    ttsStatus = optionInfoArray[OPTION_3D_FILED_DEPTH];
                    break;
                case OPTION_SUBTITLE:
                    ttsName = optionListNameArray[14] + ",";
                    ttsStatus = subtitleInfoList[optionInfoArray[OPTION_SUBTITLE]].displayName;
                    break;
                case OPTION_SOUND_TRACK:
                    ttsName = optionListNameArray[15] + ",";
                    ttsStatus = soundTrackText + " " + (optionInfoArray[OPTION_SOUND_TRACK] + 1);
                    break;
                case OPTION_TITLE:
                    ttsName = optionListNameArray[16] + ",";
                    ttsStatus = titleMenuTitleText + " " + (optionInfoArray[OPTION_TITLE] + 1);
                    break;
                case OPTION_CHAPTER:
                    ttsName = optionListNameArray[17] + "";
                    ttsStatus = chapterInfo + " " + (optionInfoArray[OPTION_CHAPTER] + 1);
                    break;
                case OPTION_INFORMATION:
                    ttsName = optionListNameArray[18];
                    ttsStatus = "";
                    break;
				default:
					break;
        ...
        case OPTION_DETAIL_AREA:
            ...
            {
                case OPTION_SOUND_PRESET:
                    ttsName = soundPresetArray[optionFocusIndex_2] + ",";
                    ttsStatus = isSelected(optionInfoArray[OPTION_SOUND_PRESET] == optionFocusIndex_2);
                    break;
                case OPTION_FOOTBALL_MODE:
                    ttsName = 	modeOptionOnOff[optionFocusIndex_2] + ",";
                    ttsStatus = isSelected(optionInfoArray[OPTION_FOOTBALL_MODE] == optionFocusIndex_2);
                    break;
                case OPTION_PLAY_MODE:
                    ttsName = 	playModeArray[optionFocusIndex_2] + ",";
                    ttsStatus = isSelected(optionInfoArray[OPTION_PLAY_MODE] == optionFocusIndex_2);
                    break;
                case OPTION_3D_MODE://
                    ttsName = 	Mode3DArray[optionFocusIndex_2] + ",";
                    ttsStatus = isSelected(optionInfoArray[OPTION_3D_MODE] == optionFocusIndex_2);
                    break;
                case OPTION_3D_L_R:
                    ttsName = 	Mode3DLRArray[optionFocusIndex_2] + ",";
                    ttsStatus = isSelected(optionInfoArray[OPTION_3D_L_R] == optionFocusIndex_2);
                    break;
                case OPTION_SUBTITLE:
                    ttsName = 	subtitleInfoList[optionFocusIndex_2+tmpStartIndex].displayName + ",";
                    ttsStatus = isSelected(optionInfoArray[OPTION_SUBTITLE] == optionFocusIndex_2+tmpStartIndex);
                    break;
                case OPTION_SOUND_TRACK:
                    ttsName = 	soundTrackText + " " + (optionFocusIndex_2 +tmpStartIndex + 1) + ",";
                    ttsStatus = isSelected(optionInfoArray[OPTION_SOUND_TRACK] == optionFocusIndex_2 + tmpStartIndex);
                    break;
                case OPTION_TITLE:
                    ttsName = 	 titleMenuTitleText + " " + (optionFocusIndex_2 + tmpStartIndex + 1) + ",";
                    ttsStatus = isSelected(optionInfoArray[OPTION_TITLE] == optionFocusIndex_2 + tmpStartIndex);
                    break;
                case OPTION_CHAPTER:
                    ttsName = 	chapterInfo + " " + (optionFocusIndex_2 + tmpStartIndex + 1) + ",";
                    ttsStatus = isSelected(optionInfoArray[OPTION_CHAPTER] == optionFocusIndex_2 + tmpStartIndex);
                    break;
                case OPTION_INFORMATION:
                    ttsName = 	$("infoDetail0").innerText + ","+ $("infoDetail1").innerText + "," + $("infoDetail2").innerText + "," +
                        $("infoDetail3").innerText + "," + $("infoDetail4").innerText;
                    ttsStatus = "";
                    break;
				default:
					break;
            ...
            break;
        case PICTURE_PRESET_AREA:
            ttsName = picturePresetArray[optionFocusIndex_2] + ",";
            ttsStatus = isSelected(optionInfoArray[OPTION_PIC_PRESET] == optionFocusIndex_2);
            break;
        case PIC_ZOOM_AREA:
            ttsName = screenModeArray[optionFocusIndex_2] + ",";
            ttsStatus = isSelected(optionInfoArray[OPTION_SCREEN_MODE] == optionFocusIndex_2);
            break;
        case OPTION_BAR_AREA:
            switch(optionFocusIndex_1)
            {
            ...
            if(optionFocusIndex_1 == lastFocusIndex)
            {
                ttsName = "";
            }
            break;
        case SOUND_PRESET_AREA:
            ...
            ttsStatus = isSelected(optionInfoArray[OPTION_SOUND_PRESET] == tmpStartIndex+optionFocusIndex_2);
            break;
        case TIPS_AREA:
    ...
    lastFocusIndex = optionFocusIndex_1;
