...
function fillMetaChapterInfo()
{
    ...
    console.log("=====huangzhming fillMetaChapterInfo CurrentProgram = " + CurrentProgram);
...
{	
	var string = "";
	console.log("=====huangzhming fillMetaSoundTrackInfo index="+index);
	if (index < 0)
	{
        skipFlagSoundtrack = true;
        return;
...
function setSPOption()
{
	switch(optionFocusIndex_1)
	{
		...
		case OPTION_TITLE://title
			...
			hideMetaInfoBox(0);
			clearTimeout(operationTimer);
			operationTimer = setTimeout(function(){multimedia.changeTitle(tmpStartIndex + optionFocusIndex_2);}, DELAY_TIME);
			return;
