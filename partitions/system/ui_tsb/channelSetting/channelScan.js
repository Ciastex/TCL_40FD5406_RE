...
var STTLEnableCtryPstionInCtryIndex=[11/*Finland*/,29/*Norway*/,9/*Denmark*/,38/*Sweden*/,17/*Ireland*/,
								42/*United Kingdom*/,10/*Estonia*/];//STTL enable的国家在country_localCountry_index数组中的位置。
...
var countryPosition 	= 23;				//country对应在countryIndex中的下标；默认为泰国
...
function initOptionShowStatus(value)
{
	...
	else 
	{
		...
		else
		{
            ...
            if(top.isIRNFlag && channel.systemCountry == 60)
...
function initAPData()
{
    if (top.isIRNFlag)
	{
        countryIndex		=[68,60];				//其中>=58的是国家枚举中没有的东西 ,58为Lithuanian
        languageIndex 		= [6,503,362];
        countrysWords = [top.all_country[68],top.all_country[60]];
	}
	else
	{
        countryIndex		=[
            105,67,80,100,0,79,86,107,3,84,//10
            81,68,111,94,91,99,98,82,73,60,
            61,77,34,108,96,63,71,74,66,78,
            95,116,134,37,121,90,85,106,115,97,//40
            87,135,26,109,89,101,72,104,149,35,
            76,19,75,20,133,114,88,110,103,51,
            124,70,36,69,32,83,123,59,64,58,//70
            102,112];				//其中>=58的是国家枚举中没有的东西 ,58为Lithuanian
        languageIndex 		= [
            31,9,17,18,6,31,6,9,6,7,
            9,31,6,6,9,18,6,6,223,362,
            6,6,232,6,31,18,6,31,18,31,
            6,31,6,6,9,18,309,31,6,6,
            6,9,6,6,39,31,6,31,7,6,
            31,18,31,6,26,6,6,31,31,499,
            18,6,442,9,35,6,6,18,471,474,
            31,6];
	}
	countrySum = countryIndex.length;
	for(var i = 0; i < countryIndex.length; i++)
