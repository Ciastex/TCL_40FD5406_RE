...
function notifyProcess(e)
{
	...
	switch(msg)
	{
		...
		case top.E_BOOKING_AHEAD_RECORD_START://booking ahead record start正常情况下录制提醒,，一分钟前提醒制提醒
			doExit();//首先退出当前页面，避免booking无法跳转播放和录制
...
{channel.testLog("in  doEnter pageOn = " +　pageOn + ",list_position_h = " + list_position_h);
	switch(pageOn)
	{
		...
		case "end":
			if(speed >2000)
			{
				if(getSportsModeType())//[1]full
				{
					top.$('main').src = "pvr/sportsMode.html";
				}
				else
				{
...
function showRemind(flag)
{
	...
	if(top.g_setting.footballMode)
	{
		top.g_setting.footballMode = 0;//turn off foot ball mode
	}
...
function initSetRegion()
{
	...
	{
		var classlist;
		classlist = document.getElementById("ok");
		classlist.style.border = "medium solid" + top.uiColourStyle;
		classlist = document.getElementById("initDialog");
		classlist.style.border = "medium solid" + top.uiColourStyle;
	}
	if(getSportsModeType())//reset UI tips content for sports mode
	{
		/*
		var timeshiftInitReminds= top.timeshiftInitReminds;
		var initRemindWords   	= [top.timeshiftExtra[7],top.pvrRemindWords[6],top.timeshiftExtra[3],top.timeshiftExtra[8]];
		*/
		timeshiftInitReminds[0] = top.sportsModeInitReminds[0];
		timeshiftInitReminds[2] = top.sportsModeInitReminds[1];
		timeshiftInitReminds[5] = top.sportsModeInitReminds[2];
		timeshiftInitReminds[7] = top.sportsModeInitReminds[3];
		timeshiftInitReminds[8] = top.sportsModeInitReminds[4];
		timeshiftInitReminds[9] = top.sportsModeInitReminds[5];
		initRemindWords[2] = top.sportsModeExtra[3];
	}
}
...
function init()
{
	...
	{
		if(top.recordStatusFlag == 1 || (channel.timeshiftEnableStatus == 0 && !getSportsModeType()))
		{
...
{
	if("TRUE" != top.g_setting.getProperty("ro.sita.model.LiveTV.FullySportsMode", "FALSE"))
	{
		return 0;
	}
	...
	if(top.g_pvr.getUsbStatus() != 1 || !top.g_setting.footballMode || top.g_channel.inputSource != 1
		|| !top.g_factory.pvrOpenStatus || top.channelLock == 1 || top.inputLock == 1)
	{
		return 0;
	}
	else
