 ...
 */
var historyWords = [top.recentPageWords[1],top.videosandappsWord[1],top.recentPageWords[0],top.videosandappsWord[2],top.videosandappsWord[1],top.videosandappsWord[3],top.videosandappsWord[1],top.recentPageWords[8],top.recentPageWords[9]];
...
AppHistory.prototype = {
    ...
    initData:function () {
        var appsHistory = JSON.parse(localStorage.getItem("appHistory"));
        ...
        for(var i=0,len = appsHistory.historyItem.length;i<len;i++){
            ...
            //1、设置应用的海报
            templateDiv.getElementsByClassName('itemPic_img')[0].src = appsHistory.historyItem[i].appPicUrl;

            //2、设置应用的名字
            templateDiv.getElementsByClassName('itemName_span')[0].innerHTML = appsHistory.historyItem[i].appTitle;

            //3、设置启动应用的参数
            templateDiv.getElementsByClassName('itemDiv')[0].setAttribute("appId", appsHistory.historyItem[i].appId);
            templateDiv.getElementsByClassName('itemDiv')[0].setAttribute("appPicUrl", appsHistory.historyItem[i].appPicUrl);
            templateDiv.getElementsByClassName('itemDiv')[0].setAttribute("appTitle", appsHistory.historyItem[i].appTitle);
			templateDiv.getElementsByClassName('itemDiv')[0].setAttribute("appUrl", appsHistory.historyItem[i].appUrl);
        ...
        switch(keycode) {
            ...
            case VK_ENTER: //确认键
                if(deleteDiv.style.display == 'block'){
                    ...
                    if(this.currentArea == 'today'){
                        ...
                        if(this.deleteType == 'deleteOne'){
                            ...
                            var ah = JSON.parse(localStorage.getItem("appHistory"));
                            for(var i=0;i<ah.historyItem.length;i++){
                                if(ah.historyItem[i].appId == appId){
                                    ah.historyItem.splice(i,1);
                                }
                            }

                            localStorage.setItem("appHistory", JSON.stringify(ah));
                ...
                if(this.currentArea == 'today'){
					...
					var appUrl = this.itemDivs_1[this.items_1_index].getAttribute('appUrl');
...
window.onload = function () {
    ...
    var needTranslateSpans = document.getElementsByClassName("needTranslate_apphistory");
