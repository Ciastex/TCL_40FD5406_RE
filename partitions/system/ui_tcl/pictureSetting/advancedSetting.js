...
function toRight()
{
	...
	else if(focusType=="LEDMotioncLarity")
	{
	   ...
	   setting.previewLedMotionClarityVal(presetPositionH);
	    top.doElementScroll(presetSpans,presetPositionH);
	}
...
function toBack()
{
	...
	{
        setting.previewLedMotionClarityVal(LEDMotionClarityVal);
		updateAdvanced(picAdvConName[17]);
		returnMenuList();
	}
	else if(focusType=="FleshToneMenu")
...
function initVarValue()
{
	...
	hdmiModeVal = setting.hdmiMode;
	if (hdmiModeVal == 0)
	{
		hdmiRealMode = setting.getHdmiRealType();
	}
	else
	{
		hdmiRealMode = hdmiModeVal;
	}
...
function checkShowMenu()
{
    try
    {
        ...
        else
        {
            ...
            //var isVideo = 0;  //目前接口还没有做好，临时赋值
            var panelFrequency = setting.getProperty("ro.sita.model.TCL_PANEL_SETTING.TCL_PANEL_FREQUENCY", "60");

            if(60 == panelFrequency)
            {
            ...
            if (setting.energySaving != 0 || setting.mixDimming == 1 || top.isLAFlag)       //动态背光开时 或者 mixdimming为on时
            {
                menuList[16].showFlag = 0;
            }
        ...
        if(0 == colorEnhanceVal)     //PQ要求color extender关闭时，将flesh tone选项灰选
        {
            menuList[2].showFlag = 0;
        }
        menuList[8].showFlag = 0;      //5655EU不支持color space
    }
    catch (er)
    {
...
function ttsRead()
{
	...
	if(focusType == "advancedMenuList" && 2 == parent.currentShowMenu)
	{
		switch(currentName)
		{
			case picAdvConName[1]:
				ttsName = picAdvConName[1] + ",";
				currentStatus = offOn[dynamicConVal];
				break;
			case picAdvConName[2]:
				ttsName = picAdvConName[2] + ",";
				currentStatus = offOn[blackStrVal];
				break;
			case picAdvConName[3]:
				ttsName = picAdvConName[3] + ",";
				currentStatus = offOn[fleshToneVal];
				break;
			case picAdvConName[4]:
				ttsName = picAdvConName[4] + ",";
				currentStatus = offOn[colorEnhanceVal];
				break;
			case picAdvConName[5]:
				ttsName = picAdvConName[5] + ",";
				currentStatus = picxvYCCColorEnhance[colorEnhanceXvYCCVal];
				break;
			case picAdvConName[6]:
				ttsName = picAdvConName[6] + ",";
				currentStatus = gammaVal;
				break;
			case picAdvConName[7]:
				ttsName = picAdvConName[7] + ",";
				currentStatus = "";
				break;
			case picAdvConName[8]:
				ttsName = picAdvConName[8] + ",";
				currentStatus = "";
				break;
			case picAdvConName[9]:
				ttsName = picAdvConName[9] + ",";
				currentStatus = "";
				break;
			case picAdvConName[10]:
				ttsName = picAdvConName[10] + ",";
				currentStatus = offOn[xvYCCVal];
				break;
			case picAdvConName[11]:
				ttsName = picAdvConName[11] + ",";
				currentStatus = picAdvRGBmode[RGBVal];
				break;
			case picAdvConName[12]:
				ttsName = picAdvConName[12] + ",";
				currentStatus = offOn[gameModeVal];
				break;
			case picAdvConName[13]:
				ttsName = picAdvConName[13] + ",";
				currentStatus = picAdLevel[MPEGVal];
				break;
			case picAdvConName[14]:
				ttsName = picAdvConName[14] + ",";
				currentStatus = picAdLevel[noiseRedVal];
				break;
			case picAdvConName[15]:
				ttsName = picAdvConName[15] + ",";
				currentStatus = offOn[filmModeVal];
				break;
			case picAdvConName[16]:
				ttsName = picAdvConName[16] + ",";
				currentStatus = "";
				break;
			case picAdvConName[17]:
				ttsName = picAdvConName[17] + ",";
				currentStatus = offOn[LEDMotionClarityVal];
				break;
			case picAdvConName[18]:
				ttsName = picAdvConName[18] + ",";
				currentStatus = blurReductionVal;
				break;
			case picAdvConName[19]:
				ttsName = picAdvConName[19] + ",";
				currentStatus = judderReductionVal;
				break;
			case picAdvConName[20]:
				ttsName = picAdvConName[20] + ",";
				currentStatus = picHDMIMode[hdmiModeVal];
				break;
			case picAdvConName[21]:
				ttsName = picAdvConName[21] + ",";
				currentStatus = offOn[picFreezeVal];
				break;
			default:
				ttsName = "";
				currentStatus = "";
	...
	else if(focusType == "DynamicConMenu")
	{
		...
		isSelected(offOn[dynamicConVal] == currentStatus);
	}
	else if(focusType == "BlackStrMenu")
	{
		ttsName = "";
		currentStatus = offOn[presetPositionH];
		isSelected(offOn[blackStrVal] == currentStatus);
	}
	else if(focusType == "FleshToneMenu")
	{
		ttsName = "";
		currentStatus = offOn[presetPositionH];
		isSelected(offOn[fleshToneVal] == currentStatus);
	}
	else if(focusType == "ColorEnhanceMenu")
	{
		ttsName = "";
		currentStatus = offOn[presetPositionH];
		isSelected(offOn[colorEnhanceVal] == currentStatus);
	}
	else if(focusType == "ColorEnhanceXvYCCMenu")
	{
		ttsName = "";
		currentStatus = picxvYCCColorEnhance[presetPositionH];
		isSelected(picxvYCCColorEnhance[colorEnhanceXvYCCVal] == currentStatus);
	}
	else if(focusType == "adjustGammaBar")
	{
		ttsName = "";
	...
	else if(focusType == "xvYCCMenu")
	{
		ttsName = "";
		currentStatus = offOn[presetPositionH];
		isSelected(offOn[xvYCCVal] == currentStatus);
	}
	else if(focusType == "RGBMenu")
	{
		ttsName = "";
		currentStatus = picAdvRGBmode[presetPositionH];
		isSelected(picAdvRGBmode[RGBVal] == currentStatus);
	}
	else if(focusType == "GameModeMenu")
	{
		ttsName = "";
		currentStatus = offOn[presetPositionH];
		isSelected(offOn[gameModeVal] == currentStatus);
	}
	else if(focusType == "MPEGMenu")
	{
		ttsName = "";
		currentStatus = picAdLevel[presetPositionH];
		isSelected(picAdLevel[MPEGVal] == currentStatus);
	}
	else if(focusType == "NoiseReduMenu")
	{
		ttsName = "";
		currentStatus = picAdLevel[presetPositionH];
		isSelected(picAdLevel[noiseRedVal] == currentStatus);
	}
	else if(focusType == "FilmModeMenu")
	{
		ttsName = "";
		currentStatus = offOn[presetPositionH];
		isSelected(offOn[filmModeVal] == currentStatus);
	}
	else if(focusType == "LEDMotioncLarity")
	{
		ttsName = "";
		currentStatus = offOn[presetPositionH];
		isSelected(offOn[LEDMotionClarityVal] == currentStatus);
	}
	else if(focusType == "adjustBlurReduction")
	{
		ttsName = "";
	...
	{
		ttsName = "";
		currentStatus = picHDMIMode[presetPositionH];
		isSelected(picHDMIMode[hdmiModeVal] == currentStatus);
	}
	else if(focusType == "PicFreezeMenu")
	{
		ttsName = "";
		currentStatus = offOn[presetPositionH];
		isSelected(offOn[picFreezeVal] == currentStatus);
	}
