...
//booking消息不会在搜台的时候发送，这是由底层去判断的，代码此处直接默认处理即可
    ...
    switch(msg)
    {
		...
		case top.E_DTV_TUNE_FREQUENCY_CHANGE://dtv
            var number = channel.dtvScanGetChannelNumberOver();//1:numbers over  2.0:normal
            if(number == 1)
            {
                $("chNumOverPromt").style.display = "block";
                $("chNumContent").innerHTML = top.channelNumberOverPromt;
                $("scanForm").style.display = "none";
                focusOn = "chNumOverPromt";
...
{//channel.testLog("in auto Search keyproc");
	...
	switch(keycode)
	{
		...
		case top.VK_BACK: //back
			...
			break;
		case top.VK_EXIT: //exit
			//console.log("this is key VK_EXIT focusOn is "+focusOn);
			if(focusOn == "satEdit" || focusOn == "satellite")
			{
				channel.setServiceType(1);//dtv
				channel.inputSource = 1;//dtv
				setTimeout(function(){parent.returnPage(0);},500);
				focusOn = "exit";
			}
			else if(focusOn == "autoScan" || focusOn == "chNumOverPromt")
			{
				parent.returnPage(0);
				focusOn = "exit";
			}
...
function doEnter()
{
	...
	//console.log("in satAutoSearch doEnter is top.preTempPage is "+top.preTempPage+"top.g_temp is "+top.g_temp);
			...
			case 3:
				...
				else
				{
					...
					for(var i = 0; i < satelliteInfoList.length; i++ )
					{
						if(satelliteInfoList[i].isSelect == 1)
						{
							freq = getLocalFrequency(satelliteInfoList[i].lNBFreq,parseInt(satelliteInfoList[i].frequency));
					...
					{
						setTimeout(function(){
							$("scanStatus").innerHTML = scanStatusWords[3];},500);
						break;
					}
	...
	else if(focusOn == "satellite")
	{
        ...
        for (var i=0;i<satelliteInfoList.length;i++)
        {
            if (satelliteInfoList[i].isSelect)
                selectCount++;
        }
        if (selectCount == 20)
        {
			...
			{
                $("pleaseSelectSat").innerText = top.selectSatRemind;
                $("pleaseSelectSat").style.display = "block";
                focusOn = "remind";
                setTimeout(function(){
                    $("pleaseSelectSat").style.display = "none";
                    focusOn = "satellite";},5000);
                return;
			}
        }
		if(satelliteInfoList[list_position_h + page*ONE_PAGE_SUM].isSelect == 0)
    ...
    else if(focusOn == "chNumOverPromt")
    {
        focusOn = "exit";
        parent.returnPage(0);
        return;
    ...
    switch (id)
    {
		...
		case 2://dtv scan finish
		{
			if(isSearch == 1)//说明是正常搜台结束，而非手动停止搜台
			{
				...
				isSearch = 0;
				top.g_scanStatus = 0;
				setTimeout(function(){parent.returnPage(0);},3000);
				focusOn = "exit";
			}
...
function sliceStringToTags(tagList,string,index,cursorFlag,maxLength)
{
	...
	tagList[1].style.color=top.uiColourStyle;
...
function initOperator()
{
    ...
    if(country == 1 || country == 9) // Austria || Germany  
    {
        if(leng <= 1)
        {
            initAutoScan();
