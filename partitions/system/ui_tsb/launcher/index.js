  ...
  case VK_DOWN:
    ...
    switch (currentFocusFiled) {
		case FocusFiled_topTitle:// 上标题区域按向下的键不会有反应
			currentFocusFiled = FocusFiled_BottomTitle;
			// 取消上标题的集焦
		...
		case FocusFiled_BottomTitle:// 下标题，不会有反应
			...
			nextNodeIdValue = currentFocusObj.getAttribute('data-up');
            ...
            console.log("--huangzhming vk_down currentFocusObj = " + currentFocusObj + "  currentFocusBorderEle = " + currentFocusBorderEle);
			switchFocusOnPage(currentFocusObj, currentFocusBorderEle);
			break;
  ...
  if (iframeListDiv_0[currentElemInBottomTitleIndex].id == "tvPage") 
    {
        inputTimer = setTimeout(function () {
            ...
            {
                tcl.setting.scaleVideoWindow(95, 283, 1016, 573);
            } 
            else 
            {
                tcl.setting.scaleVideoWindow(68, 200, 723, 407);
            }
...
function switchFocusOnTitle(obj) {// 点击标题，标题的焦点产生变化的函数
  ...
  focusOnTitleCommon(obj);
	$("lfocus").style.left
	= getAbsCoorPoint(obj) + parseInt(obj.offsetWidth/2) - parseInt($("lfocus").offsetWidth/2) + 'px' ;	
  $("mainFocus").classList.add('mainFocusActive');
  currentFocusFiled = FocusFiled_BottomTitle;// 切换到下标题区
}
