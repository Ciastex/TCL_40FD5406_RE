...
function ParseElement(elementName,elementkeys)
{
    ...
    else if (elementName == "click")
    {
        if (parent.location.href.indexOf("framework.html")!=-1)
        {
            keys = collectEntries(document, "document", keys);
        }
        else
        {
            keys = collectEntries(parent.document, "parent.document", keys);
        }
        return keys;
    }
	return keys;
...
function collectIframeEntries(n, prefix, elementkeys)
{
    var keys = elementkeys;
    if (n.nodeName!="BODY" && n.nodeName!="#document" && n.innerHTML)
    {
        if (n.onclick)
        {
            var text = n.innerHTML.replace(/<\/?.+?>|[\n\t]/g,"").trim();
            console.log("liyanru check "+text);
            if (text!="")
            {
                if (n.id)
                {
                    txtLinkMap[text] = prefix+".getElementById('"+n.id+"').onclick()";
                    keys.push(text);
                }
                else
                {
                    n.id = "tcl_"+new Date().getTime();
                    txtLinkMap[text] = prefix+".getElementById('"+n.id+"').onclick()";
                    keys.push(text);
                }
            }
        }
        else if (n.onmousedown)
        {
            var text = n.innerHTML.replace(/<\/?.+?>|[\n\t]/g,"").trim();
            console.log("liyanru check "+text);
            if (text!="")
            {
                if (n.id)
                {
                    txtLinkMap[text] = "var evt = {button:0,preventDefault:function(){}};"+prefix+".getElementById('"+n.id+"').onmousedown(evt)";
                    keys.push(text);
                }
                else
                {
    ...
    {
        keys = collectIframeEntries(c[i], prefix, keys);
    }
    return keys;
}

function collectEntries(n, prefix, elementkeys)
{
    var keys = elementkeys;
    if (n.nodeName!="BODY" && n.nodeName!="#document")
    {
        if (n.nodeName == "IFRAME")
        {
            if (n.id)
            {
                keys = collectIframeEntries(n.contentDocument, prefix+".getElementById('"+n.id+"').contentDocument", keys);
            }
            else
            {
                n.id = "tcl_"+new Date().getTime();
    ...
    for(var i=0;i<c.length;i++)
    {
        keys = collectEntries(c[i], prefix, keys);
    }
    return keys;
}
...
function tcl_voice_match(value)
{
    ...
    if (x1==1)
    {
        ...
        console.log("liyanru check "+result+x2+txtLinkMap[result]+value);
        var action = txtLinkMap[result];
        eval(action);
