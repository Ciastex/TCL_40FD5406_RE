...
function showKeyBoard()
{
...

    {
        ...
        bgDiv.style.height="460px";
        bgDiv.style.width="1920px";
        bgDiv.style.position="absolute";
        bgDiv.style.top="616px";
...
function showLangSetting()
{
	...
	top.ttsLog("ttsStr="+("Keyboard and input method"+","+ttsCheckLanguage(0)));
	top.setTTSSay(top.SPEECH_TYPE_FLUSH,"Keyboard and input method"+","+ttsCheckLanguage(0));
}
var MUL_START  = 138;
var MUL_STEP   = 146;
...
function ttsGetChar()
{
	if(keyBoardFocus == "keyboard")
	{
		...
		switch(positionH)
		{
			case 0:
			{
				...
				else
				{
					if(charFlag == 1)
					{
						ttsChar = keyBoardChar[positionH][positionV];
					}
					else
					{
						ttsChar = keyBoardChar[positionH][positionV][0];	//TTS只考虑英语，不存多个候选词情况
					}
				}
			}
				break;
			case 1:
			{
				...
				else
				{
					if(keyBoardChar[positionH][positionV].length==1)
					{
						ttsChar = keyBoardChar[positionH][positionV][0];
					}
					else
					{
						ttsChar = keyBoardChar[positionH][positionV][0];	//TTS只考虑英语，不存多个候选词情况
		...
		return ttsReplaceMark(ttsChar);
	}
	else if(keyBoardFocus == "mulChar")
	{
		return keyBoardChar[positionH][positionV][mulPositionV];
	}
	else if(keyBoardFocus == "keySetting")
	{
		return ttsCheckLanguage(setPositionH);
	}
}

function ttsCheckLanguage(position)
{
	for(var i=0;i<countryCount;i++)
	{
		if(countryList[i] == position)
		{
			return language[position]+", selected";
		}
	}
	return language[position]+", not selected";
}

function ttsReplaceMark(ttsStr)
{
	ttsStr=ttsStr.replace(/#/g,"hash");
	ttsStr=ttsStr.replace(/\*/g,"asterisk");
	ttsStr=ttsStr.replace(/\(/g,"left paren");
	ttsStr=ttsStr.replace(/\)/g,"right paren");
	ttsStr=ttsStr.replace(/_/g,"underscore");
	ttsStr=ttsStr.replace(/\.\.\./g,"ellipsis");
	ttsStr=ttsStr.replace(/,/g,"comma");
	ttsStr=ttsStr.replace(/\?/g,"question mark");
	ttsStr=ttsStr.replace(/!/g,"Exclamation");
	if(0 == symbolPageFlag)
	{
		ttsStr=ttsStr.replace(/\./g,"period");
		ttsStr=ttsStr.replace(/:/g,"colon");
	}else
	{
		ttsStr=ttsStr.replace(/\./g,",point,");
		ttsStr=ttsStr.replace(/:/g,",colon");
	}
	ttsStr=ttsStr.replace(/;/g,"semicolon");
	ttsStr=ttsStr.replace(/"/g,"double quotation");
	ttsStr=ttsStr.replace(/'/g,"sigle quotation");
	ttsStr=ttsStr.replace(/</g,"less than");
	ttsStr=ttsStr.replace(/>/g,"great than");
	ttsStr=ttsStr.replace(/\[/g,"left bracket");
	ttsStr=ttsStr.replace(/]/g,"right bracket");
	ttsStr=ttsStr.replace(/}/g,"right brace");
	ttsStr=ttsStr.replace(/\{/g,"left brace");
	ttsStr=ttsStr.replace(/-/g,"en dash");
	ttsStr=ttsStr.replace(/×/g,"multiplication");
	ttsStr=ttsStr.replace(/÷/g,"division sign");
	ttsStr=ttsStr.replace(/«/g,"left double angle bracket");
	ttsStr=ttsStr.replace(/»/g,"right double angle bracket");
	ttsStr=ttsStr.replace(/¦/g,"broken bar");
	ttsStr=ttsStr.replace(/´/g,"acute");
	ttsStr=ttsStr.replace(/\^/g,"caret");
	ttsStr=ttsStr.replace(/~/g,"tilde");
	ttsStr=ttsStr.replace(/€/g,"euro");
	return ttsStr;
}
...
function ttsRead()
{
	...
	var ttsStr = "";
	if("English" == language[curCountry] || "keySetting" == keyBoardFocus)
	{
		ttsStr = ttsGetChar();
	...
	{
		top.ttsLog("current language is not English");
	}
