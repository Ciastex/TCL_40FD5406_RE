//常量-----------tclLauncherUpdateDate 20160819------------
...
(function() {
    ...
    {
        top.lastInputSource = tcl.channel.getLastinputSource();
    }
...
function parseNavData(data) {
    console.log("navigationURL "+data.result.navigation);
    if (data.result.navigation)
    {
        localStorage.setItem("navigationURL",data.result.navigation);
    }
}
...
function init() 
{
    ...
    ttsRead();
    if (tcl.utils.setData) tcl.utils.setData('zeasn-install-enable', 'true');
    if (!localStorage.getItem("navigationURL"))
    {
        var xhr = document.createElement("script");
        var brand = top.g_setting.getProperty("ro.sita.model.Apps.video_provider","null");
        if (brand=="null")
        {
            brand = top.g_setting.getProperty("ro.sita.content.provider","null");
        }
        if (brand=="null")
        {
            brand = "YouTube";
        ...
        xhr.src = "http://browsernav.api.my7v.com/tvbase/pageSearchApi/findPageConfig?callback=parseNavData&clientType="+tcl.factory.getClientTypeKey()+"&country="+tcl.setting.zoneCode+"&customerCode="+brand+"&language="+tcl.setting.menuLanguageCode;
...
function addKeyDownListener() {
  ...
  if (currentElemInBottomTitleIndex != -1 && checkId(sessionStorage.blockId)) {// 由二级目录返回
    // 切换窗口内容
...
function keyProcess(evt) 
{
  ...
  evt.preventDefault();
  console.log("keycode:"+keycode+",---top.appOpenFlag:"+top.appOpenFlag);
    if (!iframeList)
    {
        if (keycode==VK_LEFT || keycode==VK_RIGHT || keycode==VK_UP || keycode==VK_DOWN || keycode==VK_ENTER || keycode==VK_BACK)
        {
            return;
        }
    }
  ...
  case VK_UP: // 按 Up var VK_UP = 38;
    ...
    case FocusFiled_BottomTitle:// 下标题
      ...
      currentFocusObj = iframeListDiv_1[currentElemInBottomTitleIndex];
            currentFocusObj = iframeList[currentElemInBottomTitleIndex].contentWindow.document.getElementById(currentFocusObj.getAttribute("id"));
      // 查找最左下角的元素
  ...
  case VK_LEFT: // Left var VK_LEFT = 37;
    ...
    case FocusFiled_page:// page(home,videos,tv,apps)
      ...
      if (leftNodeIdValue == "-1") {
        ...
        if (1)
        {
            var n = leftNodeIdValue;
            while(true){
                var m = iframeList[currentElemInBottomTitleIndex].contentWindow.document.getElementById(n).getAttribute('data-right');
                if(m=="-1")
                    break;
                n = m;
            }
  ...
  case VK_HOME:
  {
    if (top.$("main").src.split("?")[0]!=document.location.href.split("?")[0])
    {
        return;
    }
    ...
    switch (currentFocusFiled) 
    {
      ...
      case FocusFiled_BottomTitle:
        ...
        //6、切换窗口内容、切换iframe
        refleshIframeLeft(currentElemInBottomTitleIndex);
        console.log("tempCurrentFocusFiled = " + tempCurrentFocusFiled + "  FocusFiled_BottomTitle = "  + FocusFiled_BottomTitle);
        console.log("tempCurrentElemBottomIndex = " + tempCurrentElemBottomIndex + "  currentElemInBottomTitleIndex = "  + currentElemInBottomTitleIndex);
  ...
  case VK_TV:
  {
    ...
    else
    {
      ...
      case top.VK_SOURCE_CMP1:
          hotKeyForTVSource(evt, top.E_INPUT_YPBPR);
          break;
      case top.VK_SOURCE_VGA:
          hotKeyForTVSource(evt, top.E_INPUT_PC);
          break;
      case top.VK_SOURCE_AV1:
          hotKeyForTVSource(evt, top.E_INPUT_AV_I);
          break;
      case top.VK_SOURCE_HDMI1:
          hotKeyForTVSource(evt, top.E_INPUT_HDMI_I);
          break;
      case top.VK_SOURCE_HDMI2:
          hotKeyForTVSource(evt, top.E_INPUT_HDMI_II);
          break;
      case top.VK_SOURCE_HDMI3:
          hotKeyForTVSource(evt, top.E_INPUT_HDMI_III);
          break;
 ...
 * 中东国家快捷信源键的需求
 **/
function hotKeyForTVSource(evt, source)
{
    if(top.g_factory.sourceLock == 1 && top.g_factory.hotelEnable == 1)
    {
        top.keyDownProcess(evt);
    }
    else
    {
        ...
        tcl.channel.requestMute(0);
        top.g_previousHtmlPage = "channelPlay.html";
        top.g_remindWord = "TvSourceHotKey";
        top.lastInputSource = source;
        top.$("main").src = "intermediate.html";
        top.requestFocus(top.main, 1);
    }
    onBodyBlur();
...
function focusOnTitleCommon(obj)
{
  ...
  $("mainFocus").style.left =  getAbsCoorPoint(obj) + parseInt(obj.offsetWidth/2) - parseInt($("mainFocus").offsetWidth/2) - parseInt(getAbsCoorPoint($("title"))) + 'px' ;   
...
function checkId(id){
    return iframeList[currentElemInBottomTitleIndex].contentWindow.document.getElementById(id);
}
...
function systemEventHandle(evt)
{
    ...
    switch (msg)
    {
        ...
        case E_APP_CLOSE:
            ...
            else if (top.scheduleApp == top.SCHEDULE_CAREVISION)
            {
                top.scheduleApp = 0;
                top.openCarevision();
                return;
        ...
        case E_MEDIA_USB_INSERTED:
            try {
              if(tcl.setting.getUsbResumingStatus()==0)//当STR状态时，插入U盘后 关机，开机，此时右侧没有弹出USB提示，所以不主动丢失焦点。0：插入Upan 1：U盘存在时开机
              {
                onBodyBlur();
              }
            }catch(e){
              console.log("usb-onBodyBlur");
              onBodyBlur();
            }
        ...
        case E_VOICE_CONTROL:
            top.systemEventProc(evt);
            break;
        case E_APPLIST_UPDATE:
            updateZeasn();
            break;
        case E_LAUNCHER_UPDATE:
            document.location = top.getHomepage();
            break;
        case E_DLNA_DMR_APPMANAGER_LIST:
        case E_DLNA_DMR_APPMANAGER_OPEN:
        case E_DLNA_DMR_APPMANAGER_UNINSTALL:
        case E_DLNA_DMR_APPMANAGER_INSTALL:
            top.systemEventProc(evt);
            break;
        ...
        {
            top.g_isDownloading = 1;
            top.g_downloadProgress = p1;
            if(!top.g_isForceUpgrade)
            {
                top.drawDownloadRing(p1);
            }
            break;
        }
        ...
        case top.E_DOWNLOADING_NETWORK_ABNORMAL:
                top.g_isDownloading = -1;//下载过程中断网立马进入主页的情况下， 下载中断的消息会被主页接收， 需要将g_isDownloading设置为-1，否则重新联网后无法再次下载。
                //top.systemEventProc(evt);
            break;
        default:
            break;
...
function getHomepageTTS()
{
	var homepageTTSStr = "";
	var focusName = -1;
	var ttsArry = ["history", "netflix", "Warcraft", "Krampus", "In the Heart of the Sea", "Glowie party", "Search"];
	try
	{
		focusName = currentFocusObj.id;
	} catch(e)
	{
		top.ttsLog("error:" + e);
	}

	focusName = parseInt(focusName.replace(/[^0-9]/ig, ""));
	if(-1 != focusName)
	{
		return ttsArry[focusName - 1];
	}
	else
	{
		return "";
...
function ttsRead()
{
	...
	if(currentFocusFiled == FocusFiled_topTitle)
	{
		ttsStr = toptitleList[currentElemInTopTitleIndex].getElementsByTagName("span")[0].innerHTML;
	}
	else if(currentFocusFiled == FocusFiled_BottomTitle)
	{
		ttsStr = $("mainFocus").innerHTML;
	}
	else if(currentFocusFiled == FocusFiled_page)
	{
		switch($("mainFocus").innerHTML)
		{
			case "Home":
				ttsStr = getHomepageTTS();
				break;
			case "Videos":
				try
				{
					ttsStr = currentFocusObj.getElementsByTagName("span")[0].innerHTML;
				} catch(e)
				{
					top.ttsLog("error:" + e);
				}
				break;
			case "TV":
				if("tv_00" == currentFocusObj.id)
				{
					ttsStr = "current source ";// + getCurrentSourceName();
				}
				else
				{
					try
					{
						ttsStr = currentFocusObj.getElementsByTagName("span")[0].innerText;
					} catch(e)
					{
						top.ttsLog("error:" + e);
					}
...
function getCurrentSourceName()
{
	...
	switch(tcl.channel.inputSource)
	{
		case top.E_INPUT_ATV:
		case top.E_INPUT_DTV:
			currentSourceName = top.sourceOptions[0];
			break;
		case top.E_INPUT_AV_I:
		case top.E_INPUT_AV_II:
			currentSourceName = top.sourceOptions[1];
			break;
		case top.E_INPUT_YPBPR:
			currentSourceName = top.sourceOptions[2];
			break;
		case top.E_INPUT_PC:
			currentSourceName = top.sourceOptions[7];
			break;
		case top.E_INPUT_HDMI_I:
			currentSourceName = top.sourceOptions[3];
			break;
		case top.E_INPUT_HDMI_II:
			currentSourceName = top.sourceOptions[4];
			break;
		case top.E_INPUT_HDMI_III:
			currentSourceName = top.sourceOptions[5];
			break;
		case top.E_INPUT_STORAGE:
			currentSourceName = top.sourceOptions[8];
			break;
		case top.E_INPUT_SCART:
			currentSourceName = "SCART";
			break;
		default:
			break;
	}
