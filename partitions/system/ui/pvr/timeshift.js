	...
	*/
	switch(msg)
	{
		...
		case top.E_PVR_NO_SIGNAL:	
			...
			top.RemoteConntrolType = "default";//lqt---恢复默认的类型
			pvr.timeshiftStop();
		...
		case top.E_DTV_SS_INPUT_BLOCK://信源锁//对于此消息，我们只需要处理页面显示就可，所有停止pvr、timeshift的函数调用都在底层处理
			...
			break;
		case top.E_PVR_RECORD_ERR:
			//对于此消息，我们只需要处理页面显示就可，所有停止pvr、timeshift的函数调用都在底层处理
			if(top.timeShiftStatusFlag == 1)
			{
				focusOn = "notAvailable";
				showRemind();
...
function keyproc(e)
{
	...
	switch(keycode)
	{
		...
		case top.VK_PLAY_PAUSE://pause/play
			toPauseOrPlayKey();//toPauseOrPlayKey();
			break;
		case top.VK_PAUSE:
			toPause();
			break;
		...
		if(list_position_h == 0)
		{
			...
			switch(focusOn)
			{
				case "stop":
					...
					//},500);
					console.log("urlValues[1] is "+urlValues[1]+" top.timeShiftStatusFlag is "+top.timeShiftStatusFlag);
					if(urlValues[1] == "power") //时移用户操作power键，需要待机
					{
						top.toPowerOff();
						top.showMute(0);
					}
				...
				case "guide":
					...
					else
					{
						...
						if(top.isNAFlag)
						{	
							setTimeout(function(){top.jumpHotkeyPage("programGuide_NA.html")},1000);
						}
						else
						{
				...
				case "inputsource":
					...
					pvr.returnToLive();//返回实时播放页面
					pvr.pvrMhegSuspend(0);/*将mheg状态和显示恢复*/
					console.log("urlValues[1] is "+urlValues[1]);
					if("nochange" != urlValues[1])
					{
						channel.inputSource=urlValues[1];
					}
                    			top.toSource();
...
function doExit()
{
	...
	{
		focusOn = "stop";
		setting.setProperty("memory.app.ui.powerkeyflag", "1");
		showRemind();
	}
}
...
function refreshMenuShow()
{
	...
	//确定时间片位置和显示开始两个时间和时间片 400长度对应30*60秒
//	$("timeshiftTimepieceContent").style.left = (timeshiftFormOffsetPosition -  parseInt((startTimeshiftTime + playBackOffsetTime - baseTime)*2/9,10)) + "px";
//	var t = parseInt((startTimeshiftTime + playBackOffsetTime - baseTime)/(30*60),10);
	$("timeshiftTimepieceContent").style.left = (timeshiftFormOffsetPosition -  parseInt(playBackOffsetTime*2/9,10)) + "px";
	var t = parseInt(playBackOffsetTime/(30*60),10);
    console.log("Log from timeshift.js:"+"====================:"+baseTime+":"+startTimeshiftTime+":"+playBackOffsetTime+":"+recordedDuration+":"+t);
...
function initSetRegion()
{
	if (top.uiColourStyleFlag == 1) {
		var classlist;
		classlist = document.getElementsByClassName("timeshiftTimePieceImg");
