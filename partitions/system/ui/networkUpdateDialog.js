...
function toLeft(){
	if(isButtonShow){
		...
		else if(networkUpdateDialogType == 4)
		{
...
function showUpdateDialogAfterDownload(){
	if(top.g_isForceUpgrade)     //强制升级不显示never
	{
		var lastSoftwareVersion = localStorage.getItem("software_version_before_force_upgrade");
		var currentSoftwareVersion = tcl.factory.getProductInfo().swName;
		if(lastSoftwareVersion == currentSoftwareVersion)		//当前软件版本等于上次强制升级选择later或者update时存的软件版本，说明选择的是later,直接升级，不再提醒
		{
			setting.networkUpdateButton = 0;
			setting.startUpdating(1);
		}
		...
		{
			localStorage.setItem("software_version_before_force_upgrade", currentSoftwareVersion);
			updateInfo = setting.getUpdateInfo();
			networkUpdateDialogType = 4;
			isButtonShow = true;
			whereIsTheButton = "right";
			$("networkUpdateDialogTitle").innerHTML = titleText;
			$("networkUpdateDialogButtonLeft").innerHTML = buttonText[2];
			$("networkUpdateDialogButtonCenter").style.display = "none";
			$("networkUpdateDialogButtonRight").innerHTML = buttonText[1];
			if(updateInfo)
			{
				if(undefined == promptText[5])
				{
					var tempText = "Software downloaded, do you want to update now? TV will update automatically next time if you skip now.";
					$("networkUpdateDialogContent").innerHTML = tempText.replace(/XXXX/, updateInfo.version);
					/* IR3465 1->4 */
				}
				else
				{
					$("networkUpdateDialogContent").innerHTML = promptText[5].replace(/XXXX/, updateInfo.version);
					/* IR3465 1->4 */
				}
			}
		}
