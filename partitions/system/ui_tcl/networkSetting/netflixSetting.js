var netflixConName         = [top.netOptions[8],top.netFlixOptions[0],top.netFlixOptions[1],top.netFlixOptions[2]];
...
function init()
{
    ...
    $("titleNameSpan").innerHTML=netflixConName[0];
	for(var i=1;i<=3;i++)
	{
...
function ttsRead()
{
	...
	if("netflixMenu" == focusType && 2 == parent.currentShowMenu)
	{
		switch(positionV)
		{
			case 0:
				ttsName = netflixConName[1];
				currentStatus = "";
				break;
			case 1:
				ttsName = netflixConName[2];
				currentStatus = "";
				break;
			case 2:
				ttsName = netflixConName[3];
				currentStatus = "";
				break;
			default:
				ttsName = "";
	...
	else if("ESNPop" == focusType)  //soundPreset选项菜单
	{
		ttsName = $("popContent").innerHTML;
		currentStatus = "";
	}
	else if("deactivatePop" == focusType)
	{
		ttsName = netFlixDes[0] + ",";
		currentStatus = (0 == positionPopH)?okCancel[1]:okCancel[0];	//positionPopH等于0是OK
	}
	if(lastFocusType == focusType && lastPositionV == positionV)
	{
		ttsName = "";
	}
