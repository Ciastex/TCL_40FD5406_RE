...
var isUpdateEPGevent			= 0;//用于记录当前是否在更新EPG信息，避免异步更新EPG事件列表导致UI显示错误
var VKDownUPTimer 				= 0;
var EPGEventUPdateTimer 		= 0;
var DuringTime					= 3000;
...
function keyproc(e)
{
	...
	{
		if(keycode == top.VK_LEFT || keycode == top.VK_RIGHT || keycode == top.VK_UP || keycode == top.VK_DOWN){
			isUpdateEPGevent = 1;
			clearTimeout(VKDownUPTimer);
			VKDownUPTimer = 0;
			VKDownUPTimer = setTimeout(function (){
				isUpdateEPGevent = 0;
				VKDownUPTimer = 0;
			}, DuringTime);
		}
	...
	else
	{
		switch(keycode)
		{
			case top.VK_BACK:
				...
				break;
			case top.VK_ENTER:
				if(allDigital != 0) //favorite列表无节目,ALL列表有DTV节目
				{
					top.toChannelList();
					return;
				}
...
function notifyProcess(evt)
{
	...
	switch(msg) {
		...
		case top.E_DTV_EPG_SCH_FINISH://epg refresh
			clearTimeout(EPGEventUPdateTimer);
			EPGEventUPdateTimer = 0;
			EPGEventUPdateTimer = setTimeout(function (){
				if (isUpdateEPGevent == 1) {//正在更新EPG事件时，则不要再异步去更新
					console.log("hevens E_DTV_EPG_SCH_FINISH isUpdateEPGevent ,return!!! ");
					return;
				}
		...
		case top.E_DTV_TOT_UPDATE://tot refresh
			if (isUpdateEPGevent == 1) {//正在更新EPG事件时，则不要再异步去更新
				console.log("hevens  E_DTV_TOT_UPDATE isUpdateEPGevent ,return!!! ");
				break;
			}
...
{
	//console.log("in  showBoldFav channelLength = " + channelLength);
	genreStatus = channel.favGenre.split("/");
	for(var i = 0; i < 6 && i < channelLength; i++)
	{
		//console.log("in  showBoldFav MAX_programCountOnPiece[" + i + "] = " + MAX_programCountOnPiece[i]);
		...
		for(var j = 0; j < MAX_programCountOnPiece[i] && (isHaveProgramOnPiece_list[i] == 1)/*have program*/; j++)
		{
			...
			//console.log("in  showBoldFav pgenre = " + pgenre);
			var firstGenre = pgenre>>4;
			//console.log("in  showBoldFav firstGenre = " + firstGenre);
			...
			{
				if(firstGenre < 0 || firstGenre > 15)//LA项目
				{
					tempSpanList[j].style.color = "#FFF";
				}
				else
				{
					var systemGenreInfo = genreStatus[firstGenre];
					...
					//console.log("in  showBoldFav systemGenreInfo = " + systemGenreInfo + " systemFirstGenre = " + systemFirstGenre + " systemSecondGenre = " + systemSecondGenre);
	...
	//console.log("in  showBoldFav end");
}
...
function refreshChannelShow(firstChannelIndex,flag)
{
	...
	else
	{
		...
		var tLists = channel.getEpgPrevChannelList(chListType,firstChannelIndex,0/*0 list*/,6);//获取的六个频道中不包含firstChannelIndex这个频道的内容
		len = tLists.length;
		for(var i = 0; i < 6 && i < len; i++)
		{			
			channelInfos_list[i] = tLists[i];
	...
	for(var i = 0; i < 6; i++)
	{
		...
		if(i < len)
		{
			...
			infoTag[0].innerText = (top.isLAFlag || top.isNAFlag) ? top.parseChNumber(channelInfos_list[i]) : channelInfos_list[i].number;
...
function showContent(channelPosition,eventPosition)
{
	...
	else
	{
		...
		{
			var name = nowEvent.name;
			console.log(" name 1  "+name+" length "+name.length); 
			name = name.replace(/[\r\n]/g, " "); 
			console.log(" name 2 "+name+" length "+name.length);
			$("program_content_name").innerText = name;
		...
		{
            $('program_content_timeInfo').innerText = nowEvent.startTime + "-" + nowEvent.endTime;
        }
		else
		{
		...
		if(top.isLAFlag && !top.isCBFlag)
		{
			...
			$("program_content_cc").style.display = "block";
		...
		{
			setNZParentRating(rate, $("parentRating"));
		}
		else if(top.isAPFlag && 36 == channel.systemCountry)
		{
			setAPParentRating(rate, $("parentRating"));
		}
		else if (top.isLAFlag && !top.isCBFlag)
		{	
			//console.log("liukl LA rate is "+rate);
			var lockLevel = (rate >> 4) & 0x0f;
			rate = rate & 0x0f;
			//console.log("liukl LA rate is "+rate+" lockLevel is "+lockLevel);
			if(rate == 0)
			{
				$("parentRating").innerText = "";
			}	
			...
			{
				$("parentRating").innerText = ageRatingWords[rate - 1];
			}
			if(lockLevel == 0)
			{
				$("program_content_rating").innerText = "";
			...
			{
				$("program_content_rating").innerText = contentRatingWords[lockLevel];
			}	
		}
		...
		console.log("nowEvent.id is "+nowEvent.id+"  channel.getCurrentChannelInfo().getProgramPlayList()[0].id is "+channel.getCurrentChannelInfo().getProgramPlayList()[0].id);
		...
		if(channelInfos_list[channelPosition].index == curIndex && nowEvent.id == channel.getCurrentChannelInfo().getProgramPlayList()[0].id)//(firstProgramNumOnPiece_list[channelPosition] + eventPosition) == 0)//当前频道的第一个事件
		{
            ...
            if(setting.isCCAvailable()==1 && !top.isCBFlag)
            {
                $("program_content_cc").style.display = "block";
				$("program_content_cc").innerText = "CC";
            }
...
function showGenre(genre)
{
	...
	if(top.isLAFlag)
	{
		//console.log("liukl LA -- firstLevel is "+firstLevel);
		$('program_content_genre').style.top = "10px";
		$('program_content_genre').style.left = "1500px";
		$('program_content_genre').innerText = genre_first_level_LA[firstLevel];
	}
	else
	{
		$('program_content_genre').style.top = "50px";
		$('program_content_genre').style.left = "500px";
...
function showProgramToLeft()
{
	...
	fucous_program[positionOfPiece].style.border = "thin "+top.uiColourStyle+" solid";
	fucous_program[positionOfPiece].style.zIndex = 100;
	if(top.isLAFlag && fucous_program.length < programInfos_list[list_position_h].length)//in case LA that get data not in current time piece.
	{
...
function refreshPieceTime(systemDate,distance,pieceStartTime)
{
    	...
    	if (setting.getProperty("app.persiandate.switch","0") == 1)
		{
            var str1 = calendarToPersian(str);
            $('program_date').innerText = str1;
		}
		else
		{
...
function refreshTime()
{
    ...
    if (setting.getProperty("app.persiandate.switch","0") == 1)
    {
        var nowDate1 = calendarToPersian(nowDate);
        $("dateTime").innerText = nowDate1 + " " + nowTime;
    }
    else
	{
...
var allDigital = 0;//判断当前电视机下是否有DTV节目
function init()
{
    top.initTextDirection(window);
	country = channel.systemCountry;
	if(country != 25)//UK 进入EPG不关闭subtitle
	{
		//top.closeSubtitle();
	...
	allDigital = channel.getEpgNextChannelList(1/*digital*/,tChannel.index,0/*0 list*/,6).length;
	console.log("in init allDigital is "+allDigital);
	if(channelStatus!=0 && listType == 5)//favorite channel list
	{
		var num = channel.getEpgNextChannelList(listType,tChannel.isSkip?channel.nextIndex:curIndex = tChannel.index,0,6).length;
		if(num == 0)
		{
			channelStatus = 0;
		}
	...
	if(channelStatus)//有频道
	{
		...
		for(var i = 0; i < 5; i++)
		{
			...
			if( i == 4)
			{	
				hotkeySpanList[4].style.width = "225px";		
	...
	else if(allDigital != 0 && channelStatus == 0)//favorite列表无节目,ALL列表有DTV节目
	{
		$("no_channel").innerText = top.favChannelListRemind;
		$("no_channel").style.display = "block";
		$("firstButton").innerText = top.chListWords[1];//Select list
