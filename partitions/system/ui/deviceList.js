...
var deviceDLNAText = top.smartTVOptions[2];
var pvrList = "PVR";
...
function notifyProcess(evt)
{
	...
	switch(msg)
	{
		case top.E_MEDIA_USB_INSERTED:
			...
			else if(setting.getUsbResumingStatus()==0)
			{
				displayFocusBox(0);
...
function refreshMediaEvent(evt)
{
	...
	switch(msg)
	{
		...
		case top.E_READY_TO_CHANGE_CEC:
            ...
            break;
		case top.E_STR_SCREEN_OFF:
            setTimeout("top.cancelForcePowerOff();", 500);
			return 1;
			break;
		case top.E_APP_PROCESS_INITIALIZATION:
			switch(top.g_lastEnterType)
...
function changeDivStyle(index_1, index_2)
{
	if(index_1 != -1)
	{
		...
		if($(window_name + index_1).getElementsByTagName("img")[0].src.indexOf("USB_2.png")!=-1)
		{
			$(window_name + index_1).getElementsByTagName("img")[0].src = "images/USB.png";
		}
		else if($(window_name + index_1).getElementsByTagName("img")[0].src.indexOf("DLNA_2.png")!=-1)
		{
	...
	if(index_2 != -1)
	{
		...
		if($(window_name + index_2).getElementsByTagName("img")[0].src.indexOf("USB.png")!=-1)
		{
			$(window_name + index_2).getElementsByTagName("img")[0].src = "images/USB_2.png";
		}
		else if($(window_name + index_2).getElementsByTagName("img")[0].src.indexOf("DLNA.png")!=-1)
		{
...
function toBack()
{
	...
	top.requestFocus(top.main, 1);
	//setTimeout(function(){multimedia.toRestoreSource(0, 0);},1000);//remove para
}
...
{
	// console.log("-------ttstest------listIndex="+listIndex+"  listStart="+listStart);
	// console.log("------ttstest------pvrListLength="+pvrListLength+"   usbListLength="+usbListLength+" dlnaListLength="+dlnaListLength);
	ttsArray.splice(0,ttsArray.length);
	if(1 == pvrListLength)
	{
		ttsArray.push("PVR");
	}
...
function ttsRead()
{
	...
	else if("block" == $("noDeviceTip").style.display)
	{
		console.log("------------ttstest----------ttsStr=" + emptyListInfo);
		top.setTTSSay(top.SPEECH_TYPE_FLUSH, emptyListInfo);
	}
}