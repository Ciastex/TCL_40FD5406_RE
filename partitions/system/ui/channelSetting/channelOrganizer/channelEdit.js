...
function toOK()//按OK键
{
   ...
   else if(focusType=="channelEdit")
   {
	...
	else if(focusType == "channelShowKey")
	{
		...
		{
			$("nameSpan"+positionEditH).innerHTML=replaceT(key);
			...
			{
				//channelNameIndex++;
				channelNameIndex += charlen;
				if(channelNameIndex>channelName.length || channelNameIndex>19)
				{
					channelNameIndex=0;
				}
				resolveChannelName_ext(channelName,channelNameIndex,charlen);
				inputOkFlag=0;
			}
			else
			{
...
function resolveChannelName(name,index)//分离出频道名在分散在三个span中
{
	if(index<0 || name.length==0)
	{
		...
		channelNameIndex=0;
		$("nameSpan0").innerHTML=replaceT(channelEditName[0]);
		$("nameSpan1").innerHTML=replaceT(channelEditName[1]);
		$("nameSpan2").innerHTML=replaceT(channelEditName[2]);
	}
	else if(index==0)
	{
...
function resolveChannelName_ext(name,index,charlen)//分离出频道名在分散在三个span中
{
	if(index<0 || name.length==0)
	{
		channelEditName[0]="_";
	...
	else if((index==name.length)||(index+charlen-1 == name.length))
	{
		channelEditName[0]=name.substring(0,name.length);
		channelEditName[1]="_";
		channelEditName[2]=""
	...
	{
		channelEditName[0]=name.substring(0, index+charlen-1);
		channelEditName[1]=name.substring(index+charlen-1, index+charlen);
		channelEditName[2]=name.substring(index+charlen, name.length);
         ...
         /*   else
            {
             //   keyCount++;
                ...
                inputChar = char_table[keyTableLine][keyCount];
            }*/ // remove the RCkey to change letters. by hhf
			$("nameSpan"+positionEditH).innerHTML=inputChar;
			channelEditName[positionEditH]=inputChar;
			channelName=channelEditName[0];
			if(channelEditName[1]!=""&&channelEditName[1]!="_")
...
function deleteChannelName()
{
	...
	if("لا" == channelName.substring(channelName.length-2, channelName.length))
	{
		channelNameIndex-=2;/*The length of 'لا' is 2, so it must -2.*/
